#!/usr/bin/env python3
"""NewzDeck v3.6.63 terminal-history index and diagnostics efficiency guards."""
...
if sab.TERMINAL_HISTORY_VERSION!=3 or sab.TERMINAL_HISTORY_MAX_ROWS!=5000:
    raise SystemExit('v3.6.63 changed the durable terminal-history v3/5,000-row contract.')
if sab.STATISTICS_ACCOUNTED_MAX_ROWS!=20000:
    raise SystemExit('v3.6.63 changed the 20,000-ID statistics accounting boundary.')
...
for marker in (
    ...
    if marker not in sab_source:
        raise SystemExit(f'Missing v3.6.63 history/overlap marker: {marker}')
...
for marker in (
    ...
    if marker not in server_source:
        raise SystemExit(f'Missing v3.6.63 diagnostics marker: {marker}')
for forbidden in (
...
if "const UI_VERSION = '3.6.63';" not in js:
    raise SystemExit('UI version is not 3.6.63.')
if '3.6.63-name-resolution-render-coalescing-thumbnail-trace-telemetry' not in index:
    raise SystemExit('Static asset cache identity is not v3.6.63.')
...
print('v3.6.63 regression guard passed (no-op history fast path + one rebuild per mutation + durable diagnostic counts + N/A provider measurements + overlap provenance).')
