...
server=(APP/'server.py').read_text(encoding='utf-8'); app=(APP/'static'/'app.js').read_text(encoding='utf-8'); index=(APP/'static'/'index.html').read_text(encoding='utf-8'); manifest=json.loads((APP/'build-manifest.json').read_text(encoding='utf-8')); sab=load('newzdeck_v3670_sab_guard',APP/'sab_engine.py')
# New observability only.
for marker in (
 'elif stage.startswith("thumbnail_") or stage == "video_thumbnail_post":',
 'function videoThumbnailError(code,message)',
 "videoThumbnailError('sample-no-frame'",
 "videoThumbnailError('browser-frame-timeout'",
 "videoThumbnailError('browser-decode-failed'",
 "videoThumbnailError('frame-capture-failed'",
 "?'frame-unavailable':'ffmpeg-required'",
 'thumb-store-${thumbnailErrorReason(e)}',
 "perfRecord('video_thumbnail_post'",
): check(marker in (server+app),'v3.6.70 Video post diagnostics marker missing: '+marker)
check('"schema_version": 8' in server and '"contract": "passive-runtime-browsing-performance"' in server,'Browsing telemetry schema 8/contract missing')
...
check((APP/'version.txt').read_text().strip()=='3.6.70','version.txt mismatch')
check("const UI_VERSION = '3.6.70'" in app and '3.6.70-video-thumbnail-post-diagnostics' in index,'UI/cache identity mismatch')
check(manifest.get('version')=='3.6.70' and manifest.get('base_version')=='3.6.69' and manifest.get('adapter_version')=='3.6.70','build manifest identity mismatch')
check(sab.ADAPTER_VERSION=='3.6.70' and sab.SAB_VERSION=='5.1.2','SAB identity changed')
