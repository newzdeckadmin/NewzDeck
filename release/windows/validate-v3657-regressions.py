#!/usr/bin/env python3
"""NewzDeck v3.6.57 import-hold semantics and Downloads-count integrity guards."""
...
if sab.ADAPTER_VERSION!='3.6.57': raise SystemExit(f'Wrong SAB adapter version: {sab.ADAPTER_VERSION}')
...
def make_auto(root:pathlib.Path):
    return auto.MediaAutomationEngine(root,lambda value:value,lambda value:value,DummyDownloadManager(),lambda:[],version='3.6.57')
...
with tempfile.TemporaryDirectory(prefix='newzdeck-v3657-terminal-counts-') as td:
    ...
    with mgr._terminal_history_lock:
        mgr._terminal_history={'version':2,'rows':rows}
...
for marker in (
    'ADAPTER_VERSION = "3.6.57"','_terminal_transfer_status_counts','terminal_presentation_counts','terminal_transfer_counts',
    ...
    if marker not in sab_source: raise SystemExit(f'Missing v3.6.57 SAB/count marker: {marker}')
for marker in (
    ...
    if marker not in auto_source: raise SystemExit(f'Missing v3.6.57 Automation integrity-hold marker: {marker}')
for marker in (
    'APP_VERSION = "3.6.57"',"'presentation_terminal_counts':presentation_terminal_counts","'transfer_terminal_counts':transfer_terminal_counts",
    ...
    if marker not in server_source: raise SystemExit(f'Missing v3.6.57 diagnostics/count marker: {marker}')
for marker in ("const UI_VERSION = '3.6.57';",'INTEGRITY HOLDS','automatic search paused'):
    if marker not in js: raise SystemExit(f'Missing v3.6.57 UI hold marker: {marker}')
if '3.6.57-import-hold-count-integrity' not in index: raise SystemExit('Static cache identity is not v3.6.57.')
...
print('v3.6.57 regression guard passed (presentation-vs-transfer counts + explicit import_integrity_hold + exact release blacklist + repeated-conflict target pause + carried v3.6.56 protections).')
