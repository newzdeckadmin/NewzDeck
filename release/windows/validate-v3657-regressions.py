#!/usr/bin/env python3
"""NewzDeck v3.6.72 import-hold semantics and Downloads-count integrity guards."""
...
if sab.ADAPTER_VERSION!='3.6.72': raise SystemExit(f'Wrong SAB adapter version: {sab.ADAPTER_VERSION}')
...
def make_auto(root:pathlib.Path):
    return auto.MediaAutomationEngine(root,lambda value:value,lambda value:value,DummyDownloadManager(),lambda:[],version='3.6.72')
...
for marker in (
    'ADAPTER_VERSION = "3.6.72"','_terminal_transfer_status_counts','terminal_presentation_counts','terminal_transfer_counts',
    ...
    if marker not in sab_source: raise SystemExit(f'Missing v3.6.72 SAB/count marker: {marker}')
for marker in (
    ...
    if marker not in auto_source: raise SystemExit(f'Missing v3.6.72 Automation integrity-hold marker: {marker}')
for marker in (
    'APP_VERSION = "3.6.72"',"'presentation_terminal_counts':presentation_terminal_counts","'transfer_terminal_counts':transfer_terminal_counts",
    ...
    if marker not in server_source: raise SystemExit(f'Missing v3.6.72 diagnostics/count marker: {marker}')
for marker in ("const UI_VERSION = '3.6.72';",'INTEGRITY HOLDS','automatic search paused'):
    if marker not in js: raise SystemExit(f'Missing v3.6.72 UI hold marker: {marker}')
if '3.6.72-browse-timeout-error-attribution' not in index: raise SystemExit('Static cache identity is not v3.6.72.')
manifest=json.loads(MANIFEST.read_text(encoding='utf-8'))
if manifest.get('version')!='3.6.72' or manifest.get('adapter_version')!='3.6.72' or manifest.get('base_version')!='3.6.71':
...
if WORKFLOW.exists() and 'python release/windows/validate-v3657-regressions.py' not in WORKFLOW.read_text(encoding='utf-8'):
    raise SystemExit('Canonical release workflow does not run the v3.6.72 regression guard.')
print('v3.6.72 regression guard passed (presentation-vs-transfer counts + explicit import_integrity_hold + exact release blacklist + repeated-conflict target pause + carried v3.6.56 protections).')
