#!/usr/bin/env python3
"""NewzDeck v3.6.59 import-hold semantics and Downloads-count integrity guards."""
...
for marker in (
    'ADAPTER_VERSION = "3.6.59"','_terminal_transfer_status_counts','terminal_presentation_counts','terminal_transfer_counts',
    ...
    if marker not in sab_source: raise SystemExit(f'Missing v3.6.59 SAB/count marker: {marker}')
for marker in (
    ...
    if marker not in auto_source: raise SystemExit(f'Missing v3.6.59 Automation integrity-hold marker: {marker}')
for marker in (
    'APP_VERSION = "3.6.59"',"'presentation_terminal_counts':presentation_terminal_counts","'transfer_terminal_counts':transfer_terminal_counts",
    ...
    if marker not in server_source: raise SystemExit(f'Missing v3.6.59 diagnostics/count marker: {marker}')
for marker in ("const UI_VERSION = '3.6.59';",'INTEGRITY HOLDS','automatic search paused'):
    if marker not in js: raise SystemExit(f'Missing v3.6.59 UI hold marker: {marker}')
if '3.6.59-discover-responsiveness-metadata-cache-efficiency' not in index: raise SystemExit('Static cache identity is not v3.6.59.')
...
print('v3.6.59 regression guard passed (presentation-vs-transfer counts + explicit import_integrity_hold + exact release blacklist + repeated-conflict target pause + carried v3.6.56 protections).')
