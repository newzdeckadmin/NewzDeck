#!/usr/bin/env python3
"""NewzDeck v3.7.0 import-hold semantics and Downloads-count integrity guards."""
...
if sab.ADAPTER_VERSION!='3.7.0': raise SystemExit(f'Wrong SAB adapter version: {sab.ADAPTER_VERSION}')
...
def make_auto(root:pathlib.Path):
    return auto.MediaAutomationEngine(root,lambda value:value,lambda value:value,DummyDownloadManager(),lambda:[],version='3.7.0')
...
for marker in (
    'ADAPTER_VERSION = "3.7.0"','_terminal_transfer_status_counts','terminal_presentation_counts','terminal_transfer_counts',
    ...
    if marker not in sab_source: raise SystemExit(f'Missing v3.7.0 SAB/count marker: {marker}')
for marker in (
    ...
    if marker not in auto_source: raise SystemExit(f'Missing v3.7.0 Automation integrity-hold marker: {marker}')
for marker in (
    'APP_VERSION = "3.7.0"',"'presentation_terminal_counts':presentation_terminal_counts","'transfer_terminal_counts':transfer_terminal_counts",
    ...
    if marker not in server_source: raise SystemExit(f'Missing v3.7.0 diagnostics/count marker: {marker}')
for marker in ("const UI_VERSION = '3.7.0';",'INTEGRITY HOLDS','automatic search paused'):
    if marker not in js: raise SystemExit(f'Missing v3.7.0 UI hold marker: {marker}')
if '3.7.0-production-milestone-repository-hygiene' not in index: raise SystemExit('Static cache identity is not v3.7.0.')
manifest=json.loads(MANIFEST.read_text(encoding='utf-8'))
if manifest.get('version')!='3.7.0' or manifest.get('adapter_version')!='3.7.0' or manifest.get('base_version')!='3.6.99':
...
if WORKFLOW.exists() and 'python release/windows/validate-v3657-regressions.py' not in WORKFLOW.read_text(encoding='utf-8'):
    raise SystemExit('Canonical release workflow does not run the v3.7.0 regression guard.')
print('v3.7.0 regression guard passed (presentation-vs-transfer counts + explicit import_integrity_hold + exact release blacklist + repeated-conflict target pause + carried v3.6.56 protections).')
