...
def make_auto(root: Path):
    engine = auto.MediaAutomationEngine(root, lambda value: value, lambda value: value, DummyDownloadManager(), lambda: [], version='3.6.66')
...
check('cache_write_requests=' in server_text and 'library_index_builds=' in server_text, 'Diagnostics report does not expose v3.6.66 local-data-path telemetry.')
...
check('3.6.66-preview-failure-classification-video-thumbnail-telemetry' in index_html, 'v3.6.66 static cache identity marker is missing.')
check('<div class="version"><b>NewzDeck</b><span>v3.6.66</span></div>' in index_html, 'Visible sidebar footer version is not v3.6.66.')
...
check(sab.SAB_VERSION == '5.1.2' and sab.TERMINAL_HISTORY_VERSION == 3, 'v3.6.66 changed SAB 5.1.2 or terminal-history schema unexpectedly.')
