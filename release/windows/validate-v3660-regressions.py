...
def make_auto(root: Path):
    engine = auto.MediaAutomationEngine(root, lambda value: value, lambda value: value, DummyDownloadManager(), lambda: [], version='3.7.0')
...
check('cache_write_requests=' in server_text and 'library_index_builds=' in server_text, 'Diagnostics report does not expose v3.7.0 local-data-path telemetry.')
...
check('3.7.0-production-milestone-repository-hygiene' in index_html, 'v3.7.0 static cache identity marker is missing.')
check('<div class="version"><b>NewzDeck</b><span>v3.7.0</span></div>' in index_html, 'Visible sidebar footer version is not v3.7.0.')
...
check(sab.SAB_VERSION == '5.1.2' and sab.TERMINAL_HISTORY_VERSION == 3, 'v3.7.0 changed SAB 5.1.2 or terminal-history schema unexpectedly.')
