...
for rel,expected in EXPECTED.items(): check(sha(APP/rel)==expected,f'{rel} differs from reviewed v3.6.95 carried-forward payload')
...
ns={'re':re,'Any':object,'APP_VERSION':'3.6.95','installed_version':lambda:'3.6.95'}
exec(compile(mod,'<v3683-version-helpers>','exec'),ns)
check(ns['_update_available_for']('3.6.95','3.6.95') is False,'same-version release was incorrectly treated as an update')
check(ns['_update_available_for']('3.6.96','3.6.95') is True,'newer release was not treated as an update')
check(ns['_update_available_for']('3.6.92','3.6.95') is False,'older release was incorrectly treated as an update')
check(ns['_update_cache_matches_install']({'current_version':'3.6.92'},'3.6.95') is False,'old-version update cache accepted')
check(ns['_update_cache_matches_install']({'installed_version':'3.6.95'},'3.6.95') is True,'current-version update cache rejected')
...
print('v3.6.83 Quality Profile UI & Update Version Coherency carried-forward guard under v3.6.95: PASS')
