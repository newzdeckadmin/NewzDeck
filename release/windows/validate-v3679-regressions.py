...
check(normalized_hash(server,'3.6.78','3.6.79')=='91ccda6f90cfccf1c2706805c89635bc23e0cb00442424891c807a55ff274dd4','server.py changed beyond APP_VERSION')
check(normalized_hash(sab_text,'3.6.78','3.6.79')=='7cf1daa207637f1465b93f0aabfe477f477e7dd0e80caabbf130becdd7cd3745','sab_engine.py changed beyond ADAPTER_VERSION')
check(normalized_hash(automation,'3.6.78','3.6.79')=='1bf3597b2e8d6e23423de44d3329307b0d77b4910eef1f5662f74d898f996ef8','automation_engine.py changed beyond version identity')
check(normalized_hash(app,'3.6.78','3.6.79')=='6ae8856ea6c02376a77c29e95f4c65d30fbff6ae1931acfa1428795b816f4e14','app.js logic changed; v3.6.79 must remain CSS-only')
normalized_index=index.replace('3.6.79-ux-feedback-state-clarity','3.6.78-defender-lf-build-pipeline').replace('v3.6.79','v3.6.78')
...
check(styles.count(marker)==1,'v3.6.79 Phase 3 stylesheet marker count mismatch')
prefix,suffix=styles.split(marker,1)
check(digest_text(prefix.rstrip('\n')+'\n')=='62d5bd56651caa48cf9536f3524cad6b8a26c5dbb978a2d2f7fe24244481eb71','Pre-v3.6.79 stylesheet baseline changed')
check(digest_text('\n'+marker+suffix)=='0093a6e7cfc7b4a2f120fbd245de23d691baa31551f26b989ebe964f7afc369c','v3.6.79 UX Phase 3 override block changed outside the reviewed payload')
for required in (
...
): check(forbidden not in suffix,'UX-only Phase 3 CSS touches protected Newsgroup Browser/performance surface: '+forbidden)
...
print('v3.6.79 UX feedback/state-clarity regression guard: PASS')
