...
check(len(body) == 1, 'Could not locate the v3.6.70 overview merge helper.')
...
check('updateNameResolutionActivityDomInPlace' in app_js, 'v3.6.70 removed the targeted name-resolution activity updater.')
check("const UI_VERSION = '3.6.70'" in app_js, 'UI version marker is not v3.6.70.')
check('"schema_version": 8' in server_text and '"contract": "passive-runtime-browsing-performance"' in server_text, 'Browsing telemetry schema/contract is not v3.6.70.')
...
check(sab.SAB_VERSION == '5.1.2' and sab.TERMINAL_HISTORY_VERSION == 3, 'v3.6.70 changed SAB 5.1.2 or terminal-history schema 3.')
check("version='3.6.70'" in auto_text, 'Automation default version identity did not migrate to v3.6.70.')
...
check('3.6.70-video-thumbnail-post-diagnostics' in index_html, 'v3.6.70 static cache identity is missing.')
check('<div class="version"><b>NewzDeck</b><span>v3.6.70</span></div>' in index_html, 'Visible sidebar version is not v3.6.70.')
