...
check(len(body) == 1, 'Could not locate the v3.6.71 overview merge helper.')
...
check('updateNameResolutionActivityDomInPlace' in app_js, 'v3.6.71 removed the targeted name-resolution activity updater.')
...
check(sab.SAB_VERSION == '5.1.2' and sab.TERMINAL_HISTORY_VERSION == 3, 'v3.6.71 changed SAB 5.1.2 or terminal-history schema 3.')
check("version='3.6.71'" in auto_text, 'Automation default version identity did not migrate to v3.6.71.')
...
check('3.6.71-video-thumbnail-decode-suppression' in index_html, 'v3.6.71 static cache identity is missing.')
check('<div class="version"><b>NewzDeck</b><span>v3.6.71</span></div>' in index_html, 'Visible sidebar version is not v3.6.71.')
