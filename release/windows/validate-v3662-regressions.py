...
check(len(body) == 1, 'Could not locate the v3.6.62 overview merge helper.')
...
for marker in (
    ...
    check(marker in server_text or marker in app_js, f'Missing render-reason telemetry marker: {marker}')
check("const UI_VERSION = '3.6.62'" in app_js, 'UI version marker is not v3.6.62.')
check('"schema_version": 2' in server_text and '"contract": "passive-runtime-browsing-performance"' in server_text, 'Browsing telemetry schema/contract is not v3.6.62.')
...
check("version='3.6.62'" in auto_text, 'Automation default version identity did not migrate to v3.6.62.')
