...
check(len(body) == 1, 'Could not locate the v3.6.63 overview merge helper.')
...
for marker in (
    ...
    "reason:'name-resolution-result'",
):
    check(marker in server_text or marker in app_js, f'Missing carried render-reason telemetry marker: {marker}')
check('updateNameResolutionActivityDomInPlace' in app_js, 'v3.6.63 removed the targeted name-resolution activity updater.')
check("const UI_VERSION = '3.6.63'" in app_js, 'UI version marker is not v3.6.63.')
...
check(sab.SAB_VERSION == '5.1.2' and sab.TERMINAL_HISTORY_VERSION == 3, 'v3.6.63 changed SAB 5.1.2 or terminal-history schema 3.')
check("version='3.6.63'" in auto_text, 'Automation default version identity did not migrate to v3.6.63.')
...
check('3.6.63-name-resolution-render-coalescing-thumbnail-trace-telemetry' in index_html, 'v3.6.63 static cache identity is missing.')
check('<div class="version"><b>NewzDeck</b><span>v3.6.63</span></div>' in index_html, 'Visible sidebar version is not v3.6.63.')
