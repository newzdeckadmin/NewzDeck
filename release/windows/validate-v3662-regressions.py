...
check(len(body) == 1, 'Could not locate the v3.7.0 overview merge helper.')
...
check('updateNameResolutionActivityDomInPlace' in app_js, 'v3.7.0 removed the targeted name-resolution activity updater.')
check("const UI_VERSION = '3.7.0'" in app_js, 'UI version marker is not v3.7.0.')
check('"schema_version": 11' in server_text and '"contract": "passive-runtime-browsing-performance"' in server_text, 'Browsing telemetry schema/contract is not v3.7.0.')

# 5. Release/runtime identities remain coherent and unrelated subsystems are preserved.
check((APP / 'version.txt').read_text(encoding='utf-8').strip() == '3.7.0', 'version.txt is not v3.7.0.')
check(manifest.get('version') == '3.7.0' and manifest.get('base_version') == '3.6.99', f'Build manifest version/base mismatch: {manifest}')
check(manifest.get('adapter_version') == '3.7.0' and manifest.get('sab_version') == '5.1.2', f'Build manifest adapter/SAB mismatch: {manifest}')
check(sab.ADAPTER_VERSION == '3.7.0', f'Wrong SAB adapter identity: {sab.ADAPTER_VERSION}')
check(sab.SAB_VERSION == '5.1.2' and sab.TERMINAL_HISTORY_VERSION == 3, 'v3.7.0 changed SAB 5.1.2 or terminal-history schema 3.')
check("version='3.7.0'" in auto_text, 'Automation default version identity did not migrate to v3.7.0.')
...
check('3.7.0-production-milestone-repository-hygiene' in index_html, 'v3.7.0 static cache identity is missing.')
check('<div class="version"><b>NewzDeck</b><span>v3.7.0</span></div>' in index_html, 'Visible sidebar version is not v3.7.0.')

print('v3.7.0 regression guard: PASS')
