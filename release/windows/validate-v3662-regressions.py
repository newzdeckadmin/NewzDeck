...
check(len(body) == 1, 'Could not locate the v3.6.88 overview merge helper.')
...
check('updateNameResolutionActivityDomInPlace' in app_js, 'v3.6.88 removed the targeted name-resolution activity updater.')
check("const UI_VERSION = '3.6.88'" in app_js, 'UI version marker is not v3.6.88.')
check('"schema_version": 11' in server_text and '"contract": "passive-runtime-browsing-performance"' in server_text, 'Browsing telemetry schema/contract is not v3.6.88.')

# 5. Release/runtime identities remain coherent and unrelated subsystems are preserved.
check((APP / 'version.txt').read_text(encoding='utf-8').strip() == '3.6.88', 'version.txt is not v3.6.88.')
check(manifest.get('version') == '3.6.88' and manifest.get('base_version') == '3.6.87', f'Build manifest version/base mismatch: {manifest}')
check(manifest.get('adapter_version') == '3.6.88' and manifest.get('sab_version') == '5.1.2', f'Build manifest adapter/SAB mismatch: {manifest}')
check(sab.ADAPTER_VERSION == '3.6.88', f'Wrong SAB adapter identity: {sab.ADAPTER_VERSION}')
check(sab.SAB_VERSION == '5.1.2' and sab.TERMINAL_HISTORY_VERSION == 3, 'v3.6.88 changed SAB 5.1.2 or terminal-history schema 3.')
check("version='3.6.88'" in auto_text, 'Automation default version identity did not migrate to v3.6.88.')
...
check('3.6.88-smart-import-wanted-reconciliation-fix' in index_html, 'v3.6.88 static cache identity is missing.')
check('<div class="version"><b>NewzDeck</b><span>v3.6.88</span></div>' in index_html, 'Visible sidebar version is not v3.6.88.')

print('v3.6.88 regression guard: PASS')
