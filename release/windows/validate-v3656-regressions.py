#!/usr/bin/env python3
"""NewzDeck v3.6.87 Smart Import duplicate protection and diagnostics refinement guards."""
...
for marker in (
    'ADAPTER_VERSION = "3.6.87"','_terminal_resolved_warning_keys','_engine_warning_resolved_by_terminal_history',
    ...
    if marker not in sab_source: raise SystemExit(f'Missing v3.6.87 SAB marker: {marker}')
...
for marker in (
    ...
    if marker not in auto_source: raise SystemExit(f'Missing v3.6.87 Smart Import marker: {marker}')
for marker in (
    'APP_VERSION = "3.6.87"','DIAGNOSTICS_SNAPSHOT_CACHE_TTL_SECONDS = 1.5','def _diagnostics_snapshot_uncached()',
    "result['diagnostics_cache']",'diagnostics_snapshot(force=True)','"Raw SAB slot overlap: "','resolved_historical=',
):
    if marker not in server_source: raise SystemExit(f'Missing v3.6.87 diagnostics marker: {marker}')
if "const UI_VERSION = '3.6.87';" not in js: raise SystemExit('UI version is not 3.6.87.')
...
print('v3.6.87 regression guard passed (pre-import cross-episode duplicate hold + recovered SAB warnings + bounded raw-overlap telemetry + coherent diagnostics cache markers).')
