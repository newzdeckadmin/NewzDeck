#!/usr/bin/env python3
"""NewzDeck v3.6.54 durable-history compaction and Automation startup guards."""
...
if sab.ADAPTER_VERSION!='3.6.54': raise SystemExit(f'Wrong SAB adapter version: {sab.ADAPTER_VERSION}')
if sab.TERMINAL_HISTORY_VERSION!=2 or sab.TERMINAL_HISTORY_MAX_ROWS!=5000:
    raise SystemExit('Terminal-history v2/5,000-row contract is missing.')
...
with tempfile.TemporaryDirectory(prefix='newzdeck-v3654-sidebar-') as td:
    ...
    engine=auto.MediaAutomationEngine(data,lambda x:x,lambda x:x,DummyDM(),lambda:[],version='3.6.54')
...
for marker in (
    'ADAPTER_VERSION = "3.6.54"','TERMINAL_HISTORY_VERSION = 2','STATISTICS_ACCOUNTED_MAX_ROWS = 20000',
    ...
    if marker not in sab_source: raise SystemExit(f'Missing v3.6.54 SAB/history marker: {marker}')
for marker in ('sidebar_counts_cache_signature','sidebar_counts_cache_result','def _sidebar_counts_signature','cache_hit'):
    if marker not in auto_source: raise SystemExit(f'Missing v3.6.54 Automation cache marker: {marker}')
for marker in ("const UI_VERSION = '3.6.54';",'automationStartupPrimeGeneration++;','if(!state.automation)void loadAutomation'):
    if marker not in js: raise SystemExit(f'Missing v3.6.54 startup-probe guard: {marker}')
if 'APP_VERSION = "3.6.54"' not in server_source: raise SystemExit('Server version is not 3.6.54.')
manifest=json.loads(MANIFEST.read_text(encoding='utf-8'))
if manifest.get('version')!='3.6.54' or manifest.get('adapter_version')!='3.6.54' or manifest.get('base_version')!='3.6.53':
...
if WORKFLOW.exists() and 'python release/windows/validate-v3654-regressions.py' not in WORKFLOW.read_text(encoding='utf-8'):
    raise SystemExit('Canonical release workflow does not run the v3.6.54 regression guard.')
print('v3.6.54 regression guard passed (bounded operational ledger + 5,000-row indexed terminal history + compact page transport + durable lazy detail + SAB re-adoption guard + sidebar cache/probe cancellation).')
