#!/usr/bin/env python3
"""NewzDeck v3.6.89 durable-history compaction and Automation startup guards."""
...
with tempfile.TemporaryDirectory(prefix='newzdeck-v3654-sidebar-') as td:
    ...
    engine=auto.MediaAutomationEngine(data,lambda x:x,lambda x:x,DummyDM(),lambda:[],version='3.6.89')
...
for marker in (
    'ADAPTER_VERSION = "3.6.89"','TERMINAL_HISTORY_VERSION = 3','STATISTICS_ACCOUNTED_MAX_ROWS = 20000',
    ...
    if marker not in sab_source: raise SystemExit(f'Missing v3.6.89 SAB/history marker: {marker}')
for marker in ('sidebar_counts_cache_signature','sidebar_counts_cache_result','def _sidebar_counts_signature','cache_hit'):
    if marker not in auto_source: raise SystemExit(f'Missing v3.6.89 Automation cache marker: {marker}')
for marker in ("const UI_VERSION = '3.6.89';",'automationStartupPrimeGeneration++;','if(!state.automation)void loadAutomation'):
    if marker not in js: raise SystemExit(f'Missing v3.6.89 startup-probe guard: {marker}')
if 'APP_VERSION = "3.6.89"' not in server_source: raise SystemExit('Server version is not 3.6.89.')
...
print('v3.6.89 regression guard passed (bounded operational ledger + 5,000-row indexed terminal history + compact page transport + durable lazy detail + SAB re-adoption guard + sidebar cache/probe cancellation).')
