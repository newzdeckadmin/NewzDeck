...
): check(marker in app,'v3.6.68 resolver accumulator marker missing: '+marker)
...
check('"schema_version": 7' in server and '"contract": "passive-runtime-browsing-performance"' in server,'Browsing telemetry schema 7/contract missing')
...
check(manifest.get('version')=='3.6.68' and manifest.get('base_version')=='3.6.67' and manifest.get('adapter_version')=='3.6.68','build manifest identity mismatch')
check(sab.ADAPTER_VERSION=='3.6.68' and sab.SAB_VERSION=='5.1.2','SAB identity changed')
