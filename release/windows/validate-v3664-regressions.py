...
for marker in ('function queueNameResolutionResultRender({manual=false}={})','function flushNameResolutionResultRender()','nameResolutionResultRenderBatches>=target',"reason:'name-resolution-batched-result'","perfRecord('name_resolution_batch'"):
    check(marker in app,f'Missing v3.6.95 result-batching marker: {marker}')
...
for stage in ('thumbnail_server_pair','thumbnail_transport_gap','name_resolution_batch'):
    check(f'"{stage}"' in server,f'Backend does not accept v3.6.95 client stage {stage}.')
...
check(manifest.get('version')=='3.6.95' and manifest.get('base_version')=='3.6.94' and manifest.get('adapter_version')=='3.6.95','manifest identity mismatch')
...
check(sab.ADAPTER_VERSION=='3.6.95','SAB adapter identity mismatch')
check("version='3.6.95'" in auto,'Automation default identity mismatch')
check('_discover_library_index_snapshot' in auto and '_flush_metadata_cache_now' in auto,'Discover optimization stack missing')
check('3.6.95-light-theme-readability-hotfix' in index,'Static cache marker missing')
check('<div class="version"><b>NewzDeck</b><span>v3.6.95</span></div>' in index,'Visible UI version mismatch')
