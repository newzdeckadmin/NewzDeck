...
# Controlled v3.6.84 Video-only concurrency change.
...
check(formula in app,'v3.6.84 high-connection Video ceiling formula missing')
...
check(manifest.get('sab_version')=='5.1.2' and sab.SAB_VERSION=='5.1.2' and sab.TERMINAL_HISTORY_VERSION==3 and sab.ADAPTER_VERSION=='3.6.84','SAB/history identity changed')
check("version='3.6.84'" in auto,'Automation identity mismatch')
check('_discover_library_index_snapshot' in auto and '_flush_metadata_cache_now' in auto,'Discover optimization stack missing')
check('3.6.84-wanted-upgrade-cutoff-policy-fix' in index and '<div class="version"><b>NewzDeck</b><span>v3.6.84</span></div>' in index,'UI/cache identity mismatch')
