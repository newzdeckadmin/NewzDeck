...
# Controlled v3.6.73 Video-only concurrency change.
...
check(formula in app,'v3.6.73 high-connection Video ceiling formula missing')
...
for marker in ('video_thumbnail_endpoint_concurrency','video_thumbnail_requests','video_thumbnail_failures','video_thumbnail_body','video_thumbnail_frame','"schema_version": 10'):
...
check(manifest.get('version')=='3.6.73' and manifest.get('base_version')=='3.6.72' and manifest.get('adapter_version')=='3.6.73','manifest identity mismatch')
check(manifest.get('sab_version')=='5.1.2' and sab.SAB_VERSION=='5.1.2' and sab.TERMINAL_HISTORY_VERSION==3 and sab.ADAPTER_VERSION=='3.6.73','SAB/history identity changed')
check("version='3.6.73'" in auto,'Automation identity mismatch')
check('_discover_library_index_snapshot' in auto and '_flush_metadata_cache_now' in auto,'Discover optimization stack missing')
check('3.6.73-thumbnail-task-identity-visibility-telemetry' in index and '<div class="version"><b>NewzDeck</b><span>v3.6.73</span></div>' in index,'UI/cache identity mismatch')
