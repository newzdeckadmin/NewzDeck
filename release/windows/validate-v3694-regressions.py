...
for rel,expected in EXPECTED.items():
    check(sha(APP/rel)==expected,f'{rel} differs from reviewed v3.6.96 Library Article-Aware Sorting payload')
...
check('/themes.css?v=3.6.96-library-article-aware-sorting' in index,'themes.css cache-busted link missing')
check('/styles.css?v=3.6.96-library-article-aware-sorting' in index and '/app.js?v=3.6.96-library-article-aware-sorting' in index,'theme release cache identity mismatch')
...
check('python release/windows/validate-v3694-regressions.py' in workflow,'canonical workflow does not run v3.6.96 guard')
...
print('v3.6.96 Library Article-Aware Sorting regression guard: PASS (12 palettes, Night exact fallback, contrast and Defender baseline preserved).')
