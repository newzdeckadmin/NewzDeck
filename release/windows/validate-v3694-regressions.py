...
for rel,expected in EXPECTED.items():
    check(sha(APP/rel)==expected,f'{rel} differs from reviewed v3.7.0 Production Milestone & Repository Hygiene payload')
...
check('/themes.css?v=3.7.0-production-milestone-repository-hygiene' in index,'themes.css cache-busted link missing')
check('/styles.css?v=3.7.0-production-milestone-repository-hygiene' in index and '/app.js?v=3.7.0-production-milestone-repository-hygiene' in index,'theme release cache identity mismatch')
...
check('python release/windows/validate-v3694-regressions.py' in workflow,'canonical workflow does not run v3.7.0 guard')
...
print('v3.7.0 Library Article-Aware Sorting regression guard: PASS (12 palettes, Night exact fallback, contrast and Defender baseline preserved).')
