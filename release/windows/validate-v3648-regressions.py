#!/usr/bin/env python3
"""NewzDeck v3.6.59 carried production guards, including v3.6.48 real-world regressions."""
...
with tempfile.TemporaryDirectory(prefix="newzdeck-v3649-integrity-") as td:
    ...
    engine = auto.MediaAutomationEngine(data_dir, lambda value:value, lambda value:value, DummyDownloadManager(), lambda:[], version="3.6.59")
...
for marker in (
    'ADAPTER_VERSION = "3.6.59"', 'def snapshot_view(', 'snapshot_p95_ms',
    ...
    if marker not in sab_source:
        raise SystemExit(f"Missing v3.6.59 SAB/runtime marker: {marker}")
for marker in (
    'APP_VERSION = "3.6.59"', 'scope=str((query.get("scope")',
    'X-NewzDeck-JSON-Serialize-Ms', "runtime_source']='sabnzbd'",
):
    if marker not in server_source:
        raise SystemExit(f"Missing v3.6.59 server marker: {marker}")
for marker in (
    "const UI_VERSION = '3.6.59';", "downloadHistoryLimit:50",
    "scope=terminalView?state.downloadFilter:'live'", "data-download-history-more",
    "Downloads snapshot latency", "SAB runtime",
):
    if marker not in app_source:
        raise SystemExit(f"Missing v3.6.59 UI marker: {marker}")
for marker in ("v3.6.59", "3.6.59-discover-responsiveness-metadata-cache-efficiency"):
    if marker not in index_source:
        raise SystemExit(f"Missing v3.6.59 HTML identity marker: {marker}")

print(f"v3.6.59 carried-forward supplemental regression guard passed ({len(PRODUCTION_REJECTS)} historical identity rejects + data-plane/runtime/cache/output ownership guards).")
