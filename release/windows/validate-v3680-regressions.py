...
# reviewed v3.6.86 Quality Profile Builder suffix.
...
check(styles.count(next_marker)==1,'v3.6.86 stylesheet suffix marker count mismatch')
...
for required in ('--ux-focus-ring:rgba(86,205,231,.82);','.primary-btn:disabled,.secondary-btn:disabled,.danger-btn:disabled,','@media(prefers-reduced-motion:reduce){'):
...
for required in ('YENC_ACCEPTED_SOURCE_SHA256 = "ba11eea2f880a934ff24f73be1cb12f0341456d5972c71f9c860efe9b3673edd"','YENC_ACCEPTED_BINARY_SHA256 = "4bb07f7b6d38ff99313f74cb4b45555e134af7106e32d55b106a79d603204fad"',"ap.add_argument('--prebuilt-yenc')"):
    check(required in builder,'Defender-clean builder protection missing: '+required)
for required in ('yenc-helper:','runs-on: ubuntu-24.04','newzdeck-yenc-defender-lf','needs: yenc-helper','--prebuilt-yenc $yenc','python release/windows/validate-v3680-regressions.py','python release/windows/validate-v3681-regressions.py'):
    check(required in workflow,'Canonical workflow protection missing: '+required)

# Frozen browsing/runtime values remain accepted.
for required in ('const THUMBNAIL_HTTP_ADMISSION_LIMIT=5;','let videoThumbRequestSeq = 0;',"perfRecord('thumbnail_visible_wait',visibleWaitMs,true"):
...
for required in ('VIDEO_THUMB_SAMPLE_MB = 24','max_segments=12','SETTINGS_SAVE_RETRY_SECONDS = 3.0','SETTINGS_SAVE_RETRYABLE_WINERRORS = frozenset({5, 32, 33})'):
...
check("const UI_VERSION = '3.6.86'" in app and '3.6.86-automation-cache-snapshot-performance-fix' in index,'UI/cache identity mismatch')
check(manifest.get('version')=='3.6.86' and manifest.get('base_version')=='3.6.85' and manifest.get('adapter_version')=='3.6.86','build manifest identity mismatch')
check(sab.ADAPTER_VERSION=='3.6.86' and sab.SAB_VERSION=='5.1.2','SAB identity changed')
