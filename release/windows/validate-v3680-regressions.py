...
# reviewed v3.6.87 Quality Profile Builder suffix.
...
check(styles.count(next_marker)==1,'v3.6.87 stylesheet suffix marker count mismatch')
...
check("const UI_VERSION = '3.6.87'" in app and '3.6.87-dynamic-range-evidence-authority-fix' in index,'UI/cache identity mismatch')
check(manifest.get('version')=='3.6.87' and manifest.get('base_version')=='3.6.86' and manifest.get('adapter_version')=='3.6.87','build manifest identity mismatch')
check(sab.ADAPTER_VERSION=='3.6.87' and sab.SAB_VERSION=='5.1.2','SAB identity changed')
