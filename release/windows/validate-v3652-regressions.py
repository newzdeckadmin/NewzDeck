#!/usr/bin/env python3
"""NewzDeck v3.6.78 scope-native projection / probe-efficiency guards."""
...
for marker in (
    ...
    if marker not in sab_source:
        raise SystemExit(f"Missing v3.6.78 runtime marker: {marker}")
...
if refresh_text.count("acquire(blocking=False)") != 1:
    raise SystemExit("v3.6.78 state gate is not exactly one non-blocking lock attempt.")
for forbidden in ("time.sleep", "state_file", "state_lock_file", "_json_read", "_state_file_guard", "_try_state_file_guard"):
    if forbidden in refresh_text:
        raise SystemExit(f"v3.6.78 presentation state gate contains forbidden work: {forbidden}")
...
print("v3.6.78 regression guard passed (scope-native Live projection + zero-wait state gate + sparse SAB version boundary).")
