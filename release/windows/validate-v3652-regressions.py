#!/usr/bin/env python3
"""NewzDeck v3.6.52 scope-native projection / probe-efficiency guards."""
...
for marker in (
    ...
    if marker not in sab_source:
        raise SystemExit(f"Missing v3.6.52 runtime marker: {marker}")
...
if refresh_text.count("acquire(blocking=False)") != 1:
    raise SystemExit("v3.6.52 state gate is not exactly one non-blocking lock attempt.")
for forbidden in ("time.sleep", "state_file", "state_lock_file", "_json_read", "_state_file_guard", "_try_state_file_guard"):
    if forbidden in refresh_text:
        raise SystemExit(f"v3.6.52 presentation state gate contains forbidden work: {forbidden}")
...
with tempfile.TemporaryDirectory(prefix="newzdeck-v3652-scope-") as td:
    ...
    if int((live.get("counts") or {}).get("completed") or 0) != 250:
        raise SystemExit(f"Live-native projection lost global Completed count: {live.get('counts')}")
    ...
    if len(completed.get("jobs") or []) != 50 or int((completed.get("view") or {}).get("total") or 0) != 250:
        raise SystemExit(f"Completed history was damaged by Live-native projection: {completed.get('view')}")
...
print("v3.6.52 regression guard passed (scope-native Live projection + zero-wait state gate + sparse SAB version boundary).")
