...
check((APP/'version.txt').read_text().strip()=='3.6.96','current version mismatch')
check(automation.count("version='3.6.96'")==1,'Automation version identity mismatch')
check(sab_text.count('ADAPTER_VERSION = "3.6.96"')==1,'SAB adapter version identity mismatch')
...
engine=auto.MediaAutomationEngine(Path(tempfile.mkdtemp(prefix='newzdeck-v3682-guard-')),lambda x:x,lambda x:x,DummyDownloadManager(),lambda:[],version='3.6.96')
