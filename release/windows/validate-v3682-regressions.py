...
# v3.6.82's pipeline recovery remains protected while v3.6.84 carries the
# v3.6.83 presentation/update-coherency work forward and narrowly fixes Wanted cutoff policy semantics.
check((APP/'version.txt').read_text().strip()=='3.6.84','current version mismatch')
check(automation.count("version='3.6.84'")==1,'Automation version identity mismatch')
check(sab_text.count('ADAPTER_VERSION = "3.6.84"')==1,'SAB adapter version identity mismatch')
...
check('python release/windows/validate-v3682-regressions.py' in workflow and 'python release/windows/validate-v3683-regressions.py' in workflow and 'python release/windows/validate-v3684-regressions.py' in workflow,'current guard chain incomplete')
...
engine=auto.MediaAutomationEngine(Path(tempfile.mkdtemp(prefix='newzdeck-v3682-guard-')),lambda x:x,lambda x:x,DummyDownloadManager(),lambda:[],version='3.6.84')
