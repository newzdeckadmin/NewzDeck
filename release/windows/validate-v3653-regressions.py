#!/usr/bin/env python3
"""NewzDeck v3.6.62 NZB identity, durable history and presentation-index guards."""
...
if sab.ADAPTER_VERSION!='3.6.62': raise SystemExit(f'Wrong adapter version: {sab.ADAPTER_VERSION}')
...
for marker in ('terminal-history.json','TERMINAL_HISTORY_MAX_ROWS = 5000','_presentation_index_snapshot','_queue_snapshot_state_patch','_drain_snapshot_state_patches','history_source":"newzdeck-terminal-index','lazy_details','queue_sampler_last_failure_error','queue_sampler_failure_reasons','_validate_automation_output_episode_identity'):
    if marker not in sab_source: raise SystemExit(f'Missing v3.6.62 runtime marker: {marker}')
...
# v3.6.62 active presentation path itself is sourced from the copy-on-write index.
...
for marker in ('APP_VERSION = "3.6.62"','detail_id=str((query.get("id")','snapshot_view(scope,limit=limit,offset=offset,detail_id=detail_id)'):
    if marker not in server_source: raise SystemExit(f'Missing server detail/paging marker: {marker}')
for marker in ("const UI_VERSION = '3.6.62';",'downloadHistoryOffset:0','downloadHistoryLoaded:[]','scope=detail&id=','job.details_loaded===false',"state.downloadHistoryOffset=(state.downloadHistoryLoaded||[]).length"):
...
if WORKFLOW.exists() and 'python release/windows/validate-v3653-regressions.py' not in WORKFLOW.read_text(encoding='utf-8'):
    raise SystemExit('Release workflow does not run the v3.6.62 guard.')

print('v3.6.62 regression guard passed (998-file disguised pack rejection + output fail-closed + 620-row durable history + active presentation index + retained sampler failures).')
