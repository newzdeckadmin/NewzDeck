...
# Evidence-driven v3.6.71 policy: preserve client code, suppress only definitive codec/decoder limits.
for marker in (
 ...
 "!state.unpreviewableMediaKeys.has(key)&&!state.unsupportedMediaKeys.has(key)",
 "key.startsWith(prefix)&&!state.unsupportedMediaKeys.has(key)",
 "f.info?.retryable===false?'':",
...
): check(marker in (server+app),'v3.6.71 Video decode suppression marker missing: '+marker)
...
check('"schema_version": 9' in server and '"contract": "passive-runtime-browsing-performance"' in server,'Browsing telemetry schema 9/contract missing')
...
check("const UI_VERSION = '3.6.71'" in app and '3.6.71-video-thumbnail-decode-suppression' in index,'UI/cache identity mismatch')
check(manifest.get('version')=='3.6.71' and manifest.get('base_version')=='3.6.70' and manifest.get('adapter_version')=='3.6.71','build manifest identity mismatch')
check(sab.ADAPTER_VERSION=='3.6.71' and sab.SAB_VERSION=='5.1.2','SAB identity changed')
