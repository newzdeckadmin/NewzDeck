...
Current NewzDeck Windows releases ship executables built with **Go 1.23.2**. Portions of the Go runtime and standard library are statically incorporated into those executables.
...
CPython is not embedded in the current NewzDeck Portable ZIP or Setup payload; it is obtained separately from the upstream publisher on the user's machine.
...
NewzDeck does not relicense UnRAR under GPL. The upstream terms apply independently. Current NewzDeck releases do not embed UnRAR in the GitHub release payload.
...
Current NewzDeck releases do not embed this executable in their GitHub release payload.

## 7-Zip (optional user-supplied tool)

NewzDeck can use a separately installed or user-supplied `7z.exe`, `7zz.exe`, or `7za.exe` for `.7z` archives. Current NewzDeck releases do not bundle 7-Zip.
