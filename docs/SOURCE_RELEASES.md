...
## Current release: v3.6.95

**v3.6.95 is the current stable production release.**

The `v3.6.95` tag identifies the public source used by the canonical Windows release workflow. Published installer, Portable, and SHA-256 values are recorded in the GitHub Release and checksum asset.
...
- **v3.6.95 - Light Theme Readability Hotfix.** Corrects Light, Arctic, and Sandstone by separating ordinary light UI surfaces from true dark media/modal scrims while freezing all nine dark palettes and preserving application behavior.

- **v3.6.94 - Themes & Color Schemes.** Adds twelve coordinated application color themes with exact Night fallback preservation, local presentation-only persistence, pre-paint restore, and contrast regression gates while preserving v3.6.93 functionality and the Defender-clean build pipeline.
...
- **v3.6.86 - Automation Cache Snapshot Performance Hotfix.** Reuses one quality-cache/current-file trait snapshot per logical Automation/Search operation so v3.6.85 correctness no longer causes record- or candidate-count-dependent disk I/O.
...
- **v3.6.83 - Quality Profile UI & Update Version Coherency.** Polishes the structured Quality Profile dialog and makes About/update availability authoritative across installed, runtime and UI version identities while preserving v3.6.81 Automation behavior and v3.6.82 release topology.
...
- **v3.6.80 - UX Final Consistency & Accessibility.** Completes the current staged CSS-only UX polish program with consistent dialog framing, keyboard focus, disabled-state clarity, scaling insets and reduced-motion support while preserving all working functionality and the Defender-clean build pipeline.
...
- **v3.6.77 - UX Layout & Control Consistency.** Continues the staged UX polish program with CSS-only control alignment, spacing, dialog-layout and focus-state improvements while preserving all working application behavior.

- **v3.6.76 - UX Readability & Visual Rhythm.** Begins the staged UX polish program with a CSS-only readability/hierarchy pass while freezing application logic, browsing/download behavior, Automation, Smart Import and the v3.6.75 yEnc build compatibility fix.

- **v3.6.75 - Windows Defender Compatibility.** Keeps `NewzDeckYenc.go` unchanged while retaining standard Go build metadata only for `NewzDeckYenc.exe`, resolving the tested Defender false-positive condition without changing runtime behavior.
...
- **v3.6.72 - Browse Timeout Error Attribution.** Separates upstream article-header timeout/reset HTTP errors from genuine localhost transport failures, prevents misleading local-service toasts, and keeps schema-9 Video policy reason labels within the established 48-character diagnostics contract.
...
- **v3.6.69 - Settings Save Contention Recovery & Reliability Telemetry.** Retries transient Windows settings-file replacement contention inside a hard three-second window and exposes passive recovery counters while preserving the accepted browsing architecture, Metadata Server v0.3.3 and private SABnzbd 5.1.2.
