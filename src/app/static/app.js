...
function renderIntegrityReview(audit){
  integrityReviewState.audit=audit||{};const entries=integrityReviewEntries(audit),dupes=Array.isArray(audit?.cross_title_duplicates)?audit.cross_title_duplicates:[];
  const head=`<div class="automation-section-head"><div><div class="eyebrow">READ-ONLY AUDIT</div><h2>${Number(audit?.needs_review_count||0)} finding${Number(audit?.needs_review_count||0)===1?'':'s'} need review</h2><p>${Number(audit?.file_records||0).toLocaleString()} library file records checked • ${Number(audit?.identity_mismatches||0)} identity mismatch${Number(audit?.identity_mismatches||0)===1?'':'es'} • ${Number(audit?.cross_title_duplicate_fingerprints||0)} cross-title duplicate fingerprint${Number(audit?.cross_title_duplicate_fingerprints||0)===1?'':'s'}</p></div><button class="secondary-btn" id="integrityReviewRefresh" type="button">↻ Refresh audit</button></div>`;
  ...
  $('integrityReviewBody').innerHTML=head+rows+duplicateRows;
  const allRows=new Map(entries.map(r=>[integrityReviewKey(r),r]));for(const g of dupes)for(const t of g.targets||[])if(!allRows.has(integrityReviewKey(t)))allRows.set(integrityReviewKey(t),t);
