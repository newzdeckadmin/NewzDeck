...
async function scanAutomationLibrary(id=''){const b=id?$('autoItemScan'):$('automationScanBtn'),done=beginButtonActivity(b,id?'Scanning files…':'Scanning library…');if(id)setAutomationItemActivity('Scanning files…','Checking this title’s library location and reconciling file, quality, and cutoff state.');try{const d=await api('/api/automation/library/scan',{id});await loadAutomation();if(id)await openAutomationItem(id);const offline=(d.offline_roots||[]).length,changes=(d.changes||[]).length;toast(`Scan complete: ${d.matched} media file${d.matched===1?'':'s'} matched${changes?` • ${changes} change${changes===1?'':'s'}`:''}${offline?` • ${offline} root${offline===1?'':'s'} offline`:''}.`,offline?'warning':'success')}catch(e){toast(e.message,'error')}finally{done();if(id)setAutomationItemActivity('')}}
