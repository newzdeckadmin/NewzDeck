const state = {
  ...
  loadedPages: new Set(), continuousMode: true, continuousLoading: false, thumbnailSize: 'medium', browseScrollTop: 0, browseSessionToken:'', browseAbortController:null, headerPrefetch:null, progressiveBinaryTimer:null, smartBinaryPending:false, browseScrollVelocity:0, browseScrollDirection:1, lastBrowseScrollTop:0, lastBrowseScrollTs:0, lastBrowseScrollPerfTs:0, thumbReleaseTimer:null, thumbCacheTrimTimer:null, continuousJumpHoldUntil:0, continuousJumpTimer:null, thumbGeometry:new Map(), thumbGeometryTs:0, thumbHolderRegistry:new Map(), thumbHolderRegistryStats:{hits:0,fallbacks:0}, speculativeThumbStats:{started:0,completed:0,cancelled:0},
  viewMode: 'gallery', previewCache: new Map(), previewPromises: new Map(), imageThumbCache: new Map(), imageThumbPromises: new Map(), setCoverImagePromises: new Map(), videoThumbCache: new Map(), videoThumbPromises: new Map(), thumbQueued: new Set(), thumbQueue: [],
  thumbActive: 0, thumbVideoActive: 0, thumbSetActive: 0, thumbHttpActive: 0, thumbHttpQueue: [], thumbHttpSeq: 0, thumbConcurrency: 7, videoThumbConcurrency: 1, setCoverConcurrency: 3, galleryGeneration: 0, previewConcurrencyMode:'idle', previewRamp:null, thumbFailureCache: new Map(), thumbImageRecovery: new Map(), thumbActiveTasks: new Map(), thumbEscalations: new Map(), thumbEscalationActive: 0, thumbDeferredRecoveryTimers:new Map(), unsupportedMediaKeys: new Set(), unpreviewableMediaKeys: new Set(), mediaSetBrokenKeys:new Set(), mediaSetProbePromises:new Map(), mediaSetStreamAnchors:new Map(), mediaSetIndexBuckets:new Map(), mediaSetIndexSingles:new Map(), mediaSetIndexValid:false, relatedSetPaneKey:'', relatedCoverResumeTimer:null, relatedCoverStats:{started:0,completed:0,failed:0,totalQueueWaitMs:0,maxQueueWaitMs:0,totalRunMs:0,maxRunMs:0,domTargetMisses:0,orphanRepairs:0,activationPasses:0,cachePaints:0,activationQueues:0,queuePromotions:0,crossGenerationPaints:0,staleSessionDrops:0}, lastRelatedCoverSnapshot:'', galleryVirtualTopBatches:[], galleryVirtualTopHeight:0, galleryVirtualTimer:null, galleryVirtualBusy:false, unavailableHideQueue: new Map(), unavailableHideTimer: null,
  downloadSnapshot: {paused:false,jobs:[],collections:[],counts:{},folder:''}, downloadedIndex: new Set(), queuedIndex: new Set(), downloadIndexSignature:'', downloadFilter:'active', downloadHistoryLimit:50, downloadHistoryOffset:0, downloadHistoryLoaded:[], downloadHistorySearchTerm:'', downloadSearchTerm:'', selectedDownloads:new Set(), downloadSelectionAnchor:'', expandedDownloads:new Set(), expandedCollections:new Set(), postProgressMemory:new Map(), activeView:'browse', downloadOrganization:'flat',
...
const UI_VERSION = '3.6.99';
const $ = (id) => document.getElementById(id);
const THEME_STORAGE_KEY = 'newzdeckTheme';
const THEME_IDS = Object.freeze(['night','light','midnight','ocean','emerald','amethyst','rosewood','sunset','arctic','graphite','sandstone','aurora']);
const LIGHT_THEME_IDS = new Set(['light','arctic','sandstone']);
const THEME_BROWSER_COLORS = Object.freeze({night:'#080d16',light:'#f4f7fa',midnight:'#070c18',ocean:'#071617',emerald:'#08140d',amethyst:'#100a18',rosewood:'#16090e',sunset:'#150d08',arctic:'#eff7fb',graphite:'#0d0f12',sandstone:'#f6f0e7',aurora:'#080e19'});
function normalizeTheme(value){const v=String(value||'').trim().toLowerCase();return THEME_IDS.includes(v)?v:'night'}
function savedTheme(){try{return normalizeTheme(localStorage.getItem(THEME_STORAGE_KEY)||'night')}catch(_e){return 'night'}}
function applyTheme(value,{persist=false}={}){const theme=normalizeTheme(value);document.documentElement.dataset.theme=theme;document.documentElement.style.colorScheme=LIGHT_THEME_IDS.has(theme)?'light':'dark';const meta=document.querySelector('meta[name="theme-color"]');if(meta)meta.content=THEME_BROWSER_COLORS[theme]||THEME_BROWSER_COLORS.night;if(persist){try{localStorage.setItem(THEME_STORAGE_KEY,theme)}catch(_e){}}return theme}
...
const els = {
  ...
  previewContent:$('previewContent'), previewBadge:$('previewBadge'), sidePaneEyebrow:$('sidePaneEyebrow'), sidePaneTitle:$('sidePaneTitle'), providerModal:$('providerModal'), providerList:$('providerList'),
...
function friendlyTransportErrorMessage(message,path='',source='network'){
  ...
  const refused=/(?:winerror|errno)\s*10061|connection refused|actively refused/.test(low);
  const timeout=/timed out|timeout/.test(low),fetchFailed=/failed to fetch|networkerror|load failed/.test(low);
  const grab=route.includes('/api/automation/releases/grab'),articles=route==='/api/articles';
  if(source==='http'){
    if(grab&&reset)return 'The built-in download engine briefly reset its local connection while queueing this release. Check Downloads; if the release is not listed, try Grab again in a moment.';
    if(grab&&(refused||timeout))return 'The built-in download engine was temporarily unavailable while queueing this release. Check Downloads; if it is not listed, try Grab again in a moment.';
  ...
  if(reset)return grab?'The built-in download engine briefly reset its local connection while queueing this release. Check Downloads; if the release is not listed, try Grab again in a moment.':'NewzDeck briefly lost its local backend connection. Try the operation again.';
  if(refused||fetchFailed)return grab?'The built-in download engine was temporarily unavailable while queueing this release. Check Downloads; if it is not listed, try Grab again in a moment.':"NewzDeck's local backend could not be reached. It may still be starting; try again in a moment.";
  if(timeout)return grab?'The built-in download engine was temporarily unavailable while queueing this release. Check Downloads; if it is not listed, try Grab again in a moment.':"NewzDeck's local backend did not respond in time. Try again in a moment.";
  return text||'The request could not be completed.';
}
...
async function api(path, body=null, options={}){
  ...
  try{
    ...
    if(!r.ok){let msg=data.error || `Request failed (${r.status})`;if(data.detail&&String(data.detail)!==String(data.error||''))msg+=` — ${data.detail}`;msg=friendlyTransportErrorMessage(msg,path,'http');const err=new Error(msg);err.data=data;err.status=r.status;throw err} return data;
  }catch(e){
...
function browserPerfMode(){const mode=String(els.contentFilter?.value||'all').toLowerCase();return ['images','videos','media','all'].includes(mode)?mode:'all'}
function schedulePerfTelemetryFlush(){if(state.perfTelemetryTimer||state.perfTelemetrySending)return;state.perfTelemetryTimer=setTimeout(flushPerfTelemetry,5000)}
async function flushPerfTelemetry(){
  if(state.perfTelemetryTimer){clearTimeout(state.perfTelemetryTimer);state.perfTelemetryTimer=null}if(state.perfTelemetrySending||!state.perfTelemetryPending.length)return;
  const samples=state.perfTelemetryPending.splice(0,120);state.perfTelemetrySending=true;
  try{await api('/api/browse/performance',{samples},{timeoutMs:2500})}
...
function thumbnailDemandClass(task=null,fallbackRole='item'){const role=String(task?.role||fallbackRole||'item');if(role==='speculative-page')return 'prefetch-speculative-page';const visible=Number(task?.priority??1)===0;return `${visible?'visible':'prefetch'}-${role}`}
function browseCancelledClientError(){const err=new Error('Browsing request superseded.');err.code='browse-cancelled';err.data={error:'Browsing request superseded by a newer newsgroup session',error_code:'browse_cancelled',error_label:'Browsing request cancelled',retryable:false};return err}
function pumpThumbnailHttpAdmission(){
  while(state.thumbHttpActive<THUMBNAIL_HTTP_ADMISSION_LIMIT&&state.thumbHttpQueue.length){
    let pos=-1,bestRank=Number.POSITIVE_INFINITY,bestSeq=Number.POSITIVE_INFINITY;
    for(let i=0;i<state.thumbHttpQueue.length;i++){const q=state.thumbHttpQueue[i];if(q.signal?.aborted)continue;const rank=Number(q.task?.priority??1);if(rank<bestRank||(rank===bestRank&&q.seq<bestSeq)){bestRank=rank;bestSeq=q.seq;pos=i}}
    if(pos<0){const cancelled=state.thumbHttpQueue.splice(0);for(const q of cancelled){q.cleanup?.();perfRecord('thumbnail_admission',Math.max(0,performance.now()-q.queuedAt),false,{reason:`cancelled-${q.demand}`});q.reject(browseCancelledClientError())}return}
    const q=state.thumbHttpQueue.splice(pos,1)[0];q.cleanup?.();if(q.signal?.aborted){perfRecord('thumbnail_admission',Math.max(0,performance.now()-q.queuedAt),false,{reason:`cancelled-${q.demand}`});q.reject(browseCancelledClientError());continue}
    state.thumbHttpActive++;perfRecord('thumbnail_admission',Math.max(0,performance.now()-q.queuedAt),true,{reason:q.demand});q.resolve(q.demand);
  }
}
function acquireThumbnailHttpAdmission(task=null,signal=null,fallbackRole='item'){
  const demand=thumbnailDemandClass(task,fallbackRole);return new Promise((resolve,reject)=>{const q={task,signal,demand,queuedAt:performance.now(),seq:++state.thumbHttpSeq,resolve,reject,cleanup:null};let settled=false;const abort=()=>{if(settled)return;settled=true;const i=state.thumbHttpQueue.indexOf(q);if(i>=0)state.thumbHttpQueue.splice(i,1);q.cleanup?.();perfRecord('thumbnail_admission',Math.max(0,performance.now()-q.queuedAt),false,{reason:`cancelled-${demand}`});reject(browseCancelledClientError());pumpThumbnailHttpAdmission()};q.cleanup=()=>{if(signal)signal.removeEventListener('abort',abort)};const resolveOnce=value=>{if(settled)return;settled=true;resolve(value)};q.resolve=resolveOnce;if(signal?.aborted){abort();return}if(signal)signal.addEventListener('abort',abort,{once:true});state.thumbHttpQueue.push(q);pumpThumbnailHttpAdmission()})
}
function releaseThumbnailHttpAdmission(){state.thumbHttpActive=Math.max(0,state.thumbHttpActive-1);pumpThumbnailHttpAdmission()}
...
function perfSummaryText(){const labels=[['headers','Headers'],['render','Render'],['group_index','Set index'],['virtualize','DOM window'],['search','Local search'],['thumbnail','Thumbnail'],['preview','Full preview'],['viewer_preload','Viewer preload']];return labels.map(([k,l])=>{const v=perfAverage(k);return v?`${l} ${Math.round(v)} ms`:''}).filter(Boolean).join(' • ')||'No browser timing samples yet'}
function relatedVisibleCoverQueue(){return state.thumbQueue.filter(t=>t.role==='set-cover'&&Number(t.priority??1)===0)}
function relatedCoverReserveTarget(){const waiting=relatedVisibleCoverQueue().length;if(!waiting)return 0;const scrolling=Date.now()-Number(state.lastBrowseScrollTs||0)<450||Math.abs(Number(state.browseScrollVelocity||0))>.06,reserve=scrolling?2:3;return Math.min(reserve,Math.max(1,Number(state.setCoverConcurrency||1)),Math.max(1,Number(state.thumbConcurrency||1)),state.thumbSetActive+waiting)}
function relatedSetCardByKey(setKey){if(!setKey||!els.previewContent)return null;for(const card of els.previewContent.querySelectorAll('.media-set-card[data-set-key]'))if(card.dataset.setKey===setKey)return card;return null}
function relatedCoverTaskKey(setKey,session=state.browseSessionToken){const scope=String(session||`${state.providerId}|${state.selectedGroup}`);return `${scope}|set-cover|${encodeURIComponent(String(setKey||''))}`}
function relatedCoverTaskTracked(setKey){if(!setKey)return false;const qkey=relatedCoverTaskKey(setKey);return state.thumbQueue.some(t=>t.qkey===qkey)||state.thumbActiveTasks.has(qkey)}
function relatedCoverSchedulerSummaryText(){
  const stats=state.relatedCoverStats||{},queued=state.thumbQueue.filter(t=>t.role==='set-cover'),visible=relatedVisibleCoverQueue(),now=Date.now(),oldest=visible.length?Math.max(...visible.map(t=>Math.max(0,now-Number(t.queuedAt||now)))):0;
  let visibleLoading=0,orphaned=0;if(els.previewContent&&!isAllPostsMode()){const root=els.previewContent.getBoundingClientRect();for(const card of els.previewContent.querySelectorAll('.media-set-card[data-set-key]')){const loader=card.querySelector('.media-set-cover .thumb-loader');if(!loader)continue;const r=card.getBoundingClientRect();if(r.bottom<=root.top||r.top>=root.bottom)continue;visibleLoading++;if(!relatedCoverTaskTracked(card.dataset.setKey))orphaned++}}
  const avgWait=stats.started?stats.totalQueueWaitMs/stats.started:0,finished=Number(stats.completed||0)+Number(stats.failed||0),avgRun=finished?stats.totalRunMs/finished:0;
  return `${visibleLoading} visible loading • ${orphaned} orphaned • ${visible.length}/${queued.length} visible/total queued • ${Number(state.thumbSetActive||0)} active • reserve ${relatedCoverReserveTarget()} • oldest ${Math.round(oldest)} ms • avg start wait ${Math.round(avgWait)} ms • max ${Math.round(Number(stats.maxQueueWaitMs||0))} ms • avg run ${Math.round(avgRun)} ms • target misses ${Number(stats.domTargetMisses||0)} • repairs ${Number(stats.orphanRepairs||0)} • cache paints ${Number(stats.cachePaints||0)} • activations ${Number(stats.activationPasses||0)} • promotions ${Number(stats.queuePromotions||0)} • isolated in-flight ${Number(state.setCoverImagePromises?.size||0)} • cross-generation paints ${Number(stats.crossGenerationPaints||0)} • stale-session drops ${Number(stats.staleSessionDrops||0)}`;
}
...
function beginButtonActivity(button,label='Working…'){
  if(!button)return ()=>{};
  const previous={disabled:!!button.disabled,text:button.textContent||'',aria:button.getAttribute('aria-busy')};
  button.disabled=true;button.classList.add('is-busy');button.setAttribute('aria-busy','true');if(label)button.textContent=label;
  return ()=>{if(!button?.isConnected)return;button.disabled=previous.disabled;button.classList.remove('is-busy');if(previous.aria==null)button.removeAttribute('aria-busy');else button.setAttribute('aria-busy',previous.aria);button.textContent=previous.text};
}
function setAutomationItemActivity(title='',detail=''){
  const box=$('autoItemActivity');if(!box)return;
  if(!title){box.classList.add('hidden');return}
  $('autoItemActivityTitle').textContent=title;$('autoItemActivityDetail').textContent=detail||'';box.classList.remove('hidden');
}
...
function hydrateServerThumbnailHints(items){
  const lim=browseCacheLimits();let added=false;for(const a of items||[]){if(a?.small_image_suppressed&&a?.media?.kind==='image'){a.media_meta={...(a.media_meta||{}),width:Number(a.media_meta?.width||0),height:Number(a.media_meta?.height||0)};continue}const url=String(a?.cached_thumbnail_url||'');if(!url||!a?.media)continue;const key=previewKey(a),result={url,thumbnail_url:url,thumbnail_token:a.cached_thumbnail_token||'',cached:true,method:'header-cache-hint'};if(a.media.kind==='image')boundedCacheSet(state.imageThumbCache,key,result,lim.thumb);else if(a.media.kind==='video')boundedCacheSet(state.videoThumbCache,key,result,Math.max(96,Math.floor(lim.thumb*.28)));added=true;}if(added)scheduleRelatedSetCoverActivation();
}
function thumbnailLaneHint(a,task=null){
  if(task?.role==='set-cover'||task?.role==='set-probe')return 1;
...
async function saveSettingsModal(){
  const done=beginButtonActivity(els.settingsSaveBtn,'Saving settings…');
  const payload={
  ...
  }catch(e){toast(e.message,'error')}finally{done()}
}
...
const NEWZDECK_BACKUP_FILE_FORMAT='NewzDeckBackupFile',NEWZDECK_BACKUP_ENCRYPTION_SCHEMA=1,NEWZDECK_BACKUP_PBKDF2_ITERATIONS=600000;
function backupBytesToBase64(bytes){let binary='';const data=bytes instanceof Uint8Array?bytes:new Uint8Array(bytes);for(let i=0;i<data.length;i+=0x8000)binary+=String.fromCharCode(...data.subarray(i,i+0x8000));return btoa(binary)}
function backupBase64ToBytes(text){const binary=atob(String(text||'')),out=new Uint8Array(binary.length);for(let i=0;i<binary.length;i++)out[i]=binary.charCodeAt(i);return out}
function backupFilename(kind='configuration'){const d=new Date(),pad=n=>String(n).padStart(2,'0'),stamp=`${d.getFullYear()}${pad(d.getMonth()+1)}${pad(d.getDate())}-${pad(d.getHours())}${pad(d.getMinutes())}${pad(d.getSeconds())}`;return `NewzDeck_${kind==='complete'?'Complete':'Configuration'}_Backup_${stamp}.newzdeck-backup`}
function downloadBackupObject(value,filename){const blob=new Blob([JSON.stringify(value,null,2)+'\n'],{type:'application/json'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=filename;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)}
function backupWithPresentation(payload){return {...payload,presentation:{...(payload?.presentation||{}),theme:savedTheme()}}}
async function deriveBackupKey(password,salt,usage){const material=await crypto.subtle.importKey('raw',new TextEncoder().encode(password),'PBKDF2',false,['deriveKey']);return crypto.subtle.deriveKey({name:'PBKDF2',hash:'SHA-256',salt,iterations:NEWZDECK_BACKUP_PBKDF2_ITERATIONS},material,{name:'AES-GCM',length:256},false,[usage])}
async function encryptCompleteBackup(payload,password){if(!crypto?.subtle)throw new Error('Encrypted backups require Web Crypto support.');if(String(password||'').length<8)throw new Error('Use a backup password with at least 8 characters.');const salt=crypto.getRandomValues(new Uint8Array(16)),iv=crypto.getRandomValues(new Uint8Array(12)),key=await deriveBackupKey(password,salt,'encrypt'),plain=new TextEncoder().encode(JSON.stringify(payload)),cipher=await crypto.subtle.encrypt({name:'AES-GCM',iv},key,plain);return {format:NEWZDECK_BACKUP_FILE_FORMAT,schema:NEWZDECK_BACKUP_ENCRYPTION_SCHEMA,encrypted:true,created:payload.created,created_with:payload.created_with,kind:'complete',encryption:{algorithm:'AES-256-GCM',kdf:'PBKDF2-SHA-256',iterations:NEWZDECK_BACKUP_PBKDF2_ITERATIONS,salt:backupBytesToBase64(salt),iv:backupBytesToBase64(iv)},payload:backupBytesToBase64(new Uint8Array(cipher))}}
async function decryptCompleteBackup(wrapper,password){if(wrapper?.format!==NEWZDECK_BACKUP_FILE_FORMAT||wrapper?.encrypted!==true)throw new Error('This is not an encrypted NewzDeck backup.');const enc=wrapper.encryption||{};if(enc.algorithm!=='AES-256-GCM'||enc.kdf!=='PBKDF2-SHA-256'||Number(enc.iterations)!==NEWZDECK_BACKUP_PBKDF2_ITERATIONS)throw new Error('Unsupported NewzDeck backup encryption settings.');try{const salt=backupBase64ToBytes(enc.salt),iv=backupBase64ToBytes(enc.iv),cipher=backupBase64ToBytes(wrapper.payload),key=await deriveBackupKey(password,salt,'decrypt'),plain=await crypto.subtle.decrypt({name:'AES-GCM',iv},key,cipher);return JSON.parse(new TextDecoder().decode(plain))}catch(_e){throw new Error('The backup password is incorrect or the encrypted backup is damaged.')}}
function setBackupStatus(message='',tone=''){const el=$('settingsBackupStatus');if(!el)return;el.textContent=message;el.classList.toggle('hidden',!message);el.dataset.tone=tone||''}
function backupSummaryText(payload){const media=payload?.media_automation||{},tv=(media.library||[]).filter(x=>x?.kind==='tv').length,movies=(media.library||[]).filter(x=>x?.kind==='movie').length;return `${payload?.kind==='complete'?'Complete':'Configuration'} backup • ${Number((payload?.providers||[]).length)} provider${(payload?.providers||[]).length===1?'':'s'} • ${tv} TV • ${movies} movie${movies===1?'':'s'} • ${Number((media.profiles||[]).length)} quality profile${(media.profiles||[]).length===1?'':'s'}`}
async function createConfigurationBackup(){const button=$('settingsConfigBackupBtn'),done=beginButtonActivity(button,'Creating…');try{const payload=backupWithPresentation(await api('/api/config/backup'));downloadBackupObject(payload,backupFilename('configuration'));setBackupStatus('Configuration backup created. Secrets were excluded.','success');toast('Configuration backup created.','success')}catch(e){setBackupStatus(e.message,'error');toast(`Backup failed: ${e.message}`,'error')}finally{done()}}
async function createCompleteBackup(){const password=$('settingsBackupPassword')?.value||'',confirm=$('settingsBackupPasswordConfirm')?.value||'',button=$('settingsCompleteBackupBtn'),done=beginButtonActivity(button,'Encrypting…');try{if(password.length<8)throw new Error('Enter a backup password with at least 8 characters.');if(password!==confirm)throw new Error('Backup passwords do not match.');const payload=backupWithPresentation(await api('/api/config/backup/complete',{})),wrapper=await encryptCompleteBackup(payload,password);downloadBackupObject(wrapper,backupFilename('complete'));$('settingsBackupPassword').value='';$('settingsBackupPasswordConfirm').value='';setBackupStatus('Encrypted Complete Backup created. Keep its password somewhere safe.','success');toast('Encrypted Complete Backup created.','success')}catch(e){setBackupStatus(e.message,'error');toast(`Backup failed: ${e.message}`,'error')}finally{done()}}
async function inspectRestoreBackupFile(file){if(!file)return;const status=$('settingsRestoreSummary');try{const parsed=JSON.parse(await file.text());state.pendingRestoreBackup={file,parsed,payload:null};const encrypted=parsed?.format===NEWZDECK_BACKUP_FILE_FORMAT&&parsed?.encrypted===true;$('settingsRestorePasswordWrap')?.classList.toggle('hidden',!encrypted);if($('settingsRestorePassword'))$('settingsRestorePassword').value='';if(encrypted){status.textContent=`Encrypted Complete Backup • created ${parsed.created||'unknown'} • enter its password to restore.`;$('settingsApplyRestoreBtn').textContent='Decrypt & Restore'}else{const payload=parsed;state.pendingRestoreBackup.payload=payload;status.textContent=backupSummaryText(payload);$('settingsApplyRestoreBtn').textContent='Restore Backup'}$('settingsRestoreReady')?.classList.remove('hidden');setBackupStatus('Backup file loaded and ready for validation.','')}catch(e){state.pendingRestoreBackup=null;$('settingsRestoreReady')?.classList.add('hidden');setBackupStatus(`Could not read backup: ${e.message}`,'error')}}
async function applyRestoreBackup(){const pending=state.pendingRestoreBackup;if(!pending)return;const button=$('settingsApplyRestoreBtn'),done=beginButtonActivity(button,'Restoring…');try{let payload=pending.payload;if(!payload){payload=await decryptCompleteBackup(pending.parsed,$('settingsRestorePassword')?.value||'')}if(payload?.format!=='NewzDeckBackup'&&payload?.format!=='NewzDeckConfigBackup')throw new Error('This is not a NewzDeck backup.');const summary=backupSummaryText(payload);setBackupStatus(`Restoring ${summary}…`,'');const result=await api('/api/config/restore',payload,{timeoutMs:120000,timeoutMessage:'Backup restore took too long to respond.'});if(payload?.presentation?.theme)applyTheme(payload.presentation.theme,{persist:true});await loadProviders();const saved=result.settings||await api('/api/settings');populateSettingsForm(saved,await api('/api/health'));state.pendingRestoreBackup=null;$('settingsRestoreInput').value='';$('settingsRestoreReady')?.classList.add('hidden');setBackupStatus(`Restore complete.${result.warnings?.length?' '+result.warnings.join(' '):''}`,'success');toast(`Backup restored.${result.warnings?.length?' '+result.warnings[0]:''}`,'success')}catch(e){setBackupStatus(`Restore failed: ${e.message}`,'error');toast(`Restore failed: ${e.message}`,'error')}finally{done()}}
function clearRestoreBackupSelection(){state.pendingRestoreBackup=null;if($('settingsRestoreInput'))$('settingsRestoreInput').value='';$('settingsRestoreReady')?.classList.add('hidden');if($('settingsRestorePassword'))$('settingsRestorePassword').value='';setBackupStatus('','')}
...
function applyPreviewConcurrency(connections,value,mode){
  ...
  state.videoThumbConcurrency=Math.max(1,Math.min(6,connections>=48?6:connections>=24?3:connections>=12?2:1,state.thumbConcurrency));
  state.setCoverConcurrency=state.groupRelatedMedia&&!state.activeMediaSetKey?Math.max(1,Math.min(3,state.thumbConcurrency)):Math.min(6,state.thumbConcurrency);
...
async function selectGroup(group,opts={}){if(!group)return;if(state.browseAbortController)state.browseAbortController.abort();const previousGroup=state.selectedGroup,previousProvider=state.providerId;const tab=ensureBrowserTab(group,{newTab:false,tabId:opts.tabId||''});if(tab)state.activeBrowserTabId=tab.id;if(group===state.selectedGroup&&state.articles.length&&!opts.fromTab){state.loadedPages.clear();state.articlePage=1;state.articlePaging=null;state.galleryGeneration++;state.thumbQueue=[];state.thumbQueued.clear();state.mediaSetBrokenKeys.clear();state.mediaSetProbePromises.clear();state.mediaSetStreamAnchors.clear();els.articlesList.scrollTop=0;await loadArticles({page:1,append:false,refresh:true});return}captureCurrentGroupState();if(previousGroup&&previousGroup!==group)endGroupVisit(previousGroup,previousProvider);if(state.groupSearchJob&&['queued','scanning','cancelling'].includes(state.groupSearchJob.status)&&state.groupSearchJob.group!==group){api('/api/group-search/cancel',{id:state.groupSearchJob.id}).catch(()=>{});}if(state.groupSearchPollTimer){clearInterval(state.groupSearchPollTimer);state.groupSearchPollTimer=null}closeMediaViewer();state.selectedGroup=group;state.nameResolutionAttempted.clear();state.nameResolutionFailures.clear();state.nameResolutionDeferred.clear();state.nameResolutionClassifications.clear();state.nameResolutionBackoffUntil=0;state.nameResolutionAutoRemaining=24;clearTimeout(state.nameResolutionTimer);state.nameResolutionTimer=null;clearNameResolutionResultRenderQueue();beginGroupVisit(group);markRecentGroup(group);renderBrowserTabs();state.selectedItems.clear();state.selectionAnchorKey='';state.articleSearchTerm='';state.activeMediaSetKey='';state.mediaSetBrokenKeys.clear();state.mediaSetProbePromises.clear();state.mediaSetStreamAnchors.clear();state.searchMode=false;state.groupSearchJob=null;state.continuousLoading=false;els.articleSearch.value='';state.galleryGeneration++;state.thumbQueue=[];state.thumbQueued.clear();renderGroups();updateSelectionBar();updateArticleSearchUi();els.entireGroupSearchBtn.disabled=false;els.articleTitle.textContent=group;els.articleEyebrow.textContent='VISUAL NEWSGROUP BROWSER';resetPreview();const key=groupStateKey(group),persisted=state.groupStates[key]||{};applyStoredGroupControls(persisted);state.groupSessions.delete(key);state.articles=[];state.selectedArticleKey='';state.articlePage=1;state.articlePaging=null;state.loadedPages.clear();state.browseScrollTop=0;state.browsePageBeforeSearch=1;els.articlesList.scrollTop=0;await loadArticles({page:1,append:false,refresh:true});requestAnimationFrame(()=>{els.articlesList.scrollTop=0;markGroupSeen(group)});}
...
function clearPreviewUnavailableForGroup(group){
  ...
  for(const key of [...state.unpreviewableMediaKeys]){if(key.startsWith(prefix)&&!state.unsupportedMediaKeys.has(key))state.unpreviewableMediaKeys.delete(key)}
...
function canIncrementallyAppendContinuous(previousKeys,incoming){
  if(!state.continuousMode||state.searchMode||state.viewMode!=='gallery'||state.activeMediaSetKey)return false;
...
function appendContinuousGallery(previousKeys){
  ...
  if(additions.length){
    ...
    updateNewContentBoundaryDom();scheduleBrowsingMemoryTrim();scheduleGalleryVirtualization();
  }
  updateArticleSearchUi(filteredArticles().length);updateSelectionBar();return true;
}
function galleryVirtualizationEnabled(){return !!(state.continuousMode&&!state.searchMode&&effectiveViewMode()==='gallery'&&(els.articleSort?.value||'newest')==='newest'&&state.articles.length>700)}
function resetGalleryVirtualization(){clearTimeout(state.galleryVirtualTimer);state.galleryVirtualTimer=null;state.galleryVirtualTopBatches=[];state.galleryVirtualTopHeight=0;state.galleryVirtualBusy=false;els.articlesList?.querySelector?.('#galleryVirtualTopSpacer')?.remove()}
function galleryVirtualSpacer(grid){let spacer=grid?.querySelector?.('#galleryVirtualTopSpacer');if(!spacer&&grid){spacer=document.createElement('div');spacer.id='galleryVirtualTopSpacer';spacer.className='gallery-virtual-spacer';spacer.setAttribute('aria-hidden','true');grid.insertBefore(spacer,grid.firstChild)}return spacer}
function compactGalleryDom(){
  state.galleryVirtualTimer=null;if(state.galleryVirtualBusy||!galleryVirtualizationEnabled())return;const grid=els.articlesList?.querySelector('.media-grid');if(!grid)return;const cards=[...grid.querySelectorAll(':scope > .media-card[data-index]')],rootRect=els.articlesList.getBoundingClientRect(),maxLive=620;if(cards.length<=maxLive)return;
  const cutoff=rootRect.top-Math.max(rootRect.height*4,3200),limit=Math.min(180,cards.length-maxLive),rows=[];let row=null,count=0;
  for(const card of cards){const r=card.getBoundingClientRect();if(r.bottom>=cutoff)break;const top=Math.round(r.top);if(!row||Math.abs(top-row.top)>2){if(row&&count>=limit)break;row={top,cards:[],bottom:r.bottom};rows.push(row)}row.cards.push(card);row.bottom=Math.max(row.bottom,r.bottom);count++}
  if(!rows.length)return;
  const collapsing=rows.flatMap(x=>x.cards);if(!collapsing.length)return;state.galleryVirtualBusy=true;const keep=els.articlesList.scrollTop,started=performance.now(),spacer=galleryVirtualSpacer(grid),oldHeight=Number(state.galleryVirtualTopHeight||0),startTop=oldHeight>0?spacer.getBoundingClientRect().top:collapsing[0].getBoundingClientRect().top,lastBottom=Math.max(...rows.map(x=>x.bottom)),newHeight=Math.max(oldHeight,lastBottom-startTop),contribution=Math.max(1,newHeight-oldHeight),indices=[];
  for(const card of collapsing){const index=Number(card.dataset.index);if(Number.isFinite(index)){indices.push(index);unregisterThumbnailHolder(index,'item')}card.remove()}
  state.galleryVirtualTopBatches.push({indices,contribution});state.galleryVirtualTopHeight=newHeight;spacer.style.height=`${Math.max(1,newHeight)}px`;els.articlesList.scrollTop=keep;state.galleryVirtualBusy=false;perfRecord('virtualize',performance.now()-started);updateNewContentBoundaryDom();scheduleThumbnailDemandScan();if(grid.querySelectorAll(':scope > .media-card[data-index]').length>maxLive+160)scheduleGalleryVirtualization(90)
}
function rehydrateVirtualGalleryTopIfNeeded(){
  if(state.galleryVirtualBusy||!state.galleryVirtualTopBatches.length)return;const grid=els.articlesList?.querySelector('.media-grid'),spacer=grid?.querySelector('#galleryVirtualTopSpacer');if(!grid||!spacer)return;const root=els.articlesList.getBoundingClientRect(),r=spacer.getBoundingClientRect();if(root.top-r.bottom>root.height*2.3)return;
  state.galleryVirtualBusy=true;const started=performance.now(),keep=els.articlesList.scrollTop,anchors=captureBrowseAnchors(),batch=state.galleryVirtualTopBatches.pop(),visibleKeys=new Set(filteredArticles().map(x=>articleKey(x.a))),template=document.createElement('template'),html=[];
  for(const index of batch.indices||[]){const a=state.articles[index];if(!a||!visibleKeys.has(articleKey(a)))continue;if(state.groupRelatedMedia){const key=mediaSetKey(a),members=key&&state.mediaSetIndexValid?state.mediaSetIndexBuckets.get(key):null;if(members?.length>=3)continue}html.push(galleryCard(a,index))}
  template.innerHTML=html.join('');const restored=[...template.content.children];spacer.after(template.content);state.galleryVirtualTopHeight=Math.max(0,state.galleryVirtualTopHeight-Number(batch.contribution||0));if(state.galleryVirtualTopBatches.length){spacer.style.height=`${Math.max(1,state.galleryVirtualTopHeight)}px`}else{spacer.remove();state.galleryVirtualTopHeight=0}
  for(const card of restored){rebuildThumbnailHolderRegistry(card,{clear:false});wireGalleryCards(card);wireThumbnailImages(card);observeThumbnails({reuse:true,root:card})}state.galleryVirtualBusy=false;restoreBrowsePosition(anchors,keep);updateNewContentBoundaryDom();perfRecord('virtualize',performance.now()-started);scheduleThumbnailDemandScan()
}
function scheduleGalleryVirtualization(delay=180){if(!galleryVirtualizationEnabled())return;clearTimeout(state.galleryVirtualTimer);state.galleryVirtualTimer=setTimeout(compactGalleryDom,delay)}
function articlePageRequest(page,{refresh=false,progressive=false}={}){return browsePayload({provider_id:state.providerId,group:state.selectedGroup,limit:Number(els.articleLimit.value),page,media_only:false,content_filter:browserPerfMode(),smart_binaries:isAllPostsMode(),progressive,refresh})}
...
async function warmPrefetchedPageThumbnails(data,page,group,provider,session){
  ...
  for(const a of candidates){
    ...
    try{
      const result=await thumbnailImageApiTimed(browsePayload({provider_id:provider,group:articleGroup(a),segments:segmentPayload(a),media:a.media,thumbnail_lanes:1,content_filter:browserPerfMode()}),browseRequestOptions(),'speculative',{priority:2,role:'speculative-page'});
...
function scheduleProgressiveBinaryCompletion(page,generation,attempt=0){
  ...
  // Deep multipart reconstruction can legitimately take longer than the first dozen
  // cache polls on groups with large opaque yEnc streams.  Older builds stopped after
  // 12 checks, leaving the already-rendered spinner on screen forever even when the
  // backend completed moments later. Keep polling while this exact page/session is
  // active, but back off so a long reconstruction remains cheap.
  const delay=attempt<12?Math.min(900,220+attempt*70):attempt<45?1200:attempt<120?1800:2500;
  state.progressiveBinaryTimer=setTimeout(async()=>{
    ...
    try{const data=await api('/api/articles',articlePageRequest(page,{progressive:true}),browseRequestOptions());
      ...
      if(generation!==state.galleryGeneration||group!==state.selectedGroup||provider!==state.providerId||session!==state.browseSessionToken)return;
      state.smartBinaryPending=true;scheduleProgressiveBinaryCompletion(page,generation,attempt+1);
    }
  },delay);
}
async function loadArticles({page=null,append=false,refresh=false,pageJump=false}={}){
  ...
  if(!append){
    generation=++state.galleryGeneration;state.thumbQueue=[];state.thumbQueued.clear();resetGalleryVirtualization();resetMediaSetIndex();
...
function filteredArticles({ignoreStatus=false}={}){
  ...
  // Related Media viewer scope must never narrow the browser's master filtered
  // article collection. The viewer applies its own set-only filter in
  // viewerItems()/viewerSetMembers(). Keeping filteredArticles() global ensures
  // the side pane always retains every set discovered during Continuous browse.
  return items;
}
...
function renderArticles({preserveScroll=true,scrollTop=null,anchor=null,reason="unspecified"}={}){
  ...
  const items=filteredArticles(),mode=effectiveViewMode();updateArticleSearchUi(items.length);updateBrowseModeControls();if(isAllPostsMode()&&els.sidePaneTitle?.textContent!=='Preview')resetPreview();els.articlesList.classList.remove('empty-state','media-gallery','article-list-mode','binary-list-mode-active');
  ...
  if(!items.length){
    state.binarySetGroups.clear();state.thumbHolderRegistry.clear();resetMediaSetIndex();els.articlesList.classList.add('empty-state');
...
function addMediaSetIndexItem(item,changedKeys=null){
  if(!item?.a)return;const key=mediaSetKey(item.a),itemKey=articleKey(item.a);
  if(!key){state.mediaSetIndexSingles.set(itemKey,item);return}
  let members=state.mediaSetIndexBuckets.get(key);if(!members){members=[];state.mediaSetIndexBuckets.set(key,members)}
  if(!members.some(x=>articleKey(x.a)===itemKey))members.push(item);if(changedKeys)changedKeys.add(key)
}
function mediaSetIndexSnapshot({includeUngrouped=true,changedKeys=null}={}){
  const groups=[],groupMap=new Map(),ungrouped=includeUngrouped?[...state.mediaSetIndexSingles.values()]:[];let ungroupedCount=state.mediaSetIndexSingles.size,suppressedGroups=0;
  for(const[key,members]of state.mediaSetIndexBuckets){
    if(members.length>=3){const group={key,members};groupMap.set(key,group);if(state.mediaSetBrokenKeys.has(key)||!members.some(x=>x.a?.complete))suppressedGroups++;else groups.push(group)}
    else{ungroupedCount+=members.length;if(includeUngrouped)ungrouped.push(...members)}
  }
  return{groups,ungrouped,ungroupedCount,suppressedGroups,stream:includeUngrouped?ungrouped.map(item=>({type:'item',item})):[],buckets:state.mediaSetIndexBuckets,groupMap,changedKeys}
}
function rebuildMediaSetIndex(items,{includeUngrouped=true}={}){
  const started=performance.now();resetMediaSetIndex();for(const item of items||[])addMediaSetIndexItem(item);state.mediaSetIndexValid=true;const result=mediaSetIndexSnapshot({includeUngrouped});perfRecord('group_index',performance.now()-started);return result
}
function appendMediaSetIndex(items){
  if(!state.mediaSetIndexValid)return rebuildMediaSetIndex(filteredArticles(),{includeUngrouped:false});const started=performance.now(),changedKeys=new Set();for(const item of items||[])addMediaSetIndexItem(item,changedKeys);const result=mediaSetIndexSnapshot({includeUngrouped:false,changedKeys});perfRecord('group_index',performance.now()-started);return result
}
function buildMediaSets(items){return rebuildMediaSetIndex(items,{includeUngrouped:true})}
...
function liveMediaSetItem(item){
  if(!item?.a)return item;const current=state.articles[Number(item.index)];if(current&&articleKey(current)===articleKey(item.a))return item;const index=state.articles.findIndex(a=>articleKey(a)===articleKey(item.a));return index>=0?{...item,index}:item
}
function mediaSetRepresentative(group){const complete=group.members.filter(x=>x.a?.complete),candidates=complete.filter(x=>{const key=previewKey(x.a);return !state.unpreviewableMediaKeys.has(key)&&!state.unsupportedMediaKeys.has(key)}),pool=candidates.length?candidates:(complete.length?complete:group.members),ranked=[...pool].map(liveMediaSetItem).sort((x,y)=>{const ax=x.a,ay=y.a;const cx=(state.imageThumbCache.has(previewKey(ax))||state.videoThumbCache.has(previewKey(ax)))?0:1,cy=(state.imageThumbCache.has(previewKey(ay))||state.videoThumbCache.has(previewKey(ay)))?0:1;if(cx!==cy)return cx-cy;const sx=Number(ax.segment_count||1),sy=Number(ay.segment_count||1);if(sx!==sy)return sx-sy;const bx=Number(ax.bytes||0)||Number.MAX_SAFE_INTEGER,by=Number(ay.bytes||0)||Number.MAX_SAFE_INTEGER;if(bx!==by)return bx-by;return Number(ay.article||0)-Number(ax.article||0)});return ranked[0]||liveMediaSetItem(group.members[0])}
function mediaSetUnavailableError(setKey){const e=new Error('Related media set is unavailable on this provider.');e.data={error:'Sampled members of this related-media set are missing, incomplete, or corrupt.',error_code:'media_set_unavailable',error_label:'Related media set unavailable',retryable:false};e.mediaSetKey=setKey;return e}
function mediaSetProbeMembers(setKey,preferredIndex){
  const indexed=state.mediaSetIndexValid?state.mediaSetIndexBuckets.get(setKey):null;const members=(indexed||filteredArticles().filter(x=>mediaSetKey(x.a)===setKey)).filter(x=>x.a.complete).map(liveMediaSetItem);if(!members.length)return[];
  const preferred=members.find(x=>x.index===preferredIndex)||members[0],positions=[0,Math.floor((members.length-1)/2),members.length-1],picked=[preferred],seen=new Set([articleKey(preferred.a)]);
  for(const pos of positions){const item=members[pos];if(item&&!seen.has(articleKey(item.a))){seen.add(articleKey(item.a));picked.push(item)}}
  for(const item of members){if(picked.length>=3)break;if(!seen.has(articleKey(item.a))){seen.add(articleKey(item.a));picked.push(item)}}
  return picked.slice(0,Math.min(3,members.length));
}
async function fetchMediaSetCover(task,a){
  const setKey=String(task?.setKey||mediaSetKey(a)||'');if(!setKey)return{index:task.index,articleKey:articleKey(a),data:await fetchImageThumbnail(a,{...task,requestTimeoutMs:7000})};if(state.mediaSetBrokenKeys.has(setKey))throw mediaSetUnavailableError(setKey);
  const probeKey=`${state.providerId}|${state.selectedGroup}|${setKey}`;if(state.mediaSetProbePromises.has(probeKey))return state.mediaSetProbePromises.get(probeKey);
  const request=(async()=>{
    const samples=mediaSetProbeMembers(setKey,task.index);if(!samples.length)throw mediaSetUnavailableError(setKey);
    let definitiveFailures=0,lastError=null;
    // r8: one real cover request at a time. The old Promise.all() fan-out made one
    // scheduler task secretly consume up to three NNTP thumbnail requests and could
    // wedge many side-pane covers behind requests the scheduler could not account for.
    for(const item of samples){
      try{
        const data=await fetchImageThumbnail(item.a,{...task,index:item.index,role:'set-probe',priority:0,distance:0,requestTimeoutMs:7000});
        if(data?.suppressed_small){const info={message:'Image is too small for the gallery.',label:'Image suppressed',code:'small_image',retryable:false};rememberPreviewUnavailable(item.a,info);definitiveFailures++;continue}
        if(data?.url)return{index:item.index,articleKey:articleKey(item.a),data};
        lastError=new Error('The Related Media cover did not return a thumbnail.');lastError.data={error:'The Related Media cover did not return a thumbnail.',error_code:'set_cover_empty',error_label:'Cover unavailable',retryable:true};break;
      }catch(error){
        const info=friendlyPreviewError(error),definitive=thumbnailFailureSkipsFullFallback(error);lastError=error;
        if(definitive){rememberPreviewUnavailable(item.a,info);definitiveFailures++;continue}
        // Timeout/transient errors are terminal for this attempt so a card can never
        // sit in Loading forever. The user can retry without spawning hidden fan-out.
        break;
      }
    }
    if(definitiveFailures===samples.length){state.mediaSetBrokenKeys.add(setKey);throw mediaSetUnavailableError(setKey)}
    throw lastError||new Error('Could not determine a related-media set cover.');
  })().finally(()=>state.mediaSetProbePromises.delete(probeKey));
  state.mediaSetProbePromises.set(probeKey,request);return request;
}
function mediaSetNewestArticle(group){return Math.max(0,...group.members.map(x=>Number(x.a?.article||0)))}
function mediaSetCard(group){
  const rep=mediaSetRepresentative(group),a=rep.a,index=rep.index,kind=a.media?.kind||'media',title=mediaSetTitle(group),bytes=group.members.reduce((n,x)=>n+Number(x.a.bytes||0),0),cache=kind==='image'?state.imageThumbCache.get(previewKey(a)):state.videoThumbCache.get(previewKey(a));
  const visual=cache?.url?`<img class="thumb-img" src="${cache.url}" alt="${escapeHtml(title)}" decoding="async" data-thumb-image-index="${index}" data-thumb-role="set-cover" data-thumb-set-key="${escapeHtml(group.key)}">`:`<div class="thumb-loader ${kind==='video'?'video-thumb-loader':''}" data-thumb-index="${index}" data-thumb-role="set-cover" data-thumb-set-key="${escapeHtml(group.key)}"><span></span><small>Loading cover…</small></div>`;
  return`<article class="media-set-card related-set-card" data-set-key="${escapeHtml(group.key)}" data-set-newest="${mediaSetNewestArticle(group)}" data-set-cover-index="${index}" data-set-cover-state="${cache?.url?'loaded':'pending'}"><div class="media-set-cover">${visual}<span class="set-badge">${group.members.length.toLocaleString()} ${kind==='image'?'IMAGES':'VIDEOS'}</span><span class="kind-pill ${kind}">${kind.toUpperCase()} SET</span></div><div class="media-set-info"><strong title="${escapeHtml(title)}">${escapeHtml(title)}</strong><span class="media-set-meta">${formatBytes(bytes)} • ${shortDate(a.date)}</span><div class="media-set-actions"><button class="set-open-btn" type="button">Open set</button><button class="set-queue-btn" type="button">＋ Queue all</button></div></div></article>`
}
function mediaSetGroupByKey(setKey){if(!setKey)return null;const members=state.mediaSetIndexValid?(state.mediaSetIndexBuckets.get(setKey)||[]):filteredArticles().filter(x=>mediaSetKey(x.a)===setKey);return members.length>=3?{key:setKey,members}:null}
function wireMediaSetCards(root=els.previewContent){
  const cards=root?.matches?.('.media-set-card')?[root]:[...(root?.querySelectorAll?.('.media-set-card')||[])];
  cards.forEach(card=>{if(card.dataset.setWired==='1')return;card.dataset.setWired='1';const open=()=>{const group=mediaSetGroupByKey(card.dataset.setKey);if(!group)return;const rep=mediaSetRepresentative(group);if(!rep?.a)return;state.activeMediaSetKey=group.key;openMediaViewer(rep.a)};card.addEventListener('click',e=>{if(!e.target.closest('button'))open()});card.querySelector('.set-open-btn')?.addEventListener('click',e=>{e.stopPropagation();open()});card.querySelector('.set-queue-btn')?.addEventListener('click',e=>{e.stopPropagation();const group=mediaSetGroupByKey(card.dataset.setKey);downloadItems(group?group.members.map(x=>x.a):[],e.currentTarget)})})
}
function updateMediaSetCardInPlace(card,group){
  if(!card||!group)return false;const rep=mediaSetRepresentative(group),a=rep.a,kind=a.media?.kind||'media',bytes=group.members.reduce((n,x)=>n+Number(x.a.bytes||0),0),title=mediaSetTitle(group);
  const badge=card.querySelector('.set-badge');if(badge)badge.textContent=`${group.members.length.toLocaleString()} ${kind==='image'?'IMAGES':'VIDEOS'}`;
  const titleNode=card.querySelector('.media-set-info > strong');if(titleNode){titleNode.textContent=title;titleNode.title=title}
  const meta=card.querySelector('.media-set-meta')||card.querySelector('.media-set-info > span');if(meta){meta.classList.add('media-set-meta');meta.textContent=`${formatBytes(bytes)} • ${shortDate(a.date)}`}
  const previousCoverIndex=Number(card.dataset.setCoverIndex??-1),hasLoadedCover=!!card.querySelector('.media-set-cover .thumb-img');
  if(!hasLoadedCover&&Number.isFinite(rep.index)&&rep.index!==previousCoverIndex){
    const cover=card.querySelector('.media-set-cover'),oldHolder=cover?.querySelector('[data-thumb-index][data-thumb-role="set-cover"]');if(oldHolder)unregisterThumbnailHolder(Number(oldHolder.dataset.thumbIndex),'set-cover');
    cover?.querySelectorAll('.thumb-loader,.thumb-error,.video-play-overlay').forEach(n=>n.remove());
    if(cover){cover.insertAdjacentHTML('afterbegin',`<div class="thumb-loader ${kind==='video'?'video-thumb-loader':''}" data-thumb-index="${rep.index}" data-thumb-role="set-cover" data-thumb-set-key="${escapeHtml(group.key)}"><span></span><small>Loading cover…</small></div>`);registerThumbnailHolderNode(cover.querySelector('[data-thumb-index][data-thumb-role="set-cover"]'),rep.index,'set-cover')}
    card.dataset.setCoverIndex=String(rep.index);card.dataset.setCoverState='pending';
  }
  scheduleRelatedSetCoverActivation();return true;
}
function relatedSetPaneEmpty(icon,title,message){return`<div class="related-sets-empty"><div class="related-sets-empty-icon">${icon}</div><h3>${escapeHtml(title)}</h3><p>${escapeHtml(message)}</p></div>`}
function setBrowseSidePaneHeader(eyebrow,title,badge,badgeActive=false){if(els.sidePaneEyebrow)els.sidePaneEyebrow.textContent=eyebrow;if(els.sidePaneTitle)els.sidePaneTitle.textContent=title;els.previewBadge.className=`preview-badge${badgeActive?' active':''}`;els.previewBadge.textContent=badge}
function shouldThrottleRelatedSetCovers(){const since=Date.now()-Number(state.lastBrowseScrollTs||0),velocity=Math.abs(Number(state.browseScrollVelocity||0)),foreground=state.thumbQueue.some(t=>t.role!=='set-cover'&&Number(t.priority??1)===0);return since<450||velocity>.06||foreground}
function scheduleRelatedSetCoverResume(delay=520){clearTimeout(state.relatedCoverResumeTimer);state.relatedCoverResumeTimer=setTimeout(()=>{state.relatedCoverResumeTimer=null;pumpThumbQueue();if(shouldThrottleRelatedSetCovers()){scheduleRelatedSetCoverResume(320);return}observeRelatedSetCovers(els.previewContent);pumpThumbQueue()},delay)}
let relatedCoverActivationRaf=0,relatedCoverActivationTimer=null;
function cachedMediaSetCover(group){
  if(!group?.members?.length)return null;const rep=mediaSetRepresentative(group),ordered=[];if(rep?.a)ordered.push(rep);for(const raw of group.members){const item=liveMediaSetItem(raw);if(!item?.a?.complete||ordered.some(x=>articleKey(x.a)===articleKey(item.a)))continue;ordered.push(item)}
  for(const item of ordered){const a=item.a,cache=a.media?.kind==='video'?state.videoThumbCache.get(previewKey(a)):state.imageThumbCache.get(previewKey(a));if(cache?.url)return{item,data:cache}}
  return null;
}
function ensureRelatedCoverLoader(card,setKey,rep){
  if(!card||!rep?.a)return null;let holder=card.querySelector('.media-set-cover .thumb-loader');if(!holder){const cover=card.querySelector('.media-set-cover');if(!cover)return null;cover.querySelectorAll('.thumb-error,.video-play-overlay').forEach(n=>n.remove());cover.insertAdjacentHTML('afterbegin',`<div class="thumb-loader ${rep.a.media?.kind==='video'?'video-thumb-loader':''}" data-thumb-index="${rep.index}" data-thumb-role="set-cover" data-thumb-set-key="${escapeHtml(setKey)}"><span></span><small>Loading cover…</small></div>`);holder=cover.querySelector('.thumb-loader');state.relatedCoverStats.orphanRepairs++}
  const oldIndex=Number(holder.dataset.thumbIndex);if(!Number.isFinite(oldIndex)||oldIndex!==rep.index||holder.dataset.thumbRole!=='set-cover'||holder.dataset.thumbSetKey!==setKey){if(Number.isFinite(oldIndex))unregisterThumbnailHolder(oldIndex,'set-cover');holder.dataset.thumbIndex=String(rep.index);holder.dataset.thumbRole='set-cover';holder.dataset.thumbSetKey=setKey;state.relatedCoverStats.orphanRepairs++}
  registerThumbnailHolderNode(holder,rep.index,'set-cover');card.dataset.setCoverIndex=String(rep.index);card.dataset.setCoverState='pending';return holder;
}
function activateRelatedSetCovers({margin=500}={}){
  if(state.activeView!=='browse'||isAllPostsMode()||!state.groupRelatedMedia||!els.previewContent||!state.selectedGroup)return;state.relatedCoverStats.activationPasses++;
  const root=els.previewContent.getBoundingClientRect(),top=root.top-Math.max(0,Number(margin||0)),bottom=root.bottom+Math.max(0,Number(margin||0));
  for(const card of els.previewContent.querySelectorAll('.media-set-card[data-set-key]')){
    const r=card.getBoundingClientRect();if(r.bottom<=top||r.top>=bottom)continue;if(card.querySelector('.media-set-cover .thumb-img')||card.querySelector('.media-set-cover .thumb-error'))continue;
    const setKey=card.dataset.setKey,group=mediaSetGroupByKey(setKey);if(!group)continue;
    const cached=cachedMediaSetCover(group);if(cached){if(updateMediaSetCoverDom({setKey,index:cached.item.index,sourceArticleKey:articleKey(cached.item.a)},cached.item.index,cached.data,articleKey(cached.item.a)))state.relatedCoverStats.cachePaints++;continue}
    const rep=mediaSetRepresentative(group);if(!rep?.a?.complete)continue;ensureRelatedCoverLoader(card,setKey,rep);
    const visible=r.bottom>root.top&&r.top<root.bottom,distance=visible?0:Math.min(Math.abs(r.top-root.bottom),Math.abs(root.top-r.bottom)),qkey=relatedCoverTaskKey(setKey),tracked=state.thumbQueue.some(t=>t.qkey===qkey)||state.thumbActiveTasks.has(qkey);
    if(state.thumbQueued.has(qkey)&&!tracked)state.thumbQueued.delete(qkey);
    // Visible Related Media: call through even when this set is already queued. queueThumbnail() can
    // promote the existing set-key task from near/offscreen priority to visible
    // priority in place, rather than leaving it permanently classified offscreen.
    if(queueThumbnail(rep.index,visible?0:1,distance,'set-cover',{setKey}))state.relatedCoverStats.activationQueues++;
  }
  pumpThumbQueue();
}
function scheduleRelatedSetCoverActivation(delay=0){
  if(delay>0){clearTimeout(relatedCoverActivationTimer);relatedCoverActivationTimer=setTimeout(()=>{relatedCoverActivationTimer=null;scheduleRelatedSetCoverActivation(0)},delay);return}
  if(relatedCoverActivationRaf)return;relatedCoverActivationRaf=requestAnimationFrame(()=>{relatedCoverActivationRaf=0;activateRelatedSetCovers()});
}
function observeRelatedSetCovers(root=els.previewContent){
  if(isAllPostsMode()||!root)return;if(relatedSetObserver)relatedSetObserver.disconnect();
  relatedSetObserver=new IntersectionObserver(entries=>{const rootRect=els.previewContent.getBoundingClientRect();for(const entry of entries){if(!entry.isIntersecting)continue;const card=entry.target,setKey=card.dataset.setKey,group=mediaSetGroupByKey(setKey),rep=group?mediaSetRepresentative(group):null;if(!rep?.a)continue;const r=entry.boundingClientRect,visible=r.bottom>rootRect.top&&r.top<rootRect.bottom,distance=visible?0:Math.min(Math.abs(r.top-rootRect.bottom),Math.abs(rootRect.top-r.bottom));const loader=card.querySelector('.media-set-cover .thumb-loader');if(loader){loader.dataset.thumbIndex=String(rep.index);loader.dataset.thumbRole='set-cover';loader.dataset.thumbSetKey=setKey;registerThumbnailHolderNode(loader,rep.index,'set-cover');card.dataset.setCoverIndex=String(rep.index);card.dataset.setCoverState='pending';queueThumbnail(rep.index,visible?0:1,distance,'set-cover',{setKey})}if(shouldThrottleRelatedSetCovers())scheduleRelatedSetCoverResume()}},{root:els.previewContent,rootMargin:'350px 0px 700px 0px',threshold:.01});
  root.querySelectorAll('.media-set-card[data-set-key]').forEach(card=>relatedSetObserver.observe(card));
  // r10: physical visibility is checked immediately and card identity is authoritative;
  // transient representative-index/holder changes can no longer orphan a spinner.
  scheduleRelatedSetCoverActivation();
}
function relatedSetPaneSummaryMarkup(built){const individual=Number(built.ungroupedCount??built.ungrouped?.length??0);return`<div class="related-sets-summary"><strong>${built.groups.length.toLocaleString()} set${built.groups.length===1?'':'s'}</strong><span>${individual.toLocaleString()} individual${built.suppressedGroups?` • ${built.suppressedGroups.toLocaleString()} unavailable hidden`:''}</span></div>`}
function renderRelatedMediaPane(built=null,{incremental=false,changedKeys=null}={}){
  if(isAllPostsMode())return false;
  if(relatedSetObserver){relatedSetObserver.disconnect();relatedSetObserver=null}
  setBrowseSidePaneHeader('GROUPED SETS','Related Media',state.groupRelatedMedia?'LIVE':'OFF',state.groupRelatedMedia);
  els.previewContent.className='preview-content related-sets-content';
  if(!state.selectedGroup){els.previewContent.innerHTML=relatedSetPaneEmpty('▦','Choose a newsgroup','Related image and video sets discovered while browsing will appear here.');return true}
  if(!state.groupRelatedMedia){els.previewContent.innerHTML=relatedSetPaneEmpty('▦','Group Related Media is off','Turn on Group related to move image/video sets out of the main browser and organize them here.');return true}
  const grouped=built||(state.mediaSetIndexValid?mediaSetIndexSnapshot({includeUngrouped:false}):buildMediaSets(filteredArticles())),groups=[...grouped.groups].sort((a,b)=>mediaSetNewestArticle(b)-mediaSetNewestArticle(a));
  setBrowseSidePaneHeader('GROUPED SETS','Related Media',`${groups.length.toLocaleString()} SET${groups.length===1?'':'S'}`,groups.length>0);
  if(!groups.length){els.previewContent.innerHTML=relatedSetPaneEmpty('◫','No related sets yet','Keep browsing. Sets appear here automatically after NewzDeck identifies at least three related media posts.');return true}
  const oldTop=Number(els.previewContent.scrollTop||0),oldHeight=Number(els.previewContent.scrollHeight||0),canIncrement=incremental&&!!els.previewContent.querySelector('.related-sets-list');
  if(!canIncrement){els.previewContent.innerHTML=relatedSetPaneSummaryMarkup(grouped)+`<div class="related-sets-list">${groups.map(mediaSetCard).join('')}</div>`;rebuildThumbnailHolderRegistry(els.previewContent,{clear:false});wireMediaSetCards(els.previewContent);wireThumbnailImages(els.previewContent);observeRelatedSetCovers(els.previewContent);scheduleRelatedSetCoverActivation();els.previewContent.scrollTop=Math.min(oldTop,Math.max(0,els.previewContent.scrollHeight-els.previewContent.clientHeight));return true}
  const summary=els.previewContent.querySelector('.related-sets-summary'),list=els.previewContent.querySelector('.related-sets-list');if(summary)summary.outerHTML=relatedSetPaneSummaryMarkup(grouped);
  const changed=changedKeys&&changedKeys.size?new Set(changedKeys):null;
  if(!changed){const live=new Set(groups.map(g=>g.key));list.querySelectorAll('.media-set-card[data-set-key]').forEach(card=>{if(!live.has(card.dataset.setKey)){unregisterThumbnailHolder(Number(card.querySelector('[data-thumb-index]')?.dataset.thumbIndex||-1),'set-cover');card.remove()}})}
  const targetGroups=changed?groups.filter(g=>changed.has(g.key)):groups,existing=new Map([...list.querySelectorAll('.media-set-card[data-set-key]')].map(card=>[card.dataset.setKey,card])),created=[];
  if(changed){for(const key of changed){if(grouped.groupMap.has(key)&&!state.mediaSetBrokenKeys.has(key))continue;const card=existing.get(key);if(card){unregisterThumbnailHolder(Number(card.querySelector('[data-thumb-index]')?.dataset.thumbIndex||-1),'set-cover');card.remove();existing.delete(key)}}}
  for(const group of targetGroups){let card=existing.get(group.key);if(card){updateMediaSetCardInPlace(card,group);card.dataset.setNewest=String(mediaSetNewestArticle(group));continue}const wrap=document.createElement('template');wrap.innerHTML=mediaSetCard(group);card=wrap.content.firstElementChild;if(!card)continue;let before=null;for(const node of list.querySelectorAll('.media-set-card[data-set-key]')){if(Number(node.dataset.setNewest||0)<mediaSetNewestArticle(group)){before=node;break}}if(before)list.insertBefore(card,before);else list.appendChild(card);created.push(card)}
  for(const card of created){rebuildThumbnailHolderRegistry(card,{clear:false});wireMediaSetCards(card);wireThumbnailImages(card)}observeRelatedSetCovers(els.previewContent);scheduleRelatedSetCoverActivation();
  if(oldTop>12){const delta=els.previewContent.scrollHeight-oldHeight;if(delta<0)els.previewContent.scrollTop=Math.max(0,oldTop+delta);else els.previewContent.scrollTop=oldTop}
  return true
}
function refreshGroupedGallerySummary(grouped=null){
  const summary=els.articlesList?.querySelector('.sets-summary');if(!summary)return;const built=grouped||buildMediaSets(filteredArticles());const strong=summary.querySelector('strong'),span=summary.querySelector('span');if(strong)strong.textContent=`${built.groups.length} related media set${built.groups.length===1?'':'s'}`;if(span)span.textContent=`${built.ungrouped.length} individual post${built.ungrouped.length===1?'':'s'}${built.suppressedGroups?` • ${built.suppressedGroups} unavailable set${built.suppressedGroups===1?'':'s'} hidden`:''}`;
}
function ensureGroupedSummary(grouped){
  let summary=els.articlesList?.querySelector('.sets-summary');if(!summary){summary=document.createElement('div');summary.className='sets-summary';summary.innerHTML='<strong></strong><span></span>';els.articlesList.insertBefore(summary,els.articlesList.firstChild)}refreshGroupedGallerySummary(grouped);return summary;
}
function appendContinuousGroupedGallery(previousKeys){
  const grid=els.articlesList?.querySelector('.media-grid');if(!grid)return false;const items=filteredArticles(),newItems=items.filter(({a})=>!previousKeys.has(articleKey(a))),built=appendMediaSetIndex(newItems);
  if(!newItems.length){renderRelatedMediaPane(built,{incremental:true,changedKeys:built.changedKeys});updateArticleSearchUi(items.length);updateSelectionBar();scheduleGalleryVirtualization();return true}
  const keep=els.articlesList.scrollTop,anchors=captureBrowseAnchors(),template=document.createElement('template'),newCards=[],itemHtml=[];let removedExisting=false;
  for(const item of newItems){
    const key=mediaSetKey(item.a),group=key?built.groupMap.get(key):null;
    if(group&&group.members.length>=3){
      for(const member of group.members){if(!previousKeys.has(articleKey(member.a)))continue;const old=grid.querySelector(`.media-card[data-index="${member.index}"]`);if(old){old.remove();removedExisting=true}}
      continue;
    }
    itemHtml.push(galleryCard(item.a,item.index));
  }
  if(itemHtml.length){template.innerHTML=itemHtml.join('');const sentinel=$('continuousSentinel');const added=[...template.content.children];if(sentinel&&sentinel.parentElement===grid)grid.insertBefore(template.content,sentinel);else grid.appendChild(template.content);newCards.push(...added)}
  for(const card of newCards){rebuildThumbnailHolderRegistry(card,{clear:false});wireGalleryCards(card);wireThumbnailImages(card);observeThumbnails({reuse:true,root:card})}
  renderRelatedMediaPane(built,{incremental:true,changedKeys:built.changedKeys});if(removedExisting)restoreBrowsePosition(anchors,keep);updateNewContentBoundaryDom();scheduleBrowsingMemoryTrim();scheduleGalleryVirtualization();updateArticleSearchUi(items.length);updateSelectionBar();return true
}
function renderGroupedGallery(items){
  const built=buildMediaSets(items),visible=built.ungrouped;
  const main=visible.length?visible.map(({a,index})=>galleryCard(a,index)).join(''):`<div class="grouped-main-empty"><span>▦</span><strong>Grouped media moved to Related Media</strong><small>Continue scrolling to discover individual posts and more sets.</small></div>`;
  els.articlesList.innerHTML=`<div class="media-grid">${main}${continuousSentinelMarkup()}</div>`;
  wireGalleryCards();wireThumbnailImages();observeThumbnails();renderRelatedMediaPane(built,{incremental:false});updateNewContentBoundaryDom();scheduleGalleryVirtualization(260);return true
}
function renderGallery(items){
  if(state.groupRelatedMedia){renderGroupedGallery(items);return}
  els.articlesList.innerHTML=`<div class="media-grid">${items.map(({a,index})=>galleryCard(a,index)).join('')}${continuousSentinelMarkup()}</div>`;
  wireGalleryCards();wireThumbnailImages();observeThumbnails();renderRelatedMediaPane(null,{incremental:false});updateNewContentBoundaryDom();scheduleGalleryVirtualization(260);
}
function wireGalleryCards(root=els.articlesList){const cards=root?.matches?.('.media-card')?[root]:[...(root?.querySelectorAll?.('.media-card')||[])];cards.forEach(card=>{if(card.dataset.galleryWired==='1')return;card.dataset.galleryWired='1';card.addEventListener('click',e=>{if(e.target.closest('.card-download-btn,.thumb-retry'))return;handleMediaSelectionClick(Number(card.dataset.index),e)});card.addEventListener('dblclick',e=>{if(e.target.closest('.card-download-btn,.thumb-retry'))return;const a=state.articles[Number(card.dataset.index)];if(['image','video'].includes(a?.media?.kind)&&a.complete){e.preventDefault();state.activeMediaSetKey='';openMediaViewer(a)}});card.addEventListener('contextmenu',e=>{e.preventDefault();const a=state.articles[Number(card.dataset.index)];if(!a)return;showContextMenu(e.clientX,e.clientY,[{label:'Open viewer',action:()=>{state.activeMediaSetKey='';openMediaViewer(a)}},{label:'Add to download queue',disabled:!isSelectableMedia(a),action:()=>downloadItems([a])},{separator:true},{label:isArticleSeen(a)?'Mark post unseen':'Mark post seen',action:()=>setArticlesSeen([a],!isArticleSeen(a),{toastResult:true})},{separator:true},{label:'Copy filename',action:()=>copyText(a.media?.filename||a.subject)},{label:'Copy subject',action:()=>copyText(a.subject||'')},{label:'Copy Message-ID',action:()=>copyText(a.message_id||'')},{separator:true},{label:isPosterBlocked(a)?'Unmute this poster':'Mute this poster',action:()=>mutePoster(a.from)},{label:'Filter to this poster',action:()=>{state.articleSearchTerm=`from:${a.from}`;els.articleSearch.value=state.articleSearchTerm;renderArticles()}},{label:'Select item',action:()=>{state.selectedArticleKey=articleKey(a);updateActiveArticleDomInPlace()}}])})});els.articlesList.querySelectorAll('.card-download-btn').forEach(btn=>btn.onclick=e=>{e.stopPropagation();downloadItems([state.articles[Number(btn.dataset.index)]],btn)});els.articlesList.querySelectorAll('[data-retry-thumb]').forEach(btn=>btn.onclick=e=>{e.stopPropagation();const index=Number(btn.dataset.retryThumb),a=state.articles[index];if(a)state.thumbFailureCache.delete(previewKey(a));renderArticles()})}
...
function thumbFailureMarkup(f,kind,index){const retry=f.info?.retryable===false?'':`<button type="button" class="thumb-retry" data-retry-thumb="${index}">Retry</button>`;return `<div class="thumb-error cached-thumb-error"><span>!</span><small>${kind==='video'?'Video thumbnail unavailable':'Preview unavailable'}</small><em class="error-label">${escapeHtml(f.info?.label||'Preview unavailable')}</em>${retry}</div>`}
function updateNewContentBoundaryDom(){
  ...
  const nodes=[...els.articlesList.querySelectorAll('.media-card[data-index],.media-set-card[data-set-key],.article-row[data-index]:not(.binary-set-member),.binary-set-row[data-binary-set-key]')];els.articlesList.querySelectorAll('.new-content-boundary').forEach(x=>x.remove());if(!nodes.length)return;let sawNew=false,boundaryBefore=null;
  for(const node of nodes){let fresh=false;if(node.dataset.binarySetKey){const group=state.binarySetGroups.get(node.dataset.binarySetKey);fresh=!!group?.members?.some(x=>isArticleNew(x.a));}else if(node.dataset.setKey){const group=mediaSetGroupByKey(node.dataset.setKey);fresh=!!group?.members?.some(x=>isArticleNew(x.a));}else{const a=state.articles[Number(node.dataset.index)];fresh=isArticleNew(a)}if(fresh){sawNew=true;continue}if(sawNew){boundaryBefore=node;break}}
...
function isUnresolvedDownloadCandidate(a){
  if(!a?.complete||a?.media||isUnresolvedMultipartFragment(a))return false;
  return !!(a.opaque_multipart||articleNeedsNameResolution(a,{manual:true}));
}
function applyNameResolutionResultToArticle(a,r){
  if(!a||!r?.resolved||!r.media?.filename)return false;const before=String(a.media?.filename||'');
  a.media={...r.media};a.name_resolution={source:r.source||'yEnc header',confidence:r.confidence||'high',original_filename:before,title_hint:r.title_hint||'',metadata_source:r.metadata_source||'',metadata_names:Array.isArray(r.metadata_names)?r.metadata_names:[],archive_source:r.archive_source||'',archive_names:Array.isArray(r.archive_names)?r.archive_names:[],title_source:r.title_source||r.metadata_source||r.archive_source||r.source||'yEnc header',archive_checked:!!r.archive_checked};delete a.raw_yenc_name;delete a.name_resolution_state;
  state.nameResolutionFailures.delete(nameResolutionKey(a));state.nameResolutionClassifications.delete(nameResolutionKey(a));return before!==String(r.media.filename||'');
}
function nameResolutionClassification(a){return a?state.nameResolutionClassifications.get(nameResolutionKey(a))||null:null}
function recordNameResolutionFailures(results){
  let retryable=0,permanent=0,opaque=0,noFilename=0,articleUnavailable=0,maxBackoff=0;const now=Date.now(),articlesByKey=new Map(state.articles.map(a=>[articleKey(a),a]));
  for(const r of results||[]){const clientKey=String(r?.client_key||'');if(!clientKey)continue;const key=`${state.providerId}|${state.selectedGroup}|${clientKey}`,code=String(r?.code||'');
    if(r?.resolved){state.nameResolutionFailures.delete(key);state.nameResolutionDeferred.delete(key);state.nameResolutionClassifications.delete(key);continue}
    if(r?.retryable||r?.deferred){const seconds=Math.max(2,Number(r?.backoff_seconds||5));state.nameResolutionAttempted.delete(key);state.nameResolutionFailures.delete(key);state.nameResolutionClassifications.delete(key);state.nameResolutionDeferred.set(key,{message:String(r?.error||'Provider temporarily unavailable'),until:now+seconds*1000,code:code||'transient'});retryable++;maxBackoff=Math.max(maxBackoff,seconds);continue}
    state.nameResolutionDeferred.delete(key);state.nameResolutionFailures.delete(key);
    if(['opaque_yenc_name','filename_not_supplied','probe_articles_missing','no_article_reference'].includes(code)){const kind=code==='opaque_yenc_name'?'opaque':code==='filename_not_supplied'?'not_supplied':'article_unavailable',info={kind,code,message:String(r?.error||'Filename classification unavailable'),rawName:String(r?.raw_yenc_name||'')};state.nameResolutionClassifications.set(key,info);const a=articlesByKey.get(clientKey);if(a){a.name_resolution_state=kind;if(info.rawName)a.raw_yenc_name=info.rawName}if(kind==='opaque')opaque++;else if(kind==='not_supplied')noFilename++;else articleUnavailable++;continue}
    state.nameResolutionClassifications.delete(key);state.nameResolutionFailures.set(key,String(r?.error||'Filename unavailable from article metadata'));permanent++;
  }
  if(retryable){state.nameResolutionBackoffUntil=Math.max(state.nameResolutionBackoffUntil,now+Math.max(2,maxBackoff)*1000)}
  return{retryable,permanent,opaque,noFilename,articleUnavailable,maxBackoff};
}
function nameResolutionCandidates({manual=false,limit=8,unresolvedOnly=false}={}){
  const out=[],now=Date.now();for(const a of state.articles){const key=nameResolutionKey(a),deferred=state.nameResolutionDeferred.get(key);if(deferred&&Number(deferred.until||0)>now)continue;if(deferred)state.nameResolutionDeferred.delete(key);if(state.nameResolutionAttempted.has(key)||!articleNeedsNameResolution(a,{manual}))continue;if(unresolvedOnly&&!isUnresolvedDownloadCandidate(a))continue;out.push(a);if(out.length>=limit)break}return out;
}
function automaticNameResolutionCandidates(limit=8){
  const unresolved=nameResolutionCandidates({limit,unresolvedOnly:true});if(unresolved.length)return{items:unresolved,budgeted:false};
  if(state.nameResolutionAutoRemaining<=0)return{items:[],budgeted:false};const items=nameResolutionCandidates({limit:Math.min(limit,state.nameResolutionAutoRemaining)});return{items,budgeted:items.length>0};
}
...
function applyNameResolutionResults(results){
  recordNameResolutionFailures(results);const byKey=new Map((results||[]).filter(r=>r?.resolved&&r.media?.filename).map(r=>[String(r.client_key||''),r]));let changed=0;if(byKey.size){for(const a of state.articles){const r=byKey.get(articleKey(a));if(r&&applyNameResolutionResultToArticle(a,r))changed++}state.articles=coalesceResolvedMultipartArticles(state.articles);sortArticles(false)}return changed;
}
function scheduleObfuscatedNameResolution(){
  clearTimeout(state.nameResolutionTimer);state.nameResolutionTimer=null;if(!state.selectedGroup||state.searchMode||!isAllPostsMode()||state.nameResolutionInFlight||state.smartBinaryPending)return;const now=Date.now(),delay=Math.max(420,state.nameResolutionBackoffUntil>now?state.nameResolutionBackoffUntil-now:0);const next=automaticNameResolutionCandidates(8);if(!next.items.length){if(state.nameResolutionDeferred.size&&state.nameResolutionBackoffUntil>now)state.nameResolutionTimer=setTimeout(()=>scheduleObfuscatedNameResolution(),delay+50);return}state.nameResolutionTimer=setTimeout(()=>resolveObfuscatedNames({manual:false}),delay);
}
function nameResolutionPayload(a){return{client_key:articleKey(a),article:a.article,message_id:a.message_id,subject:a.subject,from:a.from,date:a.date,bytes:a.bytes,multipart:a.multipart,raw_yenc_name:a.raw_yenc_name||'',segments:segmentPayload(a)}}
function updateNameResolutionActivityDomInPlace(){
  const btn=els.articlesList?.querySelector('.binary-resolve-btn');if(btn){btn.disabled=!!(state.nameResolutionInFlight||state.smartBinaryPending);if(state.smartBinaryPending)btn.textContent='Reconstructing packages…';else if(state.nameResolutionInFlight)btn.textContent='Resolving names…';else btn.textContent='Resolve more names'}
  els.articlesList?.querySelectorAll('.article-row[data-index]').forEach(row=>{const a=state.articles[Number(row.dataset.index)];if(!a)return;const top=row.querySelector('.article-top'),existing=[...row.querySelectorAll('.name-resolved-chip.resolving')].find(x=>String(x.textContent||'').trim()==='RESOLVING…'),unresolved=!a.media&&isUnresolvedDownloadCandidate(a),attempted=unresolved&&state.nameResolutionAttempted.has(nameResolutionKey(a)),failure=unresolved?state.nameResolutionFailures.get(nameResolutionKey(a)):'',classification=unresolved?nameResolutionClassification(a):null,deferred=unresolved?state.nameResolutionDeferred.get(nameResolutionKey(a)):null,shouldResolve=!!(attempted&&state.nameResolutionInFlight&&!failure&&!classification&&!deferred);if(shouldResolve&&!existing&&top){const chip=document.createElement('span');chip.className='name-resolved-chip resolving';chip.textContent='RESOLVING…';top.appendChild(chip)}else if(!shouldResolve&&existing)existing.remove()});
}
function clearNameResolutionResultRenderQueue(){if(state.nameResolutionResultRenderTimer){clearTimeout(state.nameResolutionResultRenderTimer);state.nameResolutionResultRenderTimer=null}state.nameResolutionResultRenderDirty=false;state.nameResolutionResultRenderBatches=0;state.nameResolutionResultRenderGroup='';state.nameResolutionResultRenderProvider='';state.nameResolutionResultRenderFirstAt=0;state.nameResolutionResultRenderManual=false}
function nameResolutionResultRenderPolicy(){const manual=!!state.nameResolutionResultRenderManual;return{target:manual?3:2,softMs:manual?NAME_RESOLUTION_RENDER_MANUAL_SOFT_MS:NAME_RESOLUTION_RENDER_AUTO_SOFT_MS,maxMs:manual?NAME_RESOLUTION_RENDER_MANUAL_MAX_MS:NAME_RESOLUTION_RENDER_AUTO_MAX_MS}}
function flushNameResolutionResultRender(){if(state.nameResolutionResultRenderTimer){clearTimeout(state.nameResolutionResultRenderTimer);state.nameResolutionResultRenderTimer=null}if(!state.nameResolutionResultRenderDirty)return false;const group=state.nameResolutionResultRenderGroup,provider=state.nameResolutionResultRenderProvider,batches=Math.max(1,Number(state.nameResolutionResultRenderBatches||1)),firstAt=Number(state.nameResolutionResultRenderFirstAt||Date.now()),waitMs=Math.max(0,Date.now()-firstAt);state.nameResolutionResultRenderDirty=false;state.nameResolutionResultRenderBatches=0;state.nameResolutionResultRenderGroup='';state.nameResolutionResultRenderProvider='';state.nameResolutionResultRenderFirstAt=0;state.nameResolutionResultRenderManual=false;if(group!==state.selectedGroup||provider!==state.providerId||state.searchMode||!isAllPostsMode())return false;renderArticles({preserveScroll:true,reason:'name-resolution-batched-result'});perfRecord('name_resolution_batch',batches,true,{reason:batches>1?'coalesced':'single'});perfRecord('name_resolution_render_wait',waitMs,true,{reason:batches>1?'coalesced':'single'});return true}
function scheduleNameResolutionResultRenderFlush(){if(!state.nameResolutionResultRenderDirty)return false;const policy=nameResolutionResultRenderPolicy(),target=policy.target,age=Math.max(0,Date.now()-Number(state.nameResolutionResultRenderFirstAt||Date.now()));if(state.nameResolutionResultRenderBatches>=target||age>=policy.maxMs)return flushNameResolutionResultRender();if(state.nameResolutionResultRenderTimer)clearTimeout(state.nameResolutionResultRenderTimer);const delay=age<policy.softMs?policy.softMs-age:Math.min(250,policy.maxMs-age);state.nameResolutionResultRenderTimer=setTimeout(()=>{state.nameResolutionResultRenderTimer=null;scheduleNameResolutionResultRenderFlush()},Math.max(50,Math.ceil(delay)));return false}
function queueNameResolutionResultRender({manual=false}={}){const group=state.selectedGroup,provider=state.providerId;if(!group)return false;if((state.nameResolutionResultRenderGroup&&state.nameResolutionResultRenderGroup!==group)||(state.nameResolutionResultRenderProvider&&state.nameResolutionResultRenderProvider!==provider))clearNameResolutionResultRenderQueue();if(!state.nameResolutionResultRenderDirty){state.nameResolutionResultRenderFirstAt=Date.now();state.nameResolutionResultRenderManual=!!manual}else if(manual)state.nameResolutionResultRenderManual=true;state.nameResolutionResultRenderGroup=group;state.nameResolutionResultRenderProvider=provider;state.nameResolutionResultRenderDirty=true;state.nameResolutionResultRenderBatches=Math.max(0,Number(state.nameResolutionResultRenderBatches||0))+1;return scheduleNameResolutionResultRenderFlush()}
async function resolveObfuscatedNames({manual=false}={}){
  ...
  if(manual){
    ...
    let requested=0,resolved=0,changed=0,failedBatches=0,transientRounds=0;state.nameResolutionInFlight=true;updateNameResolutionActivityDomInPlace();
    try{
      while(group===state.selectedGroup&&providerId===state.providerId){
        ...
        const candidates=nameResolutionCandidates({manual:true,limit:12});if(!candidates.length){if(state.nameResolutionDeferred.size&&transientRounds<4){transientRounds++;await new Promise(r=>setTimeout(r,600));continue}break}for(const a of candidates)state.nameResolutionAttempted.add(nameResolutionKey(a));requested+=candidates.length;updateNameResolutionActivityDomInPlace();
        try{
          const data=await api('/api/articles/resolve-names',{provider_id:providerId,group,items:candidates.map(nameResolutionPayload)},browseRequestOptions({timeoutMs:75000,timeoutMessage:'Name resolution took too long. NewzDeck will retry this batch instead of marking the remaining files unavailable.'}));if(group!==state.selectedGroup||providerId!==state.providerId)return;resolved+=Number(data.resolved||0);const results=data.results||[];changed+=applyNameResolutionResults(results);if(results.length)queueNameResolutionResultRender({manual:true});else updateNameResolutionActivityDomInPlace();const transient=Number(data.retryable||0);if(transient){transientRounds++;const seconds=Math.max(2,Number(data.backoff_seconds||5));state.nameResolutionBackoffUntil=Math.max(state.nameResolutionBackoffUntil,Date.now()+seconds*1000);updateNameResolutionActivityDomInPlace();if(transientRounds>=4){toast('The provider is temporarily refusing or timing out on filename probes. NewzDeck paused instead of marking later files unavailable; it will retry automatically.');break}await new Promise(r=>setTimeout(r,seconds*1000+75));continue}transientRounds=0;
        }catch(e){failedBatches++;for(const a of candidates){const key=nameResolutionKey(a);state.nameResolutionAttempted.delete(key);state.nameResolutionDeferred.set(key,{message:String(e?.message||'Filename probe failed'),until:Date.now()+5000,code:'request_error'})}state.nameResolutionBackoffUntil=Math.max(state.nameResolutionBackoffUntil,Date.now()+5000);transientRounds++;queueNameResolutionResultRender({manual:true});if(transientRounds>=4)break}
    ...
    }finally{if(group===state.selectedGroup&&providerId===state.providerId)flushNameResolutionResultRender();state.nameResolutionInFlight=false;if(group===state.selectedGroup){updateNameResolutionActivityDomInPlace();scheduleObfuscatedNameResolution()}}
    return;
  }
  const now=Date.now();if(state.nameResolutionBackoffUntil>now){scheduleObfuscatedNameResolution();return}
  const next=automaticNameResolutionCandidates(8),candidates=next.items;if(!candidates.length)return;for(const a of candidates)state.nameResolutionAttempted.add(nameResolutionKey(a));if(next.budgeted)state.nameResolutionAutoRemaining=Math.max(0,state.nameResolutionAutoRemaining-candidates.length);state.nameResolutionInFlight=true;updateNameResolutionActivityDomInPlace();try{const data=await api('/api/articles/resolve-names',{provider_id:providerId,group,items:candidates.map(nameResolutionPayload)},browseRequestOptions({timeoutMs:75000,timeoutMessage:'Name resolution took too long. NewzDeck will retry this batch.'}));if(group!==state.selectedGroup||providerId!==state.providerId)return;const results=data.results||[];applyNameResolutionResults(results);if(results.length)queueNameResolutionResultRender();else updateNameResolutionActivityDomInPlace();if(Number(data.retryable||0)){const seconds=Math.max(2,Number(data.backoff_seconds||5));state.nameResolutionBackoffUntil=Math.max(state.nameResolutionBackoffUntil,Date.now()+seconds*1000)}}catch(e){for(const a of candidates){const key=nameResolutionKey(a);state.nameResolutionAttempted.delete(key);state.nameResolutionDeferred.set(key,{message:String(e?.message||'Filename probe failed'),until:Date.now()+5000,code:'request_error'})}state.nameResolutionBackoffUntil=Math.max(state.nameResolutionBackoffUntil,Date.now()+5000);queueNameResolutionResultRender()}finally{state.nameResolutionInFlight=false;if(group===state.selectedGroup){updateNameResolutionActivityDomInPlace();scheduleObfuscatedNameResolution()}}
}
...
function articleRowMarkup(a,index,{member=false}={}){
  const media=a.media,key=articleKey(a),selected=state.selectedItems.has(key),dkey=itemDownloadKey(a),downloaded=!!media&&state.downloadedIndex.has(dkey),queued=!!media&&state.queuedIndex.has(dkey),binaryMode=isAllPostsMode(),fragment=isUnresolvedMultipartFragment(a),queueable=!fragment&&isSelectableMedia(a),unresolved=!media&&isUnresolvedDownloadCandidate(a),attempted=unresolved&&state.nameResolutionAttempted.has(nameResolutionKey(a));
  ...
  const filename=String(media?.filename||'').trim(),opaqueTitle=opaque?(a.complete?'Obfuscated multipart binary':'Incomplete multipart binary'):'',primary=binaryMode&&filename?filename:(opaqueTitle||a.subject||filename||'Post'),secondary=binaryMode&&String(a.subject||'').trim()&&String(a.subject||'').trim()!==primary?`<small title="${escapeHtml(a.subject||'')}">${escapeHtml(a.subject||'')}</small>`:'',failure=unresolved?state.nameResolutionFailures.get(nameResolutionKey(a)):'',classification=unresolved?nameResolutionClassification(a):null,deferred=unresolved?state.nameResolutionDeferred.get(nameResolutionKey(a)):null,resolving=!!(unresolved&&attempted&&state.nameResolutionInFlight&&!failure&&!classification&&!deferred),classificationChip=classification?.kind==='opaque'?`<span class="name-resolved-chip opaque" title="${escapeHtml(classification.rawName?`yEnc supplied the opaque/extensionless source name: ${classification.rawName}`:classification.message)}">OBFUSCATED NAME</span>`:classification?.kind==='not_supplied'?`<span class="name-resolved-chip unresolved" title="${escapeHtml(classification.message)}">NO FILENAME</span>`:classification?.kind==='article_unavailable'?`<span class="name-resolved-chip unresolved" title="${escapeHtml(classification.message)}">ARTICLE UNAVAILABLE</span>`:'',resolved=a.name_resolution?`<span class="name-resolved-chip" title="Recovered from ${escapeHtml(a.name_resolution.title_source||a.name_resolution.metadata_source||a.name_resolution.archive_source||a.name_resolution.source||'yEnc header')}">✓ Resolved</span>`:classificationChip?classificationChip:failure?`<span class="name-resolved-chip unresolved" title="${escapeHtml(failure)}">NAME UNAVAILABLE</span>`:deferred?`<span class="name-resolved-chip resolving" title="${escapeHtml(deferred.message||'Provider temporarily unavailable')}">RETRYING…</span>`:resolving?'<span class="name-resolved-chip resolving">RESOLVING…</span>':'';
  const action=binaryMode&&!member?(queueable?`<button class="article-row-download mini-btn" type="button" data-index="${index}" title="${unresolved?(classification?.kind==='opaque'?'Download this binary; NewzDeck will preserve the opaque yEnc token and infer the file type after transfer':'NewzDeck will try the yEnc filename first and fall back safely if necessary'):'Add this binary to the download queue'}">＋ ${unresolved&&(failure||classification?.kind==='not_supplied'||classification?.kind==='article_unavailable')?'Download anyway':'Download'}</button>`:(fragment?'<span class="binary-row-incomplete">Unresolved fragment</span>':!a.complete?'<span class="binary-row-incomplete">Incomplete</span>':'')):'';
...
function binaryDisplayUnitInfo(unit){
  ...
  const a=unit.item.a,fragment=isUnresolvedMultipartFragment(a),queueReady=!fragment&&!!(a?.media&&a.complete),unresolved=!fragment&&isUnresolvedDownloadCandidate(a),failure=unresolved?state.nameResolutionFailures.get(nameResolutionKey(a)):'',classification=unresolved?nameResolutionClassification(a):null,deferred=unresolved?state.nameResolutionDeferred.get(nameResolutionKey(a)):null,pending=!!(unresolved&&!failure&&!classification),nameUnavailable=!!(unresolved&&(failure||classification)),opaqueName=classification?.kind==='opaque',noFilename=classification?.kind==='not_supplied',articleUnavailable=classification?.kind==='article_unavailable',actionable=queueReady||unresolved;
  return{queueReady,actionable,pending,nameUnavailable,opaqueName,noFilename,articleUnavailable,incomplete:fragment||!actionable,fragment,date:Date.parse(a.date||'')||Number(a.article||0),size:Number(a.bytes||0),name:String(a.media?.filename||a.raw_yenc_name||a.subject||''),files:1,health:queueReady?0:pending?1:opaqueName?1:nameUnavailable?2:3};
}
...
function binaryDisplayUnitPassesStatus(unit){const info=binaryDisplayUnitInfo(unit),mode=state.binaryPackageFilter||'downloadable';if(mode==='all')return true;if(mode==='incomplete')return info.incomplete;return info.actionable??info.queueReady}
...
function renderList(items){
  ...
  if(isAllPostsMode()&&state.groupBinarySets){
    ...
    const sorted=sortBinaryDisplayUnits(units),visible=sorted.filter(binaryDisplayUnitVisible),queueable=units.filter(u=>binaryDisplayUnitInfo(u).queueReady).length,actionable=units.filter(u=>{const i=binaryDisplayUnitInfo(u);return i.actionable??i.queueReady}).length,pending=units.filter(u=>binaryDisplayUnitInfo(u).pending).length,opaqueNames=units.filter(u=>binaryDisplayUnitInfo(u).opaqueName).length,noFilenames=units.filter(u=>binaryDisplayUnitInfo(u).noFilename).length,articleUnavailable=units.filter(u=>binaryDisplayUnitInfo(u).articleUnavailable).length,nameUnavailable=units.filter(u=>{const i=binaryDisplayUnitInfo(u);return i.nameUnavailable&&!i.opaqueName&&!i.noFilename&&!i.articleUnavailable}).length,incomplete=units.filter(u=>binaryDisplayUnitInfo(u).incomplete).length,fragments=units.filter(u=>binaryDisplayUnitInfo(u).fragment).length,statusVisible=units.filter(binaryDisplayUnitPassesStatus),statusHidden=units.length-statusVisible.length,sizeHidden=statusVisible.filter(u=>binaryDisplayUnitInfo(u).size<binaryMinimumBytes()).length,obfuscated=nameResolutionCandidates({manual:true,limit:Number.MAX_SAFE_INTEGER}).length;
    const resolver=`<button class="binary-resolve-btn mini-btn" type="button" ${state.nameResolutionInFlight||state.smartBinaryPending?'disabled':''}>${state.smartBinaryPending?'Reconstructing packages…':state.nameResolutionInFlight?'Resolving names…':obfuscated?`Resolve names (${obfuscated.toLocaleString()})`:'Resolve more names'}</button>`,scan=state.smartBinaryHeaders?` • ${state.smartBinaryHeaders.toLocaleString()} raw headers scanned`:'';
    body=`<div class="binary-list-summary"><div class="binary-summary-copy"><strong>${actionable.toLocaleString()} downloadable item${actionable===1?'':'s'}</strong><span>${queueable.toLocaleString()} named/ready • ${pending.toLocaleString()} resolving names${opaqueNames?` • ${opaqueNames.toLocaleString()} obfuscated yEnc name${opaqueNames===1?'':'s'}`:''}${noFilenames?` • ${noFilenames.toLocaleString()} no filename supplied`:''}${articleUnavailable?` • ${articleUnavailable.toLocaleString()} article unavailable`:''}${nameUnavailable?` • ${nameUnavailable.toLocaleString()} other filename unavailable`:''} • ${built.groups.length.toLocaleString()} release package${built.groups.length===1?'':'s'} • ${incomplete.toLocaleString()} incomplete${fragments?` • ${fragments.toLocaleString()} unresolved segment fragment${fragments===1?'':'s'}`:''}${statusHidden?` • ${statusHidden.toLocaleString()} hidden by Show`:''}${sizeHidden?` • ${sizeHidden.toLocaleString()} below ${escapeHtml(formatBytes(binaryMinimumBytes()))}`:''}${scan}</span></div>${binaryBrowserControlsMarkup()}${resolver}</div>`+(visible.length?visible.map(u=>u.type==='group'?binarySetRowMarkup(u.group):articleRowMarkup(u.item.a,u.item.index)).join(''):`<div class="binary-filter-empty"><strong>No ${state.binaryPackageFilter==='incomplete'?'incomplete':'downloadable'} binaries in this scanned range.</strong><span>${incomplete&&state.binaryPackageFilter==='downloadable'?`${incomplete.toLocaleString()} incomplete item${incomplete===1?' is':'s are'} hidden while NewzDeck continues reconstruction. Change Show to All or Incomplete only when you want diagnostics.`:'Load older headers or change the filter.'}</span></div>`);
...
function thumbnailTaskKey(index,pkey,role='item',generation=state.galleryGeneration,setKey=''){if(role==='set-cover'&&setKey)return relatedCoverTaskKey(setKey);return `${generation}|${role}|${index}|${pkey}`}
...
function thumbnailHolder(index,role='item'){
  ...
  state.thumbHolderRegistryStats.fallbacks++;const safeRole=String(role).replace(/"/g,'\\"'),selector=`[data-thumb-index="${index}"][data-thumb-role="${safeRole}"],img.thumb-img[data-thumb-image-index="${index}"][data-thumb-role="${safeRole}"]`;const node=els.articlesList.querySelector(selector)||els.previewContent?.querySelector?.(selector);return node?registerThumbnailHolderNode(node,index,role):null
}
...
function ensureThumbnailWatchdog(){
  ...
  thumbWatchdogTimer=setInterval(()=>{if(state.activeView==='browse'&&state.selectedGroup){scanThumbnailDemand();scanRelatedSetCoverDemand()}},1000);
}
function scanRelatedSetCoverDemand(){activateRelatedSetCovers({margin:0})}
...
function scanThumbnailDemand(){
  ...
  for(const holder of holders){
    ...
    if(visible&&a.media.kind==='image'&&now-born>=6000&&thumbnailFullRecoveryIdle(now))escalateVisibleImageThumbnail(index,role);
...
function thumbnailFullRecoveryIdle(now=Date.now()){const since=now-Number(state.lastBrowseScrollTs||0),velocity=Math.abs(Number(state.browseScrollVelocity||0)),foreground=state.thumbQueue.filter(t=>t.role!=='set-cover'&&Number(t.priority??1)===0).length;return since>=900&&velocity<.05&&state.thumbActive===0&&foreground===0&&!activeDownloadTraffic()}
function escalateVisibleImageThumbnail(index,role='item'){
  const a=state.articles[index];if(!a?.media||a.media.kind!=='image'||!a.complete||state.thumbEscalationActive>=1||!thumbnailFullRecoveryIdle())return;
...
function queueThumbnail(index,priority=1,distance=0,role='item',options=null){
  const a=state.articles[index];if(!a||!a.media||!a.complete||!['image','video'].includes(a.media.kind))return false;
  const holder=thumbnailHolder(index,role),setKey=role==='set-cover'?String(options?.setKey||holder?.closest?.('.media-set-card')?.dataset?.setKey||''):'';const pkey=previewKey(a),generation=state.galleryGeneration,browseSession=state.browseSessionToken,qkey=thumbnailTaskKey(index,pkey,role,generation,setKey);
  ...
  if(cached?.url){if(role==='set-cover')updateMediaSetCoverDom({index,setKey,browseSession,sourceArticleKey:articleKey(a)},index,cached,articleKey(a));else updateThumbnailDom(index,cached,role);return false}
  if(role==='set-cover'){
...
function pumpThumbQueue(){
  while(state.thumbActive<state.thumbConcurrency&&state.thumbQueue.length){
    ...
    // visible covers are waiting, so queue admission is bounded without increasing
    // provider/preview concurrency.
    const reserveTarget=relatedCoverReserveTarget(),nonCoverActive=Math.max(0,state.thumbActive-state.thumbSetActive),nonCoverLimit=Math.max(0,state.thumbConcurrency-reserveTarget);
    let pos=-1,best=Number.POSITIVE_INFINITY,reservedCoverPos=-1,reservedCoverAge=-1;
    for(let i=0;i<state.thumbQueue.length;i++){
      const t=state.thumbQueue[i],age=Date.now()-Number(t.queuedAt||Date.now());
      if(t.kind==='video'&&state.thumbVideoActive>=state.videoThumbConcurrency)continue;
      if(t.role==='set-cover'){
        const visible=Number(t.priority??1)===0;
        if(state.thumbSetActive>=state.setCoverConcurrency)continue;
        if(visible&&state.thumbSetActive<reserveTarget){if(age>reservedCoverAge){reservedCoverAge=age;reservedCoverPos=i}continue}
        if(shouldThrottleRelatedSetCovers()&&!visible){scheduleRelatedSetCoverResume();continue}
      }else if(reserveTarget>0&&nonCoverActive>=nonCoverLimit){
        // Preserve the reserved capacity. The next completed main-gallery task will
        // be replaced by a visible cover instead of another gallery task.
        continue;
      }
      const score=liveThumbnailTaskScore(t);if(score<best){best=score;pos=i}
    }
    ...
    (async()=>{
      ...
      }catch(e){const sameContext=task.group===state.selectedGroup&&task.provider===state.providerId,live=task.role==='set-cover'?(sameContext&&task.browseSession===state.browseSessionToken):(task.generation===state.galleryGeneration&&sameContext),info=friendlyPreviewError(e);if(live&&info.code!=='browse_cancelled')markThumbnailError(task.index,e,task.kind,task.role,task.setKey);else if(task.role==='set-cover'&&!live)state.relatedCoverStats.staleSessionDrops++;}
      finally{const runMs=performance.now()-sampleStarted;if(task.role==='set-cover'){const s=state.relatedCoverStats;if(sampleOK)s.completed++;else s.failed++;s.totalRunMs+=runMs;s.maxRunMs=Math.max(Number(s.maxRunMs||0),runMs)}state.thumbActive--;state.thumbActiveTasks.delete(task.qkey);if(task.kind==='video')state.thumbVideoActive--;if(task.role==='set-cover')state.thumbSetActive--;state.thumbQueued.delete(task.qkey||task.pkey);recordPreviewSample(runMs,sampleOK);pumpThumbQueue();scheduleThumbnailDemandScan();}
    })();
  }
}
async function fetchPrepared(a){
  ...
  const started=performance.now();const request=api('/api/preview/prepare',browsePayload({provider_id:state.providerId,group:articleGroup(a),segments:segmentPayload(a),media:a.media}),browseRequestOptions())
    .then(data=>{boundedCacheSet(state.previewCache,key,data,browseCacheLimits().preview);perfRecord('preview',performance.now()-started,true);return data})
    .catch(e=>{perfRecord('preview',performance.now()-started,false);throw e})
...
function videoThumbnailError(code,message){const e=new Error(message);e.code=code;return e}
function videoThumbnailPostFailureReason(e){return thumbnailErrorReason(e)}
...
async function finishImageThumbnailResponse(a,data){
  ...
  const result={...data,url};boundedCacheSet(state.imageThumbCache,key,result,browseCacheLimits().thumb);state.unpreviewableMediaKeys.delete(key);scheduleRelatedSetCoverActivation();return result;
}
async function fetchImageThumbnail(a,task=null){
  ...
  const payload=browsePayload({provider_id:state.providerId,group:articleGroup(a),segments:segmentPayload(a),media:a.media,thumbnail_lanes:thumbnailLaneHint(a,task),content_filter:browserPerfMode()});const started=performance.now();let thumbnailOK=false;
  const request=(async()=>{
    let thumbnailError=null;
    try{return await finishImageThumbnailResponseTimed(a,await thumbnailImageApiTimed(payload,browseRequestOptions(task?.requestTimeoutMs?{timeoutMs:Number(task.requestTimeoutMs),timeoutMessage:'Related Media cover timed out. You can retry this cover.'}:{}),'primary',task))}
    catch(e){thumbnailError=e;if(thumbnailFailureSkipsFullFallback(e))throw e;if(['set-cover','set-probe'].includes(task?.role||''))throw e;if((task?.role||'item')==='item'&&!thumbnailFullRecoveryIdle()){e.deferFullPreview=true;throw e}}
    const recoveryStarted=performance.now();let recoveryOK=false,recoveryReason='full-preview';
    try{
      const full=await fetchPrepared(a);
      if(full?.url){
        try{const result=await finishImageThumbnailResponseTimed(a,await thumbnailImageApiTimed(payload,browseRequestOptions(),'retry-after-full-preview',task));recoveryOK=true;recoveryReason='thumbnail-retry';return result}
        catch(e){recoveryReason=`full-preview-${thumbnailErrorReason(e)}`;const result={...full,url:full.url,kind:'image',method:'full-preview-fallback',thumbnail_fallback:true};boundedCacheSet(state.imageThumbCache,key,result,browseCacheLimits().thumb);state.unpreviewableMediaKeys.delete(key);recoveryOK=true;return result}
      }
      recoveryReason='full-preview-empty';
    }catch(e){recoveryReason=thumbnailErrorReason(e)}
    finally{perfRecord('thumbnail_recovery',performance.now()-recoveryStarted,recoveryOK,{reason:recoveryReason})}
    throw thumbnailError||new Error('Could not load an image preview for this post.');
  })().then(result=>{thumbnailOK=true;return result}).finally(()=>{perfRecord('thumbnail',performance.now()-started,thumbnailOK);promiseMap.delete(promiseKey)});
  ...
  const request=(async()=>{
    ...
    try{
      ...
      if(!url&&data.sample_url&&data.browser_supported){
        ...
        if(data.thumbnail_token){let stored;try{stored=await persistThumbnail(data.thumbnail_token,captured.dataUrl)}catch(e){throw videoThumbnailError(`thumb-store-${thumbnailErrorReason(e)}`,String(e?.message||'Could not persist the video thumbnail.'))}url=stored.thumbnail_url||captured.dataUrl}else url=captured.dataUrl;data.width=captured.width;data.height=captured.height;data.duration=captured.duration;postReason=`${demand}-${sampleClass}-browser-frame`;
      }
      if(!url)throw videoThumbnailError(data.browser_supported?'frame-unavailable':'ffmpeg-required',data.browser_supported?'Could not decode a frame from the video sample.':'This video format needs FFmpeg for automatic thumbnails.');
    ...
    }catch(e){const failureReason=videoThumbnailPostFailureReason(e);postReason=`${demand}-${sampleClass}-${failureReason}`;if(['browser-decode-failed','ffmpeg-required'].includes(failureReason)){const policyCode=failureReason==='browser-decode-failed'?'browser-decode':failureReason;perfRecord('video_thumbnail_policy',0,true,{reason:`${demand}-${sampleClass}-${policyCode}-nr`})}throw e}
...
function captureVideoFrame(url){
  return new Promise((resolve,reject)=>{
    ...
    const capture=()=>{try{const w=video.videoWidth,h=video.videoHeight;if(!w||!h)return finish(videoThumbnailError('sample-no-frame','The video sample did not expose a frame.'));const maxW=480,scale=Math.min(1,maxW/w),cw=Math.max(1,Math.round(w*scale)),ch=Math.max(1,Math.round(h*scale));const canvas=document.createElement('canvas');canvas.width=cw;canvas.height=ch;const ctx=canvas.getContext('2d');ctx.drawImage(video,0,0,cw,ch);finish(null,{dataUrl:canvas.toDataURL('image/jpeg',.78),width:w,height:h,duration:Number.isFinite(Number(video.duration))?Number(video.duration):0});}catch(e){finish(videoThumbnailError('frame-capture-failed',String(e?.message||'The browser could not capture a video frame.')))}};
    const timer=setTimeout(()=>finish(videoThumbnailError('browser-frame-timeout','Video thumbnail timed out.')),12000);
    video.addEventListener('error',()=>finish(videoThumbnailError('browser-decode-failed','The browser could not decode this video sample.')),{once:true});
...
async function recoverBrokenThumbnail(img){
  ...
  if(role==='set-cover'){const card=img.closest('.media-set-card'),setKey=String(img.dataset.thumbSetKey||card?.dataset?.setKey||''),group=mediaSetGroupByKey(setKey),rep=group?mediaSetRepresentative(group):null;if(!card||!rep?.a)return;const loader=document.createElement('div');loader.className=`thumb-loader ${rep.a.media?.kind==='video'?'video-thumb-loader':''}`;loader.dataset.thumbIndex=String(rep.index);loader.dataset.thumbRole='set-cover';loader.dataset.thumbSetKey=setKey;loader.innerHTML='<span></span><small>Recovering cover…</small>';img.replaceWith(loader);registerThumbnailHolderNode(loader,rep.index,'set-cover');card.dataset.setCoverIndex=String(rep.index);card.dataset.setCoverState='pending';queueThumbnail(rep.index,0,0,'set-cover',{setKey});return}
...
function updateMediaSetCoverDom(task,sourceIndex,data,sourceArticleKey=''){
  const setKey=String(task?.setKey||''),card=relatedSetCardByKey(setKey),stableKey=String(sourceArticleKey||task?.sourceArticleKey||'');let liveIndex=Number(sourceIndex),a=state.articles[liveIndex];if(stableKey&&(!a||articleKey(a)!==stableKey)){const found=state.articles.findIndex(x=>articleKey(x)===stableKey);if(found>=0){liveIndex=found;a=state.articles[found]}}if(!a||!data?.url)return false;if(card?.querySelector('.media-set-cover .thumb-img'))return true;let holder=card?.querySelector('.media-set-cover .thumb-loader,.media-set-cover .thumb-error')||thumbnailHolder(Number(task?.index),'set-cover');if(!holder){state.relatedCoverStats.domTargetMisses++;scheduleRelatedSetCoverActivation();return false}
  const oldIndex=Number(holder.dataset.thumbIndex);a.media_meta={...(a.media_meta||{}),width:Number(data.width||0),height:Number(data.height||0),duration:Number(data.duration||0)};if(Number.isFinite(oldIndex))unregisterThumbnailHolder(oldIndex,'set-cover');const img=replaceThumbnailHolderWithImage(holder,liveIndex,data.url,'set-cover',{isVideo:a.media?.kind==='video'});if(img){img.dataset.thumbSetKey=setKey;if(card){card.dataset.setCoverIndex=String(liveIndex);card.dataset.setCoverState='loaded'}}return !!img;
}
...
function friendlyPreviewError(error){
  const data=error?.data||{},message=data.error||error?.message||String(error||'Preview unavailable');
  const rawCode=String(data.error_code||error?.code||'preview_failed').trim().toLocaleLowerCase(),code=rawCode.replace(/[^a-z0-9_-]+/g,'-').replace(/^-+|-+$/g,'').slice(0,48)||'preview_failed';
  const clientPermanent=['browser-decode-failed','ffmpeg-required'].includes(code),label=data.error_label||(code==='browser-decode-failed'?'Browser codec unavailable':code==='ffmpeg-required'?'Video decoder unavailable':'Preview unavailable');
  const retryable=typeof data.retryable==='boolean'?data.retryable:!clientPermanent;
  return {message,label,code,retryable};
}
...
function isDefinitiveUnsupportedMedia(info){return info?.retryable===false&&['decode_failed','browser-decode-failed','ffmpeg-required'].includes(info.code)}
function rememberPreviewUnavailable(a,info){
  if(!a)return false;
  const key=previewKey(a);state.unpreviewableMediaKeys.add(key);if(state.groupRelatedMedia)state.mediaSetIndexValid=false;
...
function hideUnavailableMediaDom(a,index,role='item'){
  if(!a||els.contentFilter.value==='all')return false;
  if(role==='set-cover'){
    ...
    if(state.mediaSetBrokenKeys.has(setKey)){card.remove();return true}
    ...
    if(rep){const cover=card.querySelector('.media-set-cover');cover?.querySelectorAll('.thumb-img,.thumb-loader,.thumb-error,.video-play-overlay').forEach(n=>n.remove());if(cover){const k=rep.a.media?.kind||'image';cover.insertAdjacentHTML('afterbegin',`<div class="thumb-loader ${k==='video'?'video-thumb-loader':''}" data-thumb-index="${rep.index}" data-thumb-role="set-cover"><span></span><small>Loading set cover…</small></div>`);registerThumbnailHolderNode(cover.querySelector('[data-thumb-index]'),rep.index,'set-cover');queueThumbnail(rep.index,0,0,'set-cover',{setKey})}}else card.remove();
    return true;
  }
  const card=els.articlesList.querySelector(`.media-card[data-index="${index}"]`);if(!card)return false;card.remove();return true;
}
function hideUnavailableMediaInPlace(a,index,role='item'){
  if(!a||els.contentFilter.value==='all')return false;const keep=els.articlesList.scrollTop,anchors=captureBrowseAnchors();if(!hideUnavailableMediaDom(a,index,role))return false;restoreAfterGalleryMutation(anchors,keep);return true;
}
function flushUnavailableMediaHides(){
  state.unavailableHideTimer=null;if(!state.unavailableHideQueue.size||els.contentFilter.value==='all'){state.unavailableHideQueue.clear();return}
  const batch=[...state.unavailableHideQueue.values()];state.unavailableHideQueue.clear();const generation=state.galleryGeneration,group=state.selectedGroup,provider=state.providerId;const live=batch.filter(x=>x.generation===generation&&x.group===group&&x.provider===provider);if(!live.length)return;
  const keep=els.articlesList.scrollTop,anchors=captureBrowseAnchors();let mainChanged=false,setChanged=false;for(const x of live){const changed=hideUnavailableMediaDom(x.a,x.index,x.role);if(changed){if(x.role==='set-cover')setChanged=true;else mainChanged=true}}
  if(mainChanged)restoreAfterGalleryMutation(anchors,keep);if(setChanged)renderRelatedMediaPane(null,{incremental:true});scheduleThumbnailDemandScan()
}
function queueUnavailableMediaHide(a,index,role='item'){
  if(!a||els.contentFilter.value==='all')return false;const key=`${articleKey(a)}|${role}`;state.unavailableHideQueue.set(key,{a,index,role,generation:state.galleryGeneration,group:state.selectedGroup,provider:state.providerId});
  if(!state.unavailableHideTimer)state.unavailableHideTimer=setTimeout(flushUnavailableMediaHides,70);return true;
}
...
function scheduleDeferredFullPreviewRecovery(index,role='item'){
  const a=state.articles[index];if(!a?.media||a.media.kind!=='image')return;const timerKey=`${state.galleryGeneration}|${index}|${role}|${previewKey(a)}`;if(state.thumbDeferredRecoveryTimers.has(timerKey))return;const generation=state.galleryGeneration,group=state.selectedGroup,provider=state.providerId;
  const tick=()=>{if(generation!==state.galleryGeneration||group!==state.selectedGroup||provider!==state.providerId){state.thumbDeferredRecoveryTimers.delete(timerKey);return}const holder=thumbnailHolder(index,role);if(!holder?.isConnected){state.thumbDeferredRecoveryTimers.delete(timerKey);return}const root=els.articlesList.getBoundingClientRect(),r=holder.getBoundingClientRect(),visible=r.bottom>root.top&&r.top<root.bottom;if(!visible){state.thumbDeferredRecoveryTimers.delete(timerKey);return}if(!thumbnailFullRecoveryIdle()){state.thumbDeferredRecoveryTimers.set(timerKey,setTimeout(tick,450));return}state.thumbDeferredRecoveryTimers.delete(timerKey);escalateVisibleImageThumbnail(index,role)};
  state.thumbDeferredRecoveryTimers.set(timerKey,setTimeout(tick,950));
}
function markThumbnailError(index,error,kind='image',role='item',setKey=''){
  const info=friendlyPreviewError(error),a=state.articles[index];
  if(error?.deferFullPreview&&a&&role==='item'){const holder=thumbnailHolder(index,role);if(holder){holder.className='thumb-loader';holder.setAttribute('data-thumb-index',String(index));holder.dataset.thumbRole=role;registerThumbnailHolderNode(holder,index,role);holder.innerHTML='<span></span><small>Waiting for idle preview recovery…</small>'}scheduleDeferredFullPreviewRecovery(index,role);return}
  if(role==='set-cover'&&info.code==='media_set_unavailable'){const card=relatedSetCardByKey(setKey);if(setKey)state.mediaSetBrokenKeys.add(setKey);if(card)card.remove();renderRelatedMediaPane(null,{incremental:true});return}
  if(role==='set-cover'){
    if(a&&isDefinitiveUnsupportedMedia(info))rememberPreviewUnavailable(a,info);
    const card=relatedSetCardByKey(setKey),holder=card?.querySelector('.media-set-cover .thumb-loader,.media-set-cover .thumb-error')||thumbnailHolder(index,role);if(!holder)return;const oldIndex=Number(holder.dataset.thumbIndex);if(Number.isFinite(oldIndex))unregisterThumbnailHolder(oldIndex,role);holder.className='thumb-error';holder.removeAttribute('data-thumb-index');holder.dataset.thumbRole='set-cover';holder.dataset.thumbSetKey=setKey;if(card)card.dataset.setCoverState='failed';holder.innerHTML=`<span>!</span><small>Cover unavailable</small><em class="error-label">${escapeHtml(info.label||'Cover unavailable')}</em><em>${escapeHtml(String(info.message||'').slice(0,120))}</em>${info.retryable?'<button type="button" class="thumb-retry">Retry</button>':''}`;
    holder.querySelector('.thumb-retry')?.addEventListener('click',e=>{e.stopPropagation();const liveCard=relatedSetCardByKey(setKey)||holder.closest('.media-set-card'),group=mediaSetGroupByKey(setKey||liveCard?.dataset.setKey||''),rep=group?mediaSetRepresentative(group):null;if(!rep?.a)return;state.thumbFailureCache.delete(previewKey(rep.a));holder.className=`thumb-loader ${rep.a.media?.kind==='video'?'video-thumb-loader':''}`;holder.setAttribute('data-thumb-index',String(rep.index));holder.dataset.thumbRole='set-cover';holder.dataset.thumbSetKey=setKey;holder.innerHTML='<span></span><small>Retrying cover…</small>';registerThumbnailHolderNode(holder,rep.index,'set-cover');if(liveCard){liveCard.dataset.setCoverIndex=String(rep.index);liveCard.dataset.setCoverState='pending'}queueThumbnail(rep.index,0,0,'set-cover',{setKey})});return
  }
  if(a&&rememberPreviewUnavailable(a,info)&&els.contentFilter.value!=='all'){if(queueUnavailableMediaHide(a,index,role))return;renderArticles({preserveScroll:true});return}
...
function isSelectableMedia(a){return !!(a?.complete&&(a?.media||isUnresolvedDownloadCandidate(a)))}
...
function handleMediaSelectionClick(index,e){
  ...
  if(selectable){if(range){selectRangeTo(index,{add:additive})}else if(additive){if(state.selectedItems.has(key))state.selectedItems.delete(key);else state.selectedItems.set(key,a);state.selectionAnchorKey=key}else{state.selectedItems.clear();state.selectedItems.set(key,a);state.selectionAnchorKey=key}}
  state.selectedArticleKey=key;state.activeMediaSetKey='';updateSelectionBar();updateSelectionDomInPlace();updateActiveArticleDomInPlace();if(isAllPostsMode()){renderPreviewDetails(a,true);document.querySelector('.preview-pane')?.classList.add('open')}
}
...
function closeMediaViewer(){if(!els.mediaViewer)return;clearTimeout(state.viewerPreloadTimer);const ret=state.viewerReturnState;state.viewerOpen=false;state.viewerKey='';state.viewerSetOnly=false;state.viewerInfoOpen=false;state.viewerDrag=null;state.activeMediaSetKey='';els.mediaViewer.classList.add('hidden');els.viewerStage.innerHTML='';els.viewerInfo?.classList.add('hidden');state.viewerReturnState=null;if(!isAllPostsMode()&&state.groupRelatedMedia)renderRelatedMediaPane(null,{incremental:true});if(ret)requestAnimationFrame(()=>{restoreBrowsePosition(ret.anchors,ret.scrollTop);if(ret.key){state.selectedArticleKey=ret.key;updateActiveArticleDomInPlace();const idx=state.articles.findIndex(x=>articleKey(x)===ret.key),node=idx>=0?els.articlesList.querySelector(`[data-index="${idx}"]`):null;if(node){const list=els.articlesList.getBoundingClientRect(),r=node.getBoundingClientRect();if(r.top<list.top||r.bottom>list.bottom)node.scrollIntoView({block:'nearest'})}}})}
async function openMediaViewer(a){
  if(!a?.media||!['image','video'].includes(a.media.kind)||!a.complete)return;const setMatch=!!(state.groupRelatedMedia&&state.activeMediaSetKey&&mediaSetKey(a)===state.activeMediaSetKey);if(!setMatch)state.activeMediaSetKey='';state.viewerReturnState={scrollTop:Number(els.articlesList.scrollTop||0),anchors:captureBrowseAnchors(),key:articleKey(a)};state.viewerOpen=true;state.viewerKey=articleKey(a);state.selectedArticleKey=articleKey(a);state.viewerMode='fit';state.viewerFit=true;state.viewerZoom=1;state.viewerRotation=0;state.viewerSetOnly=setMatch;state.viewerInfoOpen=false;els.mediaViewer.classList.remove('hidden');await renderMediaViewer(a)
}
...
function resetPreview(){
  if(!isAllPostsMode()){renderRelatedMediaPane(null,{incremental:false});return}
  if(relatedSetObserver){relatedSetObserver.disconnect();relatedSetObserver=null}setBrowseSidePaneHeader('MEDIA','Preview','READY',false);els.previewContent.className='preview-content preview-empty';
  els.previewContent.innerHTML='<div class="preview-orb">☷</div><h3>Binary newsgroup browser</h3><p>All Posts uses a compact filename-first list. Connected RAR, ZIP, 7-Zip, PAR2, and split-file volumes can collapse into one selectable package.</p><div class="feature-mini"><span>✓</span> Connected binary sets</div><div class="feature-mini"><span>✓</span> Multipart file assembly</div><div class="feature-mini"><span>✓</span> Queue complete sets together</div>';
}
function renderPreviewDetails(a,auto=false){
  if(!isAllPostsMode()){renderRelatedMediaPane(null,{incremental:true});return}
  ...
  const canDownload=isSelectableMedia(a),canPreview=!!(a.media&&a.complete&&['image','video'].includes(a.media?.kind));const unresolved=canDownload&&!a.media,mediaName=a.media?.filename||(unresolved?'Filename not resolved yet — download remains available':'No directly downloadable binary detected');const cached=state.previewCache.get(previewKey(a));
  if(cached&&canPreview){showPreparedPreview(a,cached);return}
  els.previewContent.className='preview-content';
  if(auto&&a.media?.kind==='image'&&a.complete){els.previewBadge.textContent='LOADING';els.previewContent.innerHTML=`<div class="preview-loading"><div class="preview-spinner"></div><h3>Loading image…</h3><p>${escapeHtml(mediaName)}</p></div>`;preparePreview(a);return}
  const previewLabel=canPreview?(a.media?.kind==='video'?'Load video preview':'Load preview'):a.media?.kind==='file'?'No preview for this file type':unresolved?'Filename resolution required for preview':a.media&&!a.complete?'Multipart set is incomplete':'No media detected';
  els.previewContent.innerHTML=`<div class="preview-details"><h3>${escapeHtml(a.subject)}</h3><div class="preview-meta-grid"><div class="meta-card"><label>Detected</label><b>${escapeHtml(a.media?.kind||'Article')}</b></div><div class="meta-card"><label>Size</label><b>${formatBytes(a.bytes)}</b></div><div class="meta-card"><label>Segments</label><b>${a.segment_count}/${a.segment_total}</b></div><div class="meta-card"><label>Date</label><b>${escapeHtml(shortDate(a.date))}</b></div></div><div class="meta-card" style="margin-bottom:12px"><label>Binary file</label><b title="${escapeHtml(mediaName)}">${escapeHtml(mediaName)}</b></div><div class="preview-identity-grid"><div><label>Newsgroup</label><b>${escapeHtml(articleGroup(a))}</b></div><div><label>Poster</label><b>${escapeHtml(a.from||'—')}</b></div><div class="wide"><label>Message-ID</label><code>${escapeHtml(a.message_id||'—')}</code></div></div><button id="preparePreviewBtn" class="preview-action" ${canPreview?'':'disabled'}>${previewLabel}</button>${canDownload?`<button id="downloadPreviewBtn" class="preview-download">＋ ${unresolved?'Resolve & add to queue':'Add this file to queue'}</button>`:''}<p class="preview-note">${unresolved?'NewzDeck will try to recover the yEnc filename first. If the original name is unavailable, the complete binary will still download under a stable generated name and its file type will be identified after transfer.':'Images and videos can be previewed in-app. Other binary files can be selected and queued directly without downloading their bodies during search.'}</p></div>`;
...
async function resolveNamesForDownload(items){
  const unresolved=items.filter(a=>isUnresolvedDownloadCandidate(a));if(!unresolved.length)return 0;const providerId=state.providerId;let recovered=0;
  for(let offset=0;offset<unresolved.length;offset+=12){
    const batch=unresolved.slice(offset,offset+12);for(const a of batch)state.nameResolutionAttempted.add(nameResolutionKey(a));
    try{
      const groups=new Map();for(const a of batch){const g=articleGroup(a);if(!groups.has(g))groups.set(g,[]);groups.get(g).push(a)}
      for(const [group,groupItems] of groups){
        const data=await api('/api/articles/resolve-names',{provider_id:providerId,group,download_fallback:true,items:groupItems.map(nameResolutionPayload)},browseRequestOptions({timeoutMs:12000,timeoutMessage:'Filename probe timed out; NewzDeck will download with a generated name and recover the type after transfer.'}));
        recordNameResolutionFailures(data.results||[]);const byKey=new Map((data.results||[]).map(r=>[String(r.client_key||''),r]));for(const a of groupItems){const r=byKey.get(articleKey(a));if(r?.resolved&&r.media?.filename){applyNameResolutionResultToArticle(a,r);recovered++}}
      }
    }catch(e){for(const a of batch)if(!a.media){const key=nameResolutionKey(a);state.nameResolutionAttempted.delete(key);state.nameResolutionFailures.delete(key);state.nameResolutionDeferred.set(key,{message:String(e?.message||'Filename probe failed'),until:Date.now()+5000,code:'request_error'})}state.nameResolutionBackoffUntil=Math.max(state.nameResolutionBackoffUntil,Date.now()+5000);}
  }
  return recovered;
}
async function downloadItems(items,button=null){
  const valid=items.filter(isSelectableMedia);if(!valid.length){toast(isAllPostsMode()?'No complete downloadable binaries selected.':'No complete media items selected.','error');return}
  const old=button?.textContent;if(button){button.disabled=true;button.textContent='Preparing…'}
  try{
    const initiallyUnresolved=valid.filter(a=>!a.media).length;if(initiallyUnresolved){await resolveNamesForDownload(valid);if(button)button.textContent='Adding…'}
    const collectionAssignments=browserSetCollectionAssignments(valid.filter(a=>a.media)),collectionIds=new Set();
    ...
    let added=0,dupes=0,fallbackAdded=0;const warnings=[];
    for(const [group,groupItems] of byGroup){
      const payload=groupItems.map(a=>{const collection=collectionAssignments.get(articleKey(a)),role=collection?browserSetCollectionRole(a):'';if(collection)collectionIds.add(collection.id);return{segments:segmentPayload(a),media:a.media||{},allow_unresolved:!a.media,subject:a.subject||'',from:a.from||'',message_id:a.message_id||'',article:a.article,multipart:a.multipart||null,raw_yenc_name:a.raw_yenc_name||'',...(collection?{source:'browser_set',collection_id:collection.id,collection_name:collection.name,collection_expected:collection.expected,collection_required_expected:collection.required,destination_subdir:collection.name,collection_role:role,is_auxiliary:role==='auxiliary',is_par2:role==='par2'||role==='recovery_par2',is_par2_volume:role==='recovery_par2'}:{source:'browser_loose',destination_root:true})}});
      const data=await api('/api/downloads/add',{provider_id:state.providerId,group,items:payload});added+=data.added?.length||0;fallbackAdded+=(data.added||[]).filter(x=>x?.generated_name).length;dupes+=data.duplicates?.length||0;if(data.disk_warning)warnings.push(data.disk_warning)
    }
    if(added){const packageCount=collectionIds.size;toast(packageCount?`${added} file${added===1?'':'s'} added as ${packageCount} grouped package${packageCount===1?'':'s'}.`:`${added} file${added===1?'':'s'} added to the download queue${fallbackAdded?` • ${fallbackAdded} will use safe generated names if the original filename stays unavailable`:''}.`,'success')}
...
function setMainView(view){
  if(state.activeView==='browse'&&view!=='browse'){state.browseScrollTop=els.articlesList.scrollTop;state.lastRelatedCoverSnapshot=relatedCoverSchedulerSummaryText();rotateBrowsePreviewSession()}
...
async function loadDiagnostics({userInitiated=false}={}){
  const done=userInitiated?beginButtonActivity($('refreshDiagnosticsBtn'),'Refreshing…'):()=>{};
  try{state.diagnosticsSnapshot=await api('/api/diagnostics');renderDiagnostics()}catch(e){if(state.activeView==='diagnostics')toast(e.message,'error')}finally{done()}
}
function renderDiagnostics(){
  ...
  const cloud=d.metadata_cloud||{},cloudStatus=String(cloud.status||'unknown').toUpperCase();if($('diagCloudStatus')){$('diagCloudStatus').textContent=cloudStatus;$('diagCloudStatus').className=`cloud-health ${cloud.status||'unknown'}`;const cloudBits=[];if(cloud.server_version)cloudBits.push(`Server v${cloud.server_version}`);if(cloud.tmdb_status&&cloud.tmdb_status!=='unknown')cloudBits.push(`TMDB ${String(cloud.tmdb_status).toUpperCase()}`);else if(cloud.tmdb_status_detail)cloudBits.push(String(cloud.tmdb_status_detail));if(cloud.circuit_open)cloudBits.push(`retry in ~${Number(cloud.circuit_retry_seconds||0)}s`);if(cloud.cached_fallbacks)cloudBits.push(`${Number(cloud.cached_fallbacks)} cached fallback${Number(cloud.cached_fallbacks)===1?'':'s'}`);$('diagCloudDetail').textContent=cloudBits.join(' • ')||(cloud.url||'api.newzdeck.com');$('diagCloudAuth').textContent=cloud.authenticated?'REGISTERED':'NOT REGISTERED';$('diagCloudAuth').className=`cloud-health ${cloud.authenticated?'online':'unknown'}`;$('diagCloudAuthDetail').textContent=cloud.compatible===false?`Client update required • minimum v${cloud.min_client_version||'?'}`:(cloud.authenticated?'Per-installation credential active':'Will register automatically when required');}
  const providers=d.providers||[];const good=providers.filter(p=>p.status==='connected').length,bad=providers.filter(p=>p.status==='error').length,activelyTransferring=providers.filter(p=>p.runtime_active).length;$('diagProviderSummary').textContent=providers.length?`${activelyTransferring} transferring • ${good-activelyTransferring} connected idle • ${bad} with errors • ${providers.length-good-bad} standby`:'No providers configured';
  $('providerHealthRows').innerHTML=providers.length?providers.map(p=>{const pool=p.pool||{},rate=p.success_rate==null?'—':`${Number(p.success_rate).toFixed(1)}%`,lat=p.last_latency_ms?`${Number(p.last_latency_ms).toFixed(0)} ms`:'—',statusLabel=p.runtime_active?'ACTIVE':String(p.status||'standby').toUpperCase();return `<div class="provider-health-row"><span><strong>${escapeHtml(p.name)}</strong><small>${escapeHtml(p.host)}:${Number(p.port)}</small><em class="provider-role-note">${escapeHtml((p.role||'primary')+' provider')}${p.runtime_source==='sabnzbd'?' • SAB runtime':''}</em></span><span><b class="health-pill ${healthClass(p.status)}">${escapeHtml(statusLabel)}</b></span><span>${lat}</span><span>${rate}</span><span>${Number(pool.active||0)}/${Number(pool.capacity||0)}<small>${Number(pool.open||0)} warm</small></span><span>${formatBytes(p.bytes||0)}</span><span>${Number(pool.retries||0).toLocaleString()}</span><span class="health-error" title="${escapeHtml(p.last_error||'')}">${escapeHtml(p.last_error||'—')}</span></div>`}).join(''):'<div class="diag-empty">Add an NNTP provider to see health information.</div>';
  const dl=d.downloads||{},counts=dl.counts||{},searches=d.searches||[],tel=dl.telemetry||{},stats=dl.statistics||{},thumbDecode=d.thumbnail_decode||{},thumbTransfer=d.thumbnail_transfer||{},thumbHelper=thumbDecode.helper||{},thumbCatalog=d.thumbnail_catalog||{},thumbToken=thumbCatalog.token_cache||{};$('diagEngineActivity').innerHTML=`<div><span>Download engine</span><strong>${Number(counts.downloading||0)} downloading • ${Number(counts.queued||0)} queued</strong></div><div><span>Transfer speed</span><strong>${formatSpeed(dl.speed_bps||0)}</strong></div><div><span>Statistics ledger</span><strong>${stats.statistics_source?`${formatBytes(stats.total_downloaded_bytes||0)} total • ${Number(stats.average_coverage_percent||0).toFixed(0)}% timed coverage • ${Number(stats.history_accounted_jobs||0).toLocaleString()} completions accounted`:'Legacy/session statistics'}</strong></div><div><span>Pipeline rates</span><strong>Net ${formatSpeed(tel.network_rate_bps||0)} • Decode ${Number(tel.decode_rate_bps||0)?formatSpeed(tel.decode_rate_bps):'—'} • Disk ${Number(tel.disk_rate_bps||0)?formatSpeed(tel.disk_rate_bps):'—'}</strong></div><div><span>Active-card continuity</span><strong>${Number(tel.active_card_continuity_bridges||0).toLocaleString()} SAB handoff bridge${Number(tel.active_card_continuity_bridges||0)===1?'':'s'}</strong></div><div><span>Snapshot consistency</span><strong>${Number(tel.snapshot_consistency_mismatches||0).toLocaleString()} correction${Number(tel.snapshot_consistency_mismatches||0)===1?'':'s'} • raw remaining ${formatBytes(tel.engine_remaining_bytes_raw||0)} / visible ${formatBytes(tel.visible_remaining_bytes||0)}</strong></div><div><span>Downloads snapshot latency</span><strong>p50 ${Number(tel.snapshot_p50_ms||0).toFixed(0)} ms • p95 ${Number(tel.snapshot_p95_ms||0).toFixed(0)} ms • p99 ${Number(tel.snapshot_p99_ms||0).toFixed(0)} ms • ${Number(tel.snapshot_sample_count||0)} samples</strong></div><div><span>Downloads API view</span><strong>${escapeHtml(String(tel.downloads_api_scope||'all'))} • prepare ${Number(tel.downloads_api_prepare_ms||0).toFixed(0)} ms • provider cache ${Number((dl.connections||{}).provider_health_cache_age_seconds||0).toFixed(1)}s old</strong></div><div><span>SAB Queue/History transport</span><strong>${Number(tel.sab_read_resets||0).toLocaleString()} reset${Number(tel.sab_read_resets||0)===1?'':'s'} absorbed • ${Number(tel.sab_read_stale_uses||0).toLocaleString()} short cached read${Number(tel.sab_read_stale_uses||0)===1?'':'s'} • ${Number(tel.sab_stale_snapshot_suppressed||0).toLocaleString()} stale snapshot${Number(tel.sab_stale_snapshot_suppressed||0)===1?'':'s'} suppressed</strong></div><div><span>SAB control-plane recovery</span><strong>${Number(tel.engine_transient_probe_misses||0).toLocaleString()} transient probe miss${Number(tel.engine_transient_probe_misses||0)===1?'':'es'} • ${Number(tel.engine_recovery_deferred||0).toLocaleString()} disruptive recoveries deferred • ${Number(tel.engine_launch_recoveries||0).toLocaleString()} real launch recoveries</strong></div><div><span>Authoritative SAB identity</span><strong>${Number(tel.identity_cross_adoptions_blocked||0).toLocaleString()} cross-generation adoption${Number(tel.identity_cross_adoptions_blocked||0)===1?'':'s'} blocked • ${Number(tel.identity_authoritative_key_repairs||0).toLocaleString()} current-key repair${Number(tel.identity_authoritative_key_repairs||0)===1?'':'s'}</strong></div><div><span>Historical SAB quarantine</span><strong>${Number(tel.stale_engine_ports_seen||0).toLocaleString()} stale engine observation${Number(tel.stale_engine_ports_seen||0)===1?'':'s'} • ${Number(tel.stale_engine_live_slots_paused||0).toLocaleString()} hidden slot${Number(tel.stale_engine_live_slots_paused||0)===1?'':'s'} paused • ${Number(tel.stale_engines_shutdown||0).toLocaleString()} empty engine${Number(tel.stale_engines_shutdown||0)===1?'':'s'} shut down</strong></div><div><span>Stale duplicate queue cleanup</span><strong>${Number(tel.stale_duplicate_queue_cleanups||0).toLocaleString()} proven duplicate${Number(tel.stale_duplicate_queue_cleanups||0)===1?'':'s'} removed</strong></div><div><span>Completion monitor</span><strong>${Number(tel.completion_control_failures||0).toLocaleString()} control failure${Number(tel.completion_control_failures||0)===1?'':'s'} • ${Number(tel.completion_backoff_seconds||0).toFixed(0)}s backoff</strong></div><div><span>Engine fault</span><strong>${tel.engine_fault?escapeHtml(String(tel.engine_fault)):'None'}</strong></div><div><span>SAB config stability</span><strong>${Number(tel.config_sync_attempts||0).toLocaleString()} sync${Number(tel.config_sync_attempts||0)===1?'':'s'} • ${Number(tel.config_sync_failures||0).toLocaleString()} incomplete • ${Number(tel.config_retry_storms_suppressed||0).toLocaleString()} retry storm${Number(tel.config_retry_storms_suppressed||0)===1?'':'s'} suppressed</strong></div><div><span>One-package state corrections</span><strong>${Number(tel.multiple_active_slot_corrections||0).toLocaleString()} multi-Active correction${Number(tel.multiple_active_slot_corrections||0)===1?'':'s'} • ${Number(tel.progress_regression_corrections||0).toLocaleString()} progress regression${Number(tel.progress_regression_corrections||0)===1?'':'s'} corrected</strong></div><div><span>Engine pause recovery</span><strong>${tel.engine_pause_mismatch?'RECOVERING':tel.engine_idle_paused?'Idle (SAB paused, no work)':`${Number(tel.engine_pause_reassert_count||0).toLocaleString()} resume reassertion${Number(tel.engine_pause_reassert_count||0)===1?'':'s'}`}</strong></div><div><span>Import ownership recovery</span><strong>${Number(tel.import_dead_owner_reclaims||0).toLocaleString()} dead-runtime reclaim${Number(tel.import_dead_owner_reclaims||0)===1?'':'s'}</strong></div><div><span>Hidden-transfer cleanup</span><strong>${Number(tel.removed_orphan_cleanup_count||0).toLocaleString()} verified cleanup${Number(tel.removed_orphan_cleanup_count||0)===1?'':'s'}</strong></div><div><span>Thumbnail BODY</span><strong>${Number(thumbTransfer.runs||0)?`avg ${Math.round(Number(thumbTransfer.average_ms||0))} ms • ${Number(thumbTransfer.parallel_runs||0)} parallel • max ${Number(thumbTransfer.max_lanes||1)} lanes`:'No samples yet'}</strong></div><div><span>Thumbnail decode</span><strong>${Number(thumbDecode.runs||0)?`avg ${Math.round(Number(thumbDecode.average_decode_ms||0))} ms • wait ${Math.round(Number(thumbDecode.average_wait_ms||0))} ms • WIC ${Number(thumbDecode.wic_runs||0)} / fallback ${Number(thumbDecode.fallback_runs||0)}`:`${Number(thumbDecode.workers||0)} workers • no samples yet`}</strong></div><div><span>Native thumbnail workers</span><strong>${Number(thumbHelper.jobs||0)?`${Number(thumbHelper.jobs||0)} jobs • ${Number(thumbHelper.process_launches_avoided||0)} launches avoided • ${Number(thumbHelper.reuse_rate_percent||0)}% reuse • ${Number(thumbHelper.blank_rejections||0)} native blank rejects`:`${Number(thumbHelper.workers||0)} persistent workers ready`}</strong></div><div><span>Thumbnail RAM index</span><strong>${Number(thumbCatalog.entries||0).toLocaleString()} entries • ${Number(thumbCatalog.hits||0).toLocaleString()} hits • ${Number(thumbCatalog.filesystem_fallbacks||0).toLocaleString()} FS fallbacks • token ${Number(thumbToken.hits||0).toLocaleString()} hits</strong></div><div><span>Next-page warming</span><strong>${Number(state.speculativeThumbStats.completed||0)} warmed • ${Number(state.speculativeThumbStats.cancelled||0)} yielded</strong></div><div><span>Thumbnail DOM registry</span><strong>${Number(state.thumbHolderRegistry.size||0).toLocaleString()} live • ${Number(state.thumbHolderRegistryStats.hits||0).toLocaleString()} direct hits • ${Number(state.thumbHolderRegistryStats.fallbacks||0).toLocaleString()} fallbacks</strong></div><div><span>Recovery health</span><strong>${Number(tel.soft_misses||0)} soft misses • ${Number(tel.native_parts||0).toLocaleString()} native blocks</strong></div><div><span>Header searches</span><strong>${searches.length} entire-group search${searches.length===1?'':'es'} active</strong></div><div><span>Browser timings</span><strong>${escapeHtml(perfSummaryText())}</strong></div><div><span>Related Media cover scheduler</span><strong>${escapeHtml(relatedCoverSchedulerSummaryText())}</strong></div><div><span>Preview cache</span><strong>${formatBytes(storage.preview_cache_bytes||0)}</strong></div><div><span>Download scratch</span><strong>${formatBytes(storage.download_temp_bytes||0)}</strong></div><div><span>Persistent data</span><strong>${formatBytes(storage.data_bytes||0)}</strong></div><div><span>NZB automation</span><strong>${d.automation?.watch_enabled?`Watch on • ${Number(d.automation.watch_imported||0)} imported${Number(d.automation.watch_failed||0)?` • ${Number(d.automation.watch_failed)} failed`:''}`:'Watch folder off'}${d.automation?.bandwidth?.active?` • cap ${Number(d.automation.bandwidth.limit_mb_s||0)} MB/s`:''}</strong></div>`;
...
async function copyDiagnostics(){try{const d=await api('/api/diagnostics/report');const report=`${d.report}\nBrowser timings: ${perfSummaryText()}\nRelated Media covers: ${relatedCoverSchedulerSummaryText()}\nRelated Media before leaving Newsgroups: ${state.lastRelatedCoverSnapshot||'No browse-leave snapshot captured yet.'}`;if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(report);toast('Diagnostic report copied.','success')}else{window.prompt('Copy diagnostic report:',report)}}catch(e){toast(e.message,'error')}}
...
function failedDownloadIds(jobs=state.downloadSnapshot?.jobs||[]){
  const view=state.downloadSnapshot?.view||{};
  if(String(view.scope||'')==='failed'&&Array.isArray(view.matching_ids))return [...new Set(view.matching_ids.map(String).filter(Boolean))];
  return [...new Set((jobs||[]).filter(job=>{const s=String(job?.status||''),post=String(job?.post_status||'');return ['failed','cancelled'].includes(s)||['failed','needs_attention','needs_tool','blocked','cancelled'].includes(post)}).map(job=>String(job?.id||'')).filter(Boolean))];
}
function updateRemoveAllFailedButton(jobs=state.downloadSnapshot?.jobs||[]){
  const btn=$('removeAllFailedBtn');if(!btn)return;const ids=failedDownloadIds(jobs),show=state.downloadFilter==='failed'&&ids.length>0;btn.classList.toggle('hidden',!show);btn.disabled=!show;btn.textContent=show?`Remove all failed (${ids.length.toLocaleString()})`:'Remove all failed';
}
async function removeAllFailedDownloads(){
  const btn=$('removeAllFailedBtn'),ids=failedDownloadIds();if(!ids.length){updateRemoveAllFailedButton();return}
  const noun=ids.length===1?'failed download':'failed downloads';
  if(!window.confirm(`Remove all ${ids.length.toLocaleString()} ${noun}?

This removes their NewzDeck/SAB download entries and any remaining download staging files. Library files that were already imported are left alone.`))return;
  const old=btn?.textContent||'Remove all failed';if(btn){btn.disabled=true;btn.textContent=`Removing ${ids.length.toLocaleString()}…`}
  try{
    state.downloadSnapshot=await api('/api/downloads/control',{action:'remove',id:'',value:null,ids});
    recalculatePreviewConcurrency();for(const id of ids)state.selectedDownloads.delete(String(id));rebuildDownloadIndexes();renderDownloads();updateDownloadBadgesInPlace();if(state.viewerOpen)updateViewerSelectionState();toast(`Removed ${ids.length.toLocaleString()} ${noun}.`,'success');
  }catch(e){
    toast(e.message,'error');await loadDownloads({quiet:true});
  }finally{if(btn?.isConnected){btn.disabled=false;btn.textContent=old;updateRemoveAllFailedButton()}}
}
...
async function loadDownloads({render=true,livePatch=false,quiet=false}={}){
  ...
  try{
    const terminalView=['completed','failed'].includes(state.downloadFilter);
    const scope=terminalView?state.downloadFilter:'live';
    const limit=terminalView?Math.max(1,Math.min(500,Number(state.downloadHistoryLimit||50))):200;
    const offset=terminalView?Math.max(0,Number(state.downloadHistoryOffset||0)):0;
    const downloads=await api(`/api/downloads?scope=${encodeURIComponent(scope)}&limit=${limit}&offset=${offset}`,null,{timeoutMs:3500,timeoutMessage:'Downloads status took too long to respond. NewzDeck will retry automatically.'});
    if(terminalView){
      const incoming=Array.isArray(downloads.jobs)?downloads.jobs:[];
      if(offset===0)state.downloadHistoryLoaded=incoming;else{const byId=new Map((state.downloadHistoryLoaded||[]).map(x=>[String(x.id),x]));for(const row of incoming)byId.set(String(row.id),row);state.downloadHistoryLoaded=[...byId.values()]}
      downloads.jobs=[...(state.downloadHistoryLoaded||[])];
    }
...
function downloadDiagnosticsText(job){
  ...
  const lines=[
    ...
    `SAB PAR2 outcome: ${job.repair_outcome||'not_observed'}; verification observed: ${job.verification_observed===true?'yes':'no'}; repair attempted: ${job.repair_attempted===true?'yes':'no'}; extra PAR2 fetched: ${job.par2_fetch_observed===true?'yes':'no'}`,
    `SAB repair summary: ${job.repair_summary||'—'}`,
    `SAB failure class: ${job.failure_class||'—'}; post-processing time: ${Number(job.postproc_seconds||0)}s; recovery block counts: ${job.recovery_blocks_reported===true?'reported':'not reported by SAB'}`,
    `Failure: ${job.error_label||''} ${job.error||''}`,
    `Suggested action: ${job.error_suggestion||''}`,
    `Filename recovery: ${job.filename_recovery_source||'—'}${job.filename_recovery_exact?' (exact)':''}`,
    `Filename recovery hint: ${job.filename_recovery_hint||'—'}${job.filename_recovery_hint_source?` via ${job.filename_recovery_hint_source}`:''}`,
  ];
  if(Array.isArray(job.sab_stage_log)&&job.sab_stage_log.length){
    lines.push('',`SAB repair/post-processing history:`);
    for(const line of job.sab_stage_log)lines.push(`- ${line}`);
  }
  if(errors.length){
...
async function toggleDownloadDetails(id){const key=String(id);if(state.expandedDownloads.has(key)){state.expandedDownloads.delete(key);renderDownloads();return}let job=(state.downloadSnapshot?.jobs||[]).find(j=>String(j.id)===key);if(job&&job.details_loaded===false){try{const d=await api(`/api/downloads?scope=detail&id=${encodeURIComponent(key)}`,null,{timeoutMs:3500});const detail=(d.jobs||[])[0];if(detail){state.downloadSnapshot.jobs=(state.downloadSnapshot.jobs||[]).map(x=>String(x.id)===key?detail:x);state.downloadHistoryLoaded=(state.downloadHistoryLoaded||[]).map(x=>String(x.id)===key?detail:x)}}catch(e){toast(e.message,'error');return}}state.expandedDownloads.add(key);renderDownloads()}
...
function packageStatusLabel(pkg){
  ...
  if(p==='repairing')return 'REPAIRING';if(p==='verifying')return 'VERIFYING';if(p==='extracting')return 'EXTRACTING';if(p==='importing')return pkg.import_stalled?'IMPORT STALLED':'IMPORTING';
...
function packageHealthHtml(pkg){
  const h=pkg.health||{},state=String(h.state||'healthy'),repair=String(pkg.repair_outcome||h.repair_outcome||'');
  if(repair==='repaired')return `<span class="package-health healthy">✓ PAR2 REPAIRED</span>`;
  if(repair==='unrecoverable')return `<span class="package-health incomplete">PAR2 UNRECOVERABLE</span>`;
...
function downloadStructureSignature(d,allJobs,collections){
  ...
  const packages=(collections||[]).map(p=>[String(p.id||''),String(p.status||''),String(p.post_status||''),String(p.priority||''),Number(p.queue_order||0),String(p.health?.state||''),Number(p.deferred_recovery_files||0),!!p.import_stalled,Math.floor(Number(p.import_heartbeat_age_seconds||0)/15),(p.job_ids||[]).map(String).join(','),state.expandedCollections.has(String(p.id||''))?1:0]);
...
function renderDownloads({livePatch=false}={}){
  ...
  updateRemoveAllFailedButton(allJobs);
  ...
  if($('dlLifetimeAverage')){
    $('dlLifetimeAverage').textContent=Number(lifetime.average_speed_bps||0)>0?formatSpeed(lifetime.average_speed_bps):'—';
    const coverage=Math.max(0,Math.min(100,Number(lifetime.average_coverage_percent||0)));
    const note=$('dlLifetimeAverage').parentElement?.querySelector('small');
    if(note)note.textContent=(coverage>0&&coverage<99.5)?`weighted timed history • ${coverage.toFixed(0)}% coverage`:'weighted over timed transfers';
    $('dlLifetimeAverage').title=(coverage>0&&coverage<99.5)?`Average uses transfers with recoverable timing (${coverage.toFixed(1)}% of lifetime bytes). Deleted historical timing is not invented.`:'Lifetime weighted average over timed transfers';
  }
  ...
  const tel=d.telemetry||{},enginePauseMismatch=tel.engine_pause_mismatch===true,engineFault=String(tel.engine_fault||''),sabControlDegraded=tel.sab_control_degraded===true;
  $('downloadQueueState').textContent=d.paused?'Queue paused':engineFault?`Engine blocked • disk error — see Diagnostics`:sabControlDegraded?`SAB control channel refreshing${Number(c.queued||0)?` • ${Number(c.queued)} queued`:''}`:enginePauseMismatch?`Engine paused unexpectedly • recovering${Number(d.post_processing_active||0)?` • ${Number(d.post_processing_active)} post-processing`:''}`:`${c.downloading||0} active • ${c.queued||0} queued${retryWaiting?` • ${retryWaiting} waiting to retry`:''}${Number(c.cancelling||0)?` • ${Number(c.cancelling)} stopping`:''}${Number(d.post_processing_active||0)?` • ${Number(d.post_processing_active)} post-processing`:''}${Number(d.remaining_bytes||0)?` • ${formatBytes(d.remaining_bytes)} remaining`:''}`;
  ...
  if(problemBanner){
    ...
    }else if(providerIssue&&conn.runtime_state_known!==false){
  ...
  const view=d.view||{};
  if(['completed','failed'].includes(state.downloadFilter)&&view.has_more){
    const remaining=Math.max(0,Number(view.total||0)-Number(view.returned||0));
    els.downloadsList.insertAdjacentHTML('beforeend',`<div class="download-history-more"><button data-download-history-more>Load 50 more${remaining?` • ${remaining.toLocaleString()} remaining`:''}</button></div>`);
    const more=els.downloadsList.querySelector('[data-download-history-more]');
    if(more)more.onclick=async()=>{more.disabled=true;more.textContent='Loading…';state.downloadHistoryOffset=(state.downloadHistoryLoaded||[]).length;state.downloadHistoryLimit=50;await loadDownloads({render:true,quiet:false})};
  }
  ...
  els.downloadsList.querySelectorAll('[data-package-action]').forEach(btn=>btn.onclick=async e=>{e.stopPropagation();const pkg=collections.find(x=>String(x.id)===String(btn.dataset.packageId));if(!pkg)return;const action=btn.dataset.packageAction,old=btn.textContent;if(['remove','cancel'].includes(action)){btn.disabled=true;btn.textContent=action==='remove'?'Removing…':'Cancelling…'}try{if(action==='fetch_recovery')await downloadControl('fetch_recovery','',pkg.id);else await downloadControl(action,'',null,pkg.job_ids||[])}finally{if(btn.isConnected){btn.disabled=false;btn.textContent=old}}});
...
function downloadRow(job){
  ...
  const filenameRecoveryChip=job.filename_recovery_exact?`<span class="block-chip reused" title="${escapeHtml(job.filename_recovery_source||'Recovered filename')}">NAME RECOVERED</span>`:(job.filename_recovery_hint?`<span class="block-chip phase" title="${escapeHtml((job.filename_recovery_hint_source||'Metadata')+': '+job.filename_recovery_hint)}">NAME HINT</span>`:'');
  const repairOutcome=String(job.repair_outcome||''),repairChip=repairOutcome==='repaired'?`<span class="recovery-chip" title="${escapeHtml(job.repair_summary||'SAB completed after repair activity')}">PAR2 REPAIRED</span>`:repairOutcome==='verified'?`<span class="block-chip reused" title="${escapeHtml(job.repair_summary||'SAB verification completed')}">PAR2 VERIFIED</span>`:repairOutcome==='unrecoverable'?`<span class="block-chip bad" title="${escapeHtml(job.repair_summary||'SAB reported insufficient article/recovery data')}">PAR2 UNRECOVERABLE</span>`:repairOutcome==='failed_unpack'?`<span class="block-chip bad">UNPACK FAILED</span>`:repairOutcome==='failed_password'?`<span class="block-chip bad">PASSWORD FAILURE</span>`:repairOutcome==='failed_filesystem'?`<span class="block-chip bad">DISK / FILE FAILURE</span>`:'';
  const healthChips=`${repairChip}${repairNeeded?`<span class="block-chip repair">PAR2 REPAIR • ${Number(job.repair_missing_blocks||failed)} MISSING</span>`:''}${failed&&!repairNeeded?`<span class="block-chip ${softRecheck?'retry':'bad'}">${softRecheck?'RECHECK':'MISSING'} ${failed}</span>`:''}${recovered?`<span class="recovery-chip" title="${escapeHtml(recoverySources)}">RECOVERED ${recovered}</span>`:''}${resumed?`<span class="block-chip reused">REUSED ${resumed}</span>`:''}${retries?`<span class="block-chip retry">PROVIDER RETRIES ${retries}</span>`:''}${filenameRecoveryChip}${job.transfer_phase&&job.status==='downloading'?`<span class="block-chip phase">${escapeHtml(String(job.transfer_phase).toUpperCase())}</span>`:''}`;
...
function downloadDetailPanel(job){
  ...
  const sabManaged=job.source==='nzb'&&String(job.transfer_phase||'')==='sabnzbd',repairOutcome=String(job.repair_outcome||'not_observed').replaceAll('_',' '),repairSummary=String(job.repair_summary||'—');
  const metrics=[['Block health',`${good} good • ${failed} unavailable • ${Math.max(0,total-processed)} not processed`],['Current / peak speed',`${formatSpeed(job.speed_bps||0)} / ${formatSpeed(job.peak_speed_bps||0)}`],['Average speed',Number(job.average_speed_bps||0)?formatSpeed(job.average_speed_bps):'—'],['Transfer time',Number(job.duration_seconds||0)?formatEta(job.duration_seconds):'—'],['Connections',String(Number(job.connections_used||0))],['Pipeline',`${job.pipeline||'standard'}${Number(job.native_parts||0)?` • ${Number(job.native_parts)} native blocks`:''}`],['Retries',String(Number(job.retry_count||0))],['Recovered',sabManaged?repairSummary:`${Number(job.recovered_parts||0)} block(s) • ${recoverySources}`],['Reused on this run',`${Number(job.resumed_parts||0)} verified block(s)`],['Integrity',String(job.integrity_status||'unknown').replaceAll('_',' ')],['PAR2 outcome',sabManaged?repairOutcome:'—'],['Repair attempted',sabManaged?(job.repair_attempted===true?'Yes':'No observed repair'):'—'],['Extra PAR2 fetched',sabManaged?(job.par2_fetch_observed===true?'Yes':'No observed fetch'):'—'],['PAR2 block counts',sabManaged?(job.recovery_blocks_reported===true?'Reported by SAB':'Not reported by SAB'):(Number(job.repair_missing_bytes||0)?`${formatBytes(job.repair_missing_bytes)} • ${Number(job.repair_missing_blocks||0)} article blocks`:'—')],['Failure class',job.failure_class?String(job.failure_class).replaceAll('_',' '):'—'],['Post-processing time',Number(job.postproc_seconds||0)?formatEta(job.postproc_seconds):'—'],['Provider',job.provider_name||'—'],['Source',job.source==='nzb'?`NZB${job.collection_name?` • ${job.collection_name}`:''}`:'NewzDeck browser'],['Last activity',job.last_activity_ts?new Date(Number(job.last_activity_ts)*1000).toLocaleTimeString():'—'],['Last real progress',job.last_progress_ts?new Date(Number(job.last_progress_ts)*1000).toLocaleTimeString():'—'],['Next retry',job.status==='retry_wait'?(retrySecs?`in ${retrySecs}s`:'starting now'):'—'],['Post-processing',String(job.post_status||'')?`${String(job.post_status).replaceAll('_',' ')}${job.post_progress_known===true?` • ${Number(job.post_progress||0)}%`:job.post_indeterminate?' • working':''}${job.post_message?` • ${String(job.post_message)}`:''}`:'—'],['Filename recovery',job.filename_recovery_source?`${String(job.filename_recovery_source)}${job.filename_recovery_exact?' • exact':''}`:'—'],['Recovered name hint',job.filename_recovery_hint?`${String(job.filename_recovery_hint)}${job.filename_recovery_hint_source?` • ${String(job.filename_recovery_hint_source)}`:''}`:'—']];
  let failures='';
  ...
  const repairHistory=Array.isArray(job.sab_stage_log)&&job.sab_stage_log.length?`<div class="download-block-failures"><div class="download-detail-heading"><strong>SAB repair history</strong><span>Observed Verify / PAR2 / post-processing messages</span></div>${job.sab_stage_log.map(line=>`<div class="download-block-failure"><div class="block-failure-message">${escapeHtml(line)}</div></div>`).join('')}</div>`:'';
  return `<div class="download-detail-panel"><div class="download-detail-top"><div><div class="download-detail-heading"><strong>Transfer diagnostics</strong><span>Block-level history for this file</span></div><div class="download-detail-grid">${metrics.map(([k,v])=>`<div><span>${escapeHtml(k)}</span><b>${escapeHtml(v)}</b></div>`).join('')}</div></div><div class="download-detail-actions"><button type="button" data-dl-copy="${job.id}">Copy diagnostics</button></div></div>${job.error_suggestion?`<div class="download-recovery-advice"><b>Recovery guidance</b><span>${escapeHtml(job.error_suggestion)}</span></div>`:''}${repairHistory}${failures}</div>`;
}
...
function handleKeyboardShortcuts(e){
  ...
  if(e.key==='Enter'){const a=state.articles.find(x=>articleKey(x)===state.selectedArticleKey);if(['image','video'].includes(a?.media?.kind)&&a.complete){e.preventDefault();state.activeMediaSetKey='';openMediaViewer(a)}return}
}
...
function beginManagedUpdateUiExit(resultEl,message){
  ...
  // v3.6.92 launches the checksum-verified Setup directly. window.close() is an
  // immediate best-effort path; Setup invokes the installed NewzDeck launcher
  // after overlay to close any Chromium app-mode window that remains open.
...
els.settingsCloseBtn.onclick=closeSettingsModal;els.settingsCancelBtn.onclick=closeSettingsModal;els.settingsSaveBtn.onclick=saveSettingsModal;if($('settingsTheme'))$('settingsTheme').onchange=()=>applyTheme($('settingsTheme').value);els.settingsModal.addEventListener('click',e=>{if(e.target===els.settingsModal)closeSettingsModal()});document.querySelectorAll('[data-settings-tab]').forEach(b=>b.onclick=()=>{switchSettingsTab(b.dataset.settingsTab);if(b.dataset.settingsTab==='background')refreshServiceSettings()});$('settingsChooseFolderBtn').onclick=settingsChooseDownloadFolder;$('settingsChooseWatchFolderBtn').onclick=settingsChooseWatchFolder;if($('settingsConfigBackupBtn'))$('settingsConfigBackupBtn').onclick=createConfigurationBackup;if($('settingsCompleteBackupBtn'))$('settingsCompleteBackupBtn').onclick=createCompleteBackup;if($('settingsRestoreBrowseBtn'))$('settingsRestoreBrowseBtn').onclick=()=>$('settingsRestoreInput').click();if($('settingsRestoreInput'))$('settingsRestoreInput').onchange=()=>inspectRestoreBackupFile($('settingsRestoreInput').files?.[0]);if($('settingsApplyRestoreBtn'))$('settingsApplyRestoreBtn').onclick=applyRestoreBackup;if($('settingsRestoreClearBtn'))$('settingsRestoreClearBtn').onclick=clearRestoreBackupSelection;$('settingsClearThumbCacheBtn').onclick=async()=>{const done=beginButtonActivity($('settingsClearThumbCacheBtn'),'Clearing thumbnails…');try{const r=await api('/api/cache/clear',{});state.imageThumbCache.clear();state.videoThumbCache.clear();state.imageThumbPromises.clear();state.videoThumbPromises.clear();toast(`Cleared ${Number(r.removed||0).toLocaleString()} cached thumbnails.`,'success')}catch(e){toast(e.message,'error')}finally{done()}};$('settingsClearPreviewCacheBtn').onclick=async()=>{const done=beginButtonActivity($('settingsClearPreviewCacheBtn'),'Clearing previews…');try{const r=await api('/api/cache/preview/clear',{});state.previewCache.clear();state.previewPromises.clear();resetPreview();toast(`Cleared ${Number(r.removed||0).toLocaleString()} cached preview file${Number(r.removed||0)===1?'':'s'} (${formatBytes(Number(r.removed_bytes||0))}).`,'success')}catch(e){toast(e.message,'error')}finally{done()}};$('settingsOpenDataBtn').onclick=async()=>{try{await api('/api/app/open-data',{})}catch(e){toast(e.message,'error')}};$('settingsProvidersBtn').onclick=()=>{closeSettingsModal();openProviderModal()};$('settingsDiagnosticsBtn').onclick=()=>{closeSettingsModal();setMainView('diagnostics')};if($('installServiceBtn'))$('installServiceBtn').onclick=installBackgroundService;if($('startServiceBtn'))$('startServiceBtn').onclick=()=>serviceSettingsControl('start');if($('stopServiceBtn'))$('stopServiceBtn').onclick=()=>serviceSettingsControl('stop');if($('restartServiceBtn'))$('restartServiceBtn').onclick=()=>serviceSettingsControl('restart');if($('repairServiceBtn'))$('repairServiceBtn').onclick=()=>serviceSettingsControl('repair');if($('launchTrayBtn'))$('launchTrayBtn').onclick=()=>serviceSettingsControl('launch_tray');if($('settingsTrayAutostart'))$('settingsTrayAutostart').onchange=async()=>{try{const r=await api('/api/service/control',{action:'tray_autostart',enabled:$('settingsTrayAutostart').checked});renderServiceSettings(r)}catch(e){toast(e.message,'error')}};
els.providerSelect.onchange=()=>{const oldGroup=state.selectedGroup,oldProvider=state.providerId;captureCurrentGroupState();if(oldGroup)endGroupVisit(oldGroup,oldProvider);if(state.groupSearchJob&&['queued','scanning','cancelling'].includes(state.groupSearchJob.status))api('/api/group-search/cancel',{id:state.groupSearchJob.id}).catch(()=>{});if(state.groupSearchPollTimer){clearInterval(state.groupSearchPollTimer);state.groupSearchPollTimer=null}closeMediaViewer();state.providerId=els.providerSelect.value;state.groups=[];state.groupsTotal=0;state.trackedGroupStatus={};startTrackedGroupRefresh();state.selectedGroup='';state.articles=[];state.nameResolutionAttempted.clear();state.nameResolutionFailures.clear();state.nameResolutionDeferred.clear();state.nameResolutionBackoffUntil=0;state.nameResolutionAutoRemaining=24;clearTimeout(state.nameResolutionTimer);state.nameResolutionTimer=null;clearNameResolutionResultRenderQueue();state.selectedArticleKey='';state.selectedItems.clear();state.selectionAnchorKey='';state.articlePage=1;state.articlePaging=null;state.loadedPages.clear();state.articleSearchTerm='';state.searchMode=false;state.groupSearchJob=null;state.continuousLoading=false;state.activeBrowserTabId='';els.articleSearch.value='';state.galleryGeneration++;state.thumbQueue=[];state.thumbQueued.clear();renderBrowserTabs();saveUiSettings();updateProviderState();renderGroups();els.groupsMoreWrap.classList.add('hidden');els.articleTitle.textContent='Articles';els.articleEyebrow.textContent='NO GROUP SELECTED';els.articleSummary.classList.add('hidden');els.entireGroupSearchBtn.disabled=true;updateSelectionBar();updateArticlePaging();updateArticleSearchUi();resetPreview();};
...
function discoverDetailRequestPayload(item,interaction='open'){return {provider:'tmdb',metadata_id:item.metadata_id||item.tmdb_id,tmdb_id:item.tmdb_id,kind:item.kind,title:item.title,year:item.year,interaction}}
function fetchDiscoverDetail(item,{force=false,prefetch=false}={}){if(!item)return Promise.reject(new Error('Discover item is missing'));const key=discoverDetailKey(item),cached=!force&&discoverCachedDetail(item);if(cached)return Promise.resolve(cached);const existing=state.discoverDetailInflight[key];if(existing){if(!prefetch&&existing.prefetch){return existing.promise.then(value=>api('/api/discover/detail',discoverDetailRequestPayload(item,'open'),{timeoutMs:3500,timeoutMessage:'NewzDeck could not record this Discover view.'}).then(d=>{const next=d.item||value;state.discoverDetailCache[key]=next;state.discoverDetailCacheTs[key]=Date.now();return next}).catch(()=>value))}return existing.promise}const interaction=prefetch?'prefetch':'open';const p=api('/api/discover/detail',discoverDetailRequestPayload(item,interaction),{timeoutMs:10000,timeoutMessage:'TMDB title details took too long. NewzDeck stopped waiting so Discover can recover.'}).then(d=>{const value=d.item||item;state.discoverDetailCache[key]=value;state.discoverDetailCacheTs[key]=Date.now();return value}).finally(()=>{if(state.discoverDetailInflight[key]?.promise===p)delete state.discoverDetailInflight[key]});state.discoverDetailInflight[key]={promise:p,prefetch:!!prefetch};return p}
function scheduleDiscoverDetailPrefetch(item,element){if(!item||discoverCachedDetail(item))return;const key=discoverDetailKey(item);clearTimeout(state.discoverDetailPrefetchTimers[key]);state.discoverDetailPrefetchTimers[key]=setTimeout(()=>{delete state.discoverDetailPrefetchTimers[key];if(element&&!element.matches(':hover,:focus-within'))return;if(state.discoverDetailPrefetchActive>=state.discoverDetailPrefetchLimit)return;state.discoverDetailPrefetchActive++;fetchDiscoverDetail(item,{prefetch:true}).catch(()=>{}).finally(()=>{state.discoverDetailPrefetchActive=Math.max(0,state.discoverDetailPrefetchActive-1)})},650)}
...
function renderDiscoverDetailPreview(item){state.discoverCurrentDetail=item;state.discoverPersonReturn=null;$('discoverDetailTitle').textContent=item.title||'Title';$('discoverDetailEyebrow').textContent=item.kind==='tv'?'TV SERIES • TMDB':'MOVIE • TMDB';$('discoverDetailMeta').textContent=[item.year||'',discoverRatingText(item),(item.genres||[]).slice(0,3).join(' • ')].filter(Boolean).join(' • ');const backdrop=$('discoverDetailBackdrop');backdrop.style.backgroundImage=item.backdrop_url?`linear-gradient(180deg,rgba(5,11,18,.12),rgba(5,11,18,.95)),url("${String(item.backdrop_url).replace(/"/g,'')}")`:'';backdrop.classList.toggle('empty',!item.backdrop_url);$('discoverDetailBody').innerHTML=`<div class="discover-detail-main"><div class="discover-detail-poster">${discoverPosterMarkup(item)}${discoverLibraryBadge(item)}</div><div class="discover-detail-copy"><div class="discover-facts">${discoverFacts(item).map(x=>`<span>${escapeHtml(x)}</span>`).join('')}</div><p class="discover-overview">${escapeHtml(item.overview||'Loading the full TMDB overview and title details…')}</p><div class="discover-loading discover-detail-enriching"><span></span><p>Refreshing cast, recommendations, release details, and links in the background…</p></div>${discoverAttribution(item)}</div></div>`}
...
async function openDiscoverDetail(item,opts={}){if(!item)return;const token=++state.discoverDetailToken;$('discoverDetailModal').classList.remove('hidden');const cached=discoverCachedDetail(item);if(cached&&state.automation)renderDiscoverDetail(cached,opts);else renderDiscoverDetailPreview(cached||item);try{const automationReady=state.automation?Promise.resolve(true):loadAutomation({quiet:true,render:false});const detailReady=cached?api('/api/discover/detail',discoverDetailRequestPayload(item,'open'),{timeoutMs:3500,timeoutMessage:'NewzDeck could not refresh this cached Discover view.'}).then(d=>d.item||cached).catch(()=>cached):fetchDiscoverDetail(item);const [,detail]=await Promise.all([automationReady,detailReady]);if(token!==state.discoverDetailToken||$('discoverDetailModal').classList.contains('hidden'))return;const value=detail||cached||item;state.discoverDetailCache[discoverDetailKey(item)]=value;state.discoverDetailCacheTs[discoverDetailKey(item)]=Date.now();renderDiscoverDetail(value,opts)}catch(e){if(token===state.discoverDetailToken)$('discoverDetailBody').innerHTML=discoverEmpty('Could not load title details',e.message)}}
...
function automationSurfaceBusy(){
  ...
  return ['automationAddModal','automationItemModal','manualImportModal','releaseSearchModal','qualityProfileModal','indexerModal'].some(id=>{const el=$(id);return el&&!el.classList.contains('hidden')});
}
...
async function loadAutomation({quiet=false,render=true,background=false}={}){
  ...
  try{
    const next=await api('/api/automation/summary');state.automation=next;state.automationLoadError='';void resumeAutomationScanProgress();renderAutomationBadges();
...
function renderAutomationWanted(){
  ...
  $('automationTitle').textContent='Wanted';$('automationSubtitle').textContent=c.automatic_grab_enabled?'Continuous Automation is watching releases and will queue qualifying media automatically.':'Monitored media that is missing or still has a profile-approved upgrade path.';$('automationAddBtn').classList.add('hidden');$('automationScanBtn').classList.remove('hidden');
...
function renderAutomationHealth(){
  const h=state.automation?.health||{},a=state.automation?.automatic||{},roots=h.roots||[],bl=h.blacklists||[],holds=h.integrity_holds||[],ih=h.indexer_health||[];
  ...
  $('automationContent').innerHTML=`
    <div class="automation-health-grid">
      ...
      <article class="automation-health-card ${Number(h.needs_attention_count||0)?'warning':'good'}"><span>NEEDS ATTENTION</span><strong>${Number(h.needs_attention_count||0)}</strong><small>${Number(h.needs_attention_count||0)?'preserved imports or integrity holds waiting for action':'No blocked imports'}</small></article>
      <article class="automation-health-card ${holds.length?'attention':'good'}"><span>INTEGRITY HOLDS</span><strong>${holds.length}</strong><small>${holds.length?'automatic search paused for manual review':'No repeated duplicate-media targets'}</small></article>
    ...
    ${holds.length?`<section class="automation-health-panel automation-blacklist-panel"><div class="automation-section-head"><div><div class="eyebrow">INTEGRITY HOLD</div><h2>Automatic Search Paused</h2><p>Two or more distinct releases produced the same conflicting media fingerprint. NewzDeck stopped unattended retries to avoid wasting bandwidth. Review the existing episode and use Interactive Search or Retry Import after correcting the source/library state.</p></div></div><div class="automation-blacklist-list">${holds.map(x=>`<div class="automation-blacklist-row"><div><strong>${escapeHtml(x.target_label||x.target_key||'Automation target')}</strong><span>${escapeHtml(x.message||'Repeated duplicate-media conflict')}</span><small>${Number(x.distinct_releases||0)} distinct releases • fingerprint ${escapeHtml(x.fingerprint||'unknown')} • ${x.updated_ts?new Date(Number(x.updated_ts)*1000).toLocaleString():''}</small></div></div>`).join('')}</div></section>`:''}
...
function automationStorageSetupCard(){
  const storage=state.automation?.storage||{},roots=Array.isArray(storage.roots)?storage.roots:[],lowRoots=Array.isArray(storage.low_roots)?storage.low_roots:[];
  const reserveGb=Number(storage.reserve_gb??state.automation?.config?.automatic_storage_reserve_gb??5),reservePercent=Number(storage.reserve_percent??state.automation?.config?.automatic_storage_reserve_percent??5);
  const summary=lowRoots.length?`⚠ ${lowRoots.length} media root${lowRoots.length===1?' is':'s are'} below the configured reserve`:roots.length?'✓ All online media roots meet the configured reserve':'No media roots configured';
  const rows=roots.length?roots.map(r=>{const online=!!r.online,reserveOk=!!r.reserve_ok,status=!online?'OFFLINE':reserveOk?'RESERVE OK':'BELOW RESERVE',kind=String(r.kind||'media').toUpperCase(),free=Number(r.free||0),freePercent=Number(r.free_percent||0),reserve=Number(r.reserve||0);return `<div class="automation-health-row"><span class="health-dot ${online&&reserveOk?'good':'bad'}"></span><div><strong>${escapeHtml(kind)} • ${escapeHtml(r.path||'')}</strong><small>${online?`${free>0?formatBytes(free):'0 B'} free • ${freePercent.toFixed(1)}% free • reserve ${reserve>0?formatBytes(reserve):'0 B'}`:'Root folder is offline or unavailable'}</small></div><b>${status}</b></div>`}).join(''):'<p class="metadata-provider-note">Add a TV or Movie Root Folder to monitor free space.</p>';
  return `<article class="automation-setup-card automation-storage-setup-card"><div class="eyebrow">STORAGE SAFETY</div><h2>Media drive reserve health</h2><p>Continuous Automation uses the larger of the configured GB minimum and percentage reserve before unattended grabs and Smart Import.</p><div class="setup-status ${lowRoots.length?'warning':roots.length?'good':''}" id="automaticStorageHealthStatus">${summary}</div><div class="automation-health-list">${rows}</div><small class="metadata-provider-note">Current policy: ${reserveGb} GB minimum • ${reservePercent?`${reservePercent}% of drive`:'percentage reserve off'}.</small></article>`;
}
const integrityReviewState={audit:null};
function automationIntegritySetupCard(){
  const a=integrityReviewState.audit,count=Number(a?.needs_review_count||0),known=!!a;
  const summary=known?(count?`⚠ ${count} finding${count===1?'':'s'} need review`:`✓ No current integrity findings`):'Run a read-only audit of current library assignments';
  return `<article class="automation-setup-card automation-integrity-setup-card"><div class="eyebrow">LIBRARY INTEGRITY</div><h2>Needs Review</h2><p>Compare stored release provenance and file fingerprints against the monitored title. Nothing is deleted or moved automatically.</p><div class="setup-status ${known?(count?'warning':'good'):''}" id="libraryIntegrityStatus">${summary}</div><div class="setup-card-actions"><button class="primary-btn" id="libraryIntegrityReviewBtn" type="button">${known?'Review findings':'Run integrity review'}</button></div><small class="metadata-provider-note">Mark Missing clears only NewzDeck's association and excludes that exact reviewed file fingerprint from immediate re-attachment. The media file stays on disk.</small></article>`;
}
function integrityReviewKey(r){return `${String(r?.item_id||'')}|${r?.season??''}|${r?.episode??''}|${String(r?.path||'').toLowerCase()}`}
function integrityReviewEntries(audit){
  const map=new Map();
  const add=(row,reason,type)=>{if(!row?.item_id||!row?.path)return;const key=integrityReviewKey(row),rec=map.get(key)||{...row,reasons:[],types:[]};if(reason&&!rec.reasons.includes(reason))rec.reasons.push(reason);if(type&&!rec.types.includes(type))rec.types.push(type);map.set(key,rec)};
  (audit?.identity_mismatch_examples||[]).forEach(r=>add(r,r.reason||'Stored release identity does not match this series.','identity'));
  (audit?.edition_mismatch_examples||[]).forEach(r=>add(r,`Country/edition mismatch: expected ${r.expected_country||'—'}, observed ${r.observed_country||'—'}.`,'edition'));
  return [...map.values()];
}
function integrityEpisodeLabel(r){return r?.season!=null?`S${String(Number(r.season||0)).padStart(2,'0')}E${String(Number(r.episode||0)).padStart(2,'0')}`:''}
function renderIntegrityReview(audit){
  integrityReviewState.audit=audit||{};const entries=integrityReviewEntries(audit),dupes=Array.isArray(audit?.cross_title_duplicates)?audit.cross_title_duplicates:[],episodeDupes=Array.isArray(audit?.same_title_cross_episode_duplicates)?audit.same_title_cross_episode_duplicates:[];
  const head=`<div class="automation-section-head"><div><div class="eyebrow">READ-ONLY AUDIT</div><h2>${Number(audit?.needs_review_count||0)} finding${Number(audit?.needs_review_count||0)===1?'':'s'} need review</h2><p>${Number(audit?.file_records||0).toLocaleString()} library file records checked • ${Number(audit?.identity_mismatches||0)} identity mismatch${Number(audit?.identity_mismatches||0)===1?'':'es'} • ${Number(audit?.cross_title_duplicate_fingerprints||0)} cross-title duplicate fingerprint${Number(audit?.cross_title_duplicate_fingerprints||0)===1?'':'s'} • ${Number(audit?.same_title_cross_episode_duplicate_fingerprints||0)} same-series episode duplicate${Number(audit?.same_title_cross_episode_duplicate_fingerprints||0)===1?'':'s'}</p></div><button class="secondary-btn" id="integrityReviewRefresh" type="button">↻ Refresh audit</button></div>`;
  const rows=entries.length?entries.map(r=>{const ep=integrityEpisodeLabel(r),release=String(r.release_title||r.identity_probe||''),reason=(r.reasons||[]).join(' • ');return `<article class="automation-card"><div class="automation-card-head"><div><b>${escapeHtml(r.title||'Library item')}${ep?` • ${escapeHtml(ep)}`:''}</b><small>${escapeHtml(reason)}</small></div><span class="service-status-badge error">REVIEW</span></div><div class="settings-callout warning"><b>Original release / identity</b><span>${escapeHtml(release||'No retained release provenance; finding is based on current library identity.')}</span></div><div class="settings-callout"><b>Current file</b><span>${escapeHtml(r.path||'')}</span></div><div class="setup-card-actions"><button class="secondary-btn compact" data-integrity-open="${escapeHtml(integrityReviewKey(r))}" type="button">Open folder</button><button class="danger-lite" data-integrity-missing="${escapeHtml(integrityReviewKey(r))}" type="button">Mark Missing (keep file)</button></div></article>`}).join(''):`<div class="automation-empty"><strong>No identity mismatches</strong><span>The strict series audit found no mismatched stored release/file identities.</span></div>`;
  const duplicateRows=dupes.length?`<div class="automation-section-head"><div><div class="eyebrow">CROSS-TITLE DUPLICATES</div><h2>${dupes.length} shared fingerprint group${dupes.length===1?'':'s'}</h2><p>Review which library target legitimately owns the physical file before clearing an association.</p></div></div>${dupes.map(g=>`<article class="automation-card"><div class="automation-card-head"><div><b>Same physical fingerprint assigned to multiple titles</b><small>${escapeHtml(g.fingerprint||'')}</small></div><span class="service-status-badge warning">DUPLICATE</span></div>${(g.targets||[]).map(t=>`<div class="automation-health-row"><span class="health-dot bad"></span><div><strong>${escapeHtml(t.title||'')}${t.season!=null?` • ${escapeHtml(integrityEpisodeLabel(t))}`:''}</strong><small>${escapeHtml(t.path||'')}</small></div><div class="setup-card-actions"><button class="secondary-btn compact" data-integrity-open="${escapeHtml(integrityReviewKey(t))}" type="button">Open</button><button class="danger-lite" data-integrity-missing="${escapeHtml(integrityReviewKey(t))}" type="button">Mark Missing</button></div></div>`).join('')}</article>`).join('')}`:'';
  const episodeDuplicateRows=episodeDupes.length?`<div class="automation-section-head"><div><div class="eyebrow">SAME-SERIES EPISODE DUPLICATES</div><h2>${episodeDupes.length} suspicious fingerprint group${episodeDupes.length===1?'':'s'}</h2><p>Identical media bytes exist as separate physical files assigned to different episodes of the same series. Review the episode associations before clearing one.</p></div></div>${episodeDupes.map(g=>`<article class="automation-card"><div class="automation-card-head"><div><b>Identical media assigned to different episodes</b><small>${escapeHtml(g.fingerprint||'')}</small></div><span class="service-status-badge warning">DUPLICATE</span></div>${(g.targets||[]).map(t=>`<div class="automation-health-row"><span class="health-dot bad"></span><div><strong>${escapeHtml(t.title||'')}${t.season!=null?` • ${escapeHtml(integrityEpisodeLabel(t))}`:''}</strong><small>${escapeHtml(t.path||'')}</small></div><div class="setup-card-actions"><button class="secondary-btn compact" data-integrity-open="${escapeHtml(integrityReviewKey(t))}" type="button">Open</button><button class="danger-lite" data-integrity-missing="${escapeHtml(integrityReviewKey(t))}" type="button">Mark Missing</button></div></div>`).join('')}</article>`).join('')}`:'';
  $('integrityReviewBody').innerHTML=head+rows+duplicateRows+episodeDuplicateRows;
  const allRows=new Map(entries.map(r=>[integrityReviewKey(r),r]));for(const g of [...dupes,...episodeDupes])for(const t of g.targets||[])if(!allRows.has(integrityReviewKey(t)))allRows.set(integrityReviewKey(t),t);
  $('integrityReviewRefresh').onclick=()=>loadIntegrityReview(true);
  document.querySelectorAll('[data-integrity-open]').forEach(b=>b.onclick=async()=>{const r=allRows.get(b.dataset.integrityOpen);if(!r)return;const done=beginButtonActivity(b,'Opening…');try{await api('/api/automation/library/integrity/open-folder',{item_id:r.item_id,season:r.season,episode:r.episode,path:r.path})}catch(e){toast(e.message,'error')}finally{done()}});
  document.querySelectorAll('[data-integrity-missing]').forEach(b=>b.onclick=async()=>{const r=allRows.get(b.dataset.integrityMissing);if(!r)return;const ep=integrityEpisodeLabel(r),label=`${r.title||'this item'}${ep?` ${ep}`:''}`;if(!confirm(`Mark ${label} as Missing in NewzDeck?\n\nThe media file will NOT be deleted, moved, or renamed. NewzDeck will clear only this library association and exclude this exact reviewed file fingerprint from immediate re-attachment.`))return;const done=beginButtonActivity(b,'Updating…');try{await api('/api/automation/library/integrity/mark-missing',{item_id:r.item_id,season:r.season,episode:r.episode,path:r.path});toast('Association cleared. The media file was left untouched.','success');await loadAutomation({quiet:true,render:false});await loadIntegrityReview(true)}catch(e){toast(e.message,'error')}finally{done()}});
}
async function loadIntegrityReview(refresh=false){
  $('integrityReviewBody').innerHTML='<div class="release-loading">Auditing library identity and fingerprints…</div>';
  try{const audit=await api('/api/automation/library/integrity-audit');renderIntegrityReview(audit);if(state.activeView==='automation'&&state.automationTab==='setup')renderAutomationSetup()}catch(e){$('integrityReviewBody').innerHTML=automationEmpty('!','Integrity audit failed',e.message)}
}
function openLibraryIntegrityReview(){$('integrityReviewModal').classList.remove('hidden');loadIntegrityReview(true)}
...
function renderAutomationSetup(){
  ...
  $('automationContent').innerHTML=`<div class="automation-setup-grid">
    ...
    ${automationStorageSetupCard()}
    ${automationIntegritySetupCard()}
    <article class="automation-setup-card automation-auto-card"><div class="eyebrow">CONTINUOUS AUTOMATION</div><h2>Always-on Monitoring</h2><p>NewzDeck can watch recent Newznab RSS releases every few minutes, discover newly aired episodes through metadata refreshes, fall back to scheduled Wanted searches, automatically retry misses, and queue acceptable releases without opening the app.</p>
      ...
      <div class="automation-template-grid continuous-settings-grid">
        ...
        <label>Storage safety reserve<select id="automaticStorageReserveGb">${[2,5,10,20,50].map(n=>`<option value="${n}" ${Number(c.automatic_storage_reserve_gb||5)===n?'selected':''}>${n} GB minimum</option>`).join('')}</select><small>Absolute minimum free space kept on the destination drive.</small></label>
        <label>Drive free-space reserve<select id="automaticStorageReservePercent">${[0,1,3,5,10].map(n=>`<option value="${n}" ${Number(c.automatic_storage_reserve_percent??5)===n?'selected':''}>${n?`${n}% of drive`:'Off'}</option>`).join('')}</select><small>NewzDeck uses the larger of this percentage and the GB minimum before unattended grabs/imports.</small></label>
  ...
  $('libraryIntegrityReviewBtn').onclick=openLibraryIntegrityReview;
...
async function savePlexOrganization(){
  ...
  const done=beginButtonActivity($('savePlexOrganizationBtn'),'Saving import settings…');
  try{await api('/api/automation/config/save',data);await loadAutomation();renderAutomation();toast('Smart Import settings saved.','success')}catch(e){toast(e.message,'error')}finally{done()}
}
...
function syncSelectedEpisodeSelectionUi(item){if(automationMonitoringMode(item)!=='selected')return;document.querySelectorAll('[data-season-monitor]').forEach(c=>{const sn=String(c.dataset.seasonMonitor||''),boxes=[...document.querySelectorAll('[data-episode-monitor]')].filter(x=>String(x.dataset.episodeMonitor||'').startsWith(sn+':')),checked=boxes.filter(x=>x.checked).length;c.checked=boxes.length>0&&checked===boxes.length;c.indeterminate=checked>0&&checked<boxes.length;const count=document.querySelector(`[data-season-selection-count="${sn}"]`);if(count)count.textContent=`${checked}/${boxes.length} selected`})}
...
async function openAutomationItem(id){const item=autoItem(id);if(!item)return;$('automationItemTitle').textContent=item.title;$('automationItemEyebrow').textContent=item.kind==='tv'?'TV SERIES':'MOVIE';$('automationItemMeta').textContent=`${item.year||''}${item.status?` • ${item.status}`:''}`;const profiles=state.automation?.profiles||[];const roots=item.kind==='tv'?(state.automation?.config?.tv_roots||[]):state.automation?.config?.movie_roots||[];let body=`<div class="automation-item-hero">${item.poster_url?`<img src="${escapeHtml(item.poster_url)}" alt="">`:'<div class="automation-poster-placeholder large">MEDIA</div>'}<div><p>${escapeHtml(item.overview||'No description available.')}</p><div class="automation-item-facts">${[item.rating?`★ ${Number(item.rating).toFixed(1)}`:'',(item.genres||[]).slice(0,3).join(' • '),item.network||'',item.runtime?`${item.runtime} min`:''].filter(Boolean).map(x=>`<span>${escapeHtml(x)}</span>`).join('')}</div><div class="automation-item-settings ${item.kind==='tv'?'tv-settings':'movie-settings'}"><label class="auto-item-monitor-field">Monitoring type<select id="autoItemMonitorMode">${automationMonitoringOptions(item.kind,automationMonitoringMode(item))}</select><small id="autoItemMonitorHelp">${escapeHtml(automationMonitoringHelp(item.kind,automationMonitoringMode(item)))}</small></label><label class="auto-item-profile-field">Quality profile<select id="autoItemProfile">${profiles.map(p=>`<option value="${escapeHtml(p.id)}" ${p.id===item.quality_profile_id?'selected':''}>${escapeHtml(p.name)}</option>`).join('')}</select></label><label class="auto-item-root-field">Root folder<select id="autoItemRoot"><option value="">Automatic root (first configured)</option>${roots.map(r=>`<option value="${escapeHtml(r)}" ${r===item.root_folder?'selected':''}>${escapeHtml(r)}</option>`).join('')}</select></label>${item.kind==='tv'?`<label class="auto-item-library-field">Library name<input id="autoItemLibraryTitle" value="${escapeHtml(String(item.library_title_source||'').toLowerCase()==='manual'?(item.library_title||''):'')}" placeholder="Automatic series name"><small>Automatic uses the show title only (for example Silo). Same-title country editions are disambiguated when needed (for example Big Brother (US)). Enter a value only to override automatic naming.</small></label>`:''}</div><div class="automation-item-actions"><button class="secondary-btn" id="autoItemSave">Save</button><button class="secondary-btn" id="autoItemScan">Scan files</button><button class="secondary-btn" id="autoItemRefresh">Refresh metadata</button><button class="secondary-btn" id="autoItemOpenFolder">Open folder</button>${item.kind==='tv'?'<button class="primary-btn" id="autoItemImport">Import Media</button>':''}${item.kind==='movie'?'<button class="primary-btn" id="autoItemSearch">Search releases</button>':''}<button class="danger-btn" id="autoItemDelete">Remove</button></div><div class="automation-item-activity hidden" id="autoItemActivity" role="status" aria-live="polite"><span class="automation-item-activity-spinner" aria-hidden="true"></span><div style="flex:1;min-width:0"><strong id="autoItemActivityTitle">Working…</strong><small id="autoItemActivityDetail"></small><div class="download-progress-track hidden" id="autoItemScanProgressTrack" style="margin-top:8px"><div class="download-progress-fill" id="autoItemScanProgressFill" style="width:0%"></div></div><small id="autoItemScanProgressMeta"></small></div></div></div></div>`;
 ...
 $('automationItemBody').innerHTML=body;$('automationItemModal').classList.remove('hidden');$('autoItemSave').onclick=()=>saveAutomationItem(item);$('autoItemScan').onclick=()=>scanAutomationLibrary(item.id);$('autoItemRefresh').onclick=()=>refreshAutomationItem(item.id);$('autoItemOpenFolder').onclick=()=>openAutomationFolder(item.id);$('autoItemDelete').onclick=()=>deleteAutomationItem(item);$('autoItemImport')?.addEventListener('click',()=>openManualMediaImport(item));$('autoItemSearch')?.addEventListener('click',()=>openReleaseSearch(item.id));const monitorSelect=$('autoItemMonitorMode');if(monitorSelect)monitorSelect.onchange=()=>{const help=$('autoItemMonitorHelp'),mode=monitorSelect.value,current=automationMonitoringMode(item);if(help)help.textContent=automationMonitoringHelp(item.kind,mode)+(mode==='selected'&&current!=='selected'?' Click Save first; then choose seasons or episodes below.':'');const waitForSave=mode==='selected'&&current!=='selected';document.querySelectorAll('[data-season-monitor],[data-episode-monitor]').forEach(c=>c.disabled=waitForSave)};document.querySelectorAll('[data-season-monitor]').forEach(c=>c.onchange=async e=>{e.stopPropagation();try{if(automationMonitoringMode(item)==='selected'){const sn=String(c.dataset.seasonMonitor||'');document.querySelectorAll('[data-episode-monitor]').forEach(x=>{if(String(x.dataset.episodeMonitor||'').startsWith(sn+':'))x.checked=c.checked});c.indeterminate=false}await api('/api/automation/media/update',{id:item.id,season_number:Number(c.dataset.seasonMonitor),season_monitored:c.checked});syncSelectedEpisodeSelectionUi(item);void loadAutomation({quiet:true,background:true})}catch(err){toast(err.message,'error')}});document.querySelectorAll('[data-episode-monitor]').forEach(c=>c.onchange=async()=>{const [season,episode]=c.dataset.episodeMonitor.split(':');try{await api('/api/automation/media/update',{id:item.id,season_number:Number(season),episode_number:Number(episode),episode_monitored:c.checked});syncSelectedEpisodeSelectionUi(item);void loadAutomation({quiet:true,background:true})}catch(e){toast(e.message,'error')}});syncSelectedEpisodeSelectionUi(item);wireReleaseSearchButtons();if(libraryScanState.lastJob&&['queued','running'].includes(String(libraryScanState.lastJob.status||'')))renderAutomationScanProgress(libraryScanState.lastJob)}
const manualImportState={itemId:'',preview:null,jobId:'',pollTimer:null,importing:false};
const libraryScanState={jobId:'',itemId:'',pollTimer:null,lastJob:null,resumeChecked:false,buttonDone:null,hideTimer:null};
function manualImportItem(){return autoItem(manualImportState.itemId)}
function manualImportSeasonRows(item){return (item?.seasons||[]).filter(s=>Array.isArray(s.episodes)&&s.episodes.length).slice().sort((a,b)=>Number(a.season_number)-Number(b.season_number))}
function resetManualImportProgress(){if(manualImportState.pollTimer){clearTimeout(manualImportState.pollTimer);manualImportState.pollTimer=null}manualImportState.jobId='';manualImportState.importing=false;const box=$('manualImportProgress');box?.classList.add('hidden');box?.classList.remove('is-complete','is-failed');if($('manualImportProgressFill'))$('manualImportProgressFill').style.width='0%';if($('manualImportProgressPercent'))$('manualImportProgressPercent').textContent='0%';if($('manualImportProgressStatus'))$('manualImportProgressStatus').textContent='Preparing import…';if($('manualImportProgressDetail'))$('manualImportProgressDetail').textContent='NewzDeck will show live Smart Import progress here.';if($('manualImportClose'))$('manualImportClose').disabled=false}
function setManualImportBusy(busy){manualImportState.importing=!!busy;['manualImportMode','manualImportSeason','manualImportEpisode','manualImportSource','manualImportBrowse','manualImportInspect'].forEach(id=>{const el=$(id);if(el)el.disabled=!!busy});if($('manualImportClose'))$('manualImportClose').disabled=!!busy}
function renderManualImportProgress(job){const box=$('manualImportProgress');if(!box)return;box.classList.remove('hidden','is-complete','is-failed');const status=String(job?.status||'running'),pct=Math.max(0,Math.min(100,Number(job?.progress||0)));if(status==='completed')box.classList.add('is-complete');if(status==='failed')box.classList.add('is-failed');$('manualImportProgressFill').style.width=`${pct}%`;$('manualImportProgressPercent').textContent=`${Math.round(pct)}%`;let heading='Importing & organizing media';if(status==='queued')heading='Import queued';else if(status==='completed')heading='Import complete';else if(status==='failed')heading='Import failed';$('manualImportProgressStatus').textContent=heading;$('manualImportProgressDetail').textContent=String(job?.message||job?.error||'Smart Import is organizing the selected media.')}
function refreshManualImportEpisodeOptions(){
  const item=manualImportItem(),season=Number($('manualImportSeason')?.value||0),episodes=(manualImportSeasonRows(item).find(s=>Number(s.season_number)===season)?.episodes||[]).slice().sort((a,b)=>Number(a.episode_number)-Number(b.episode_number));
  const select=$('manualImportEpisode'),prior=Number(select?.value||0);if(!select)return;
  select.innerHTML=episodes.map(ep=>`<option value="${Number(ep.episode_number)}">E${String(Number(ep.episode_number)).padStart(2,'0')} • ${escapeHtml(ep.name||'Episode')}</option>`).join('');if(prior&&episodes.some(ep=>Number(ep.episode_number)===prior))select.value=String(prior)
}
function refreshManualImportMode(){const episodeMode=$('manualImportMode')?.value==='episode';$('manualImportEpisodeField')?.classList.toggle('hidden',!episodeMode);refreshManualImportEpisodeOptions();manualImportState.preview=null;$('manualImportCommit').disabled=true;$('manualImportPreview').innerHTML='<div class="manual-import-hint">Choose the external media folder, then preview exactly how NewzDeck will rename, move, and reconcile the files.</div>';resetManualImportProgress()}
function openManualMediaImport(item){
  if(!item||item.kind!=='tv')return;manualImportState.itemId=String(item.id||'');manualImportState.preview=null;resetManualImportProgress();setManualImportBusy(false);
  $('manualImportTitle').textContent=`Import Media · ${item.title}`;$('manualImportSubtitle').textContent='Import an externally obtained season or episode through NewzDeck Smart Import. Nothing moves until you review the preview.';
  const seasons=manualImportSeasonRows(item),wanted=[...(state.automation?.wanted?.upgrades||[]),...(state.automation?.wanted?.missing||[])].find(x=>String(x.item_id||'')===String(item.id||''));
  $('manualImportSeason').innerHTML=seasons.map(sr=>`<option value="${Number(sr.season_number)}">Season ${Number(sr.season_number)}</option>`).join('');
  if(wanted?.season!=null&&seasons.some(sr=>Number(sr.season_number)===Number(wanted.season)))$('manualImportSeason').value=String(Number(wanted.season));
  $('manualImportMode').value='season';$('manualImportSource').value=localStorage.getItem('newzdeckManualImportSource')||'';refreshManualImportMode();
  $('manualImportCommit').textContent='Import & Organize';$('manualImportModal').classList.remove('hidden');
}
function closeManualMediaImport(){if(manualImportState.importing){toast('Manual Import is still running. Keep this window open to see its progress.','info');return}$('manualImportModal').classList.add('hidden');manualImportState.preview=null;resetManualImportProgress()}
function invalidateManualImportPreview(message='Selection changed. Preview the import again before committing.'){if(manualImportState.importing)return;manualImportState.preview=null;resetManualImportProgress();const commit=$('manualImportCommit');if(commit){commit.disabled=true;commit.textContent='Import & Organize'}const preview=$('manualImportPreview');if(preview)preview.innerHTML=`<div class="manual-import-hint">${escapeHtml(message)}</div>`}
async function browseManualImportSource(){const b=$('manualImportBrowse'),old=b.textContent;b.disabled=true;b.textContent='Opening…';try{const r=await api('/api/automation/manual-import/choose-source',{initial:$('manualImportSource').value||localStorage.getItem('newzdeckManualImportSource')||'',title:`Choose folder containing media for ${manualImportItem()?.title||'this TV show'}`});if(r.cancelled)return;$('manualImportSource').value=r.folder||'';if(r.folder)localStorage.setItem('newzdeckManualImportSource',r.folder);invalidateManualImportPreview('Folder selected. Preview the import before committing any changes.')}catch(e){toast(e.message,'error')}finally{b.disabled=false;b.textContent=old}}
function manualImportPayload(){return {item_id:manualImportState.itemId,source_folder:$('manualImportSource').value.trim(),season:Number($('manualImportSeason').value),episode:$('manualImportMode').value==='episode'?Number($('manualImportEpisode').value):null}}
function renderManualImportPreview(d){
  manualImportState.preview=d;resetManualImportProgress();const rows=d.inspection||[],c=d.counts||{},cutoff=d.cutoff||'profile cutoff';
  const summary=`<div class="manual-import-summary"><div><span>VIDEO FILES</span><strong>${Number(d.files_found||0)}</strong></div><div><span>IMPORT</span><strong>${Number(c.IMPORT||0)}</strong></div><div><span>UPGRADE</span><strong>${Number(c.UPGRADE||0)}</strong></div><div><span>KEEP EXISTING</span><strong>${Number(c.KEEP_EXISTING||0)+Number(c.DUPLICATE||0)}</strong></div><div><span>NEEDS REVIEW</span><strong>${Number(c.NEEDS_ATTENTION||0)}</strong></div></div>`;
  const table=rows.length?`<div class="manual-import-table">${rows.map(x=>{const action=String(x.action||'REVIEW'),mapped=!['IGNORE','NEEDS_ATTENTION'].includes(action),result=mapped?(x.cutoff_met?`Cutoff met (${escapeHtml(x.result_quality||x.quality||'Unknown')})`:`Still below ${escapeHtml(cutoff)} (${escapeHtml(x.result_quality||x.quality||'Unknown')})`):'';return `<div class="manual-import-row action-${action.toLowerCase().replace(/_/g,'-')}"><div><strong>${escapeHtml(String(x.source||'').split(/[\\/]/).pop()||'Media file')}</strong><small>${escapeHtml(x.identified||'Unknown media')}</small></div><div><b>${escapeHtml(action)}</b><small>${escapeHtml(x.reason||'')}</small></div><div><span>${escapeHtml(x.quality||'—')}</span>${x.current_quality?`<small>Current: ${escapeHtml(x.current_quality)}</small>`:''}</div><div><span title="${escapeHtml(x.destination||'')}">${escapeHtml(x.destination||'—')}</span>${result?`<small class="${x.cutoff_met?'cutoff-ok':'cutoff-wait'}">${result}</small>`:''}</div></div>`}).join('')}</div>`:'<div class="manual-import-hint">No video files were mapped.</div>';
  $('manualImportPreview').innerHTML=(d.ok?'':`<div class="manual-import-warning"><strong>Import needs attention</strong><span>${escapeHtml(d.reason||'NewzDeck could not build a safe import plan.')}</span></div>`)+summary+table;
  $('manualImportCommit').disabled=!d.ok||!d.can_import||Number(c.NEEDS_ATTENTION||0)>0;
}
async function previewManualMediaImport(){const payload=manualImportPayload();if(!payload.source_folder){toast('Choose the folder containing the external TV media first.','error');return}const b=$('manualImportInspect'),old=b.textContent;b.disabled=true;b.textContent='Inspecting…';$('manualImportCommit').disabled=true;try{const d=await api('/api/automation/manual-import/preview',payload);renderManualImportPreview(d)}catch(e){$('manualImportPreview').innerHTML=`<div class="manual-import-warning"><strong>Preview failed</strong><span>${escapeHtml(e.message)}</span></div>`;toast(e.message,'error')}finally{b.disabled=false;b.textContent=old}}
async function finishManualImportJob(job){setManualImportBusy(false);manualImportState.jobId='';manualImportState.preview=null;const b=$('manualImportCommit');if(job.status==='completed'){const d=job.result||{},moved=Number(d.imported_count||0),kept=Number(d.kept_existing||0),satisfied=Number(d.wanted_satisfied_count||0),remaining=Number(d.wanted_remaining_count||0);renderManualImportProgress({...job,progress:100,message:`${moved} file${moved===1?'':'s'} organized${kept?` • ${kept} existing kept`:''} • ${satisfied} Wanted target${satisfied===1?'':'s'} satisfied${remaining?` • ${remaining} still Wanted`:''}`});b.disabled=true;b.textContent='Import Complete';await loadAutomation({quiet:true});toast(`Manual Import complete • ${moved} file${moved===1?'':'s'} organized${satisfied?` • ${satisfied} Wanted target${satisfied===1?'':'s'} satisfied`:''}.`,'success');return}renderManualImportProgress({...job,status:'failed',message:job.error||job.message||'Manual Media Import could not be completed.'});b.disabled=false;b.textContent='Import & Organize';toast(job.error||'Manual Media Import could not be completed.','error')}
...
async function refreshAutomationItem(id){const done=beginButtonActivity($('autoItemRefresh'),'Refreshing metadata…');setAutomationItemActivity('Refreshing metadata…','Updating title, season, episode, artwork, and release metadata for this library item.');try{await api('/api/automation/media/refresh',{id});await loadAutomation({quiet:true});await openAutomationItem(id);toast('Metadata refreshed.','success')}catch(e){toast(e.message,'error')}finally{done();setAutomationItemActivity('')}}
...
async function saveAutomationItem(item){
  const previousMode=automationMonitoringMode(item),mode=$('autoItemMonitorMode')?.value||previousMode;
  ...
  try{
    ...
    if(item.kind==='tv'&&mode==='selected'&&previousMode!=='selected')await openAutomationItem(item.id);
...
async function deleteAutomationItem(item){if(!confirm(`Remove ${item.title} from NewzDeck monitoring? Your media files will not be deleted.`))return;const done=beginButtonActivity($('autoItemDelete'),'Removing…');try{await api('/api/automation/media/delete',{id:item.id});$('automationItemModal').classList.add('hidden');await loadAutomation();toast('Removed from monitoring.')}catch(e){toast(e.message,'error')}finally{done()}}
function automationScanPhaseLabel(job){const p=String(job?.phase||'starting');return ({queued:'Scan queued',waiting:'Waiting for current scan',starting:'Preparing scan',discovering:'Discovering files',matching:'Scanning files',metadata:'Reading media metadata',reconciling:'Reconciling library',saving:'Saving library state',complete:'Scan complete',failed:'Scan failed'})[p]||'Scanning library'}
function automationScanMeta(job){const parts=[],items=Number(job?.items_total||0),done=Number(job?.items_done||0),cf=Number(job?.current_item_file||0),ct=Number(job?.current_item_files||0),found=Number(job?.files_discovered||0),matched=Number(job?.matched||0),root=String(job?.current_root||''),eta=job?.eta_seconds==null?null:Number(job.eta_seconds);if(items)parts.push(`${Math.min(items,done+1)} / ${items} title${items===1?'':'s'}`);if(ct)parts.push(`${Math.min(cf,ct).toLocaleString()} / ${ct.toLocaleString()} files in current title`);else if(found)parts.push(`${found.toLocaleString()} media files discovered`);if(root)parts.push(root);if(matched)parts.push(`${matched.toLocaleString()} matched`);if(Number(job?.changes_found||0))parts.push(`${Number(job.changes_found)} change${Number(job.changes_found)===1?'':'s'} found`);if(eta!=null&&eta>0)parts.push(`ETA ${formatEta(eta)}`);else if(job?.determinate&&String(job?.status)==='running')parts.push('Estimating time remaining…');return parts.join(' • ')}
function syncAutomationScanButtons(job){const active=['queued','running'].includes(String(job?.status||''));const full=$('automationScanBtn');if(full){full.disabled=active;full.classList.toggle('is-busy',active);full.textContent=active?'Scanning library…':'↻ Scan library'}const item=$('autoItemScan');if(item){item.disabled=active;item.classList.toggle('is-busy',active);item.textContent=active?'Scanning files…':'Scan files'}}
function renderAutomationScanProgress(job){if(!job)return;libraryScanState.lastJob=job;const box=$('automationScanProgress'),pct=Math.max(0,Math.min(100,Number(job.progress_percent||0))),determinate=job.determinate!==false,status=String(job.status||'running');if(box){box.classList.remove('hidden');$('automationScanProgressTitle').textContent=automationScanPhaseLabel(job);$('automationScanProgressDetail').textContent=String(job.detail||'Scanning Automation library…');$('automationScanProgressPercent').textContent=determinate?`${Math.round(pct)}%`:'Working…';$('automationScanProgressFill').style.width=`${determinate?pct:Math.max(10,Math.min(28,Number(job.files_discovered||0)?18:12))}%`;$('automationScanProgressMeta').textContent=automationScanMeta(job)||'NewzDeck is measuring real scan progress.';box.classList.toggle('is-complete',status==='completed');box.classList.toggle('is-failed',status==='failed')}syncAutomationScanButtons(job);const itemBox=$('autoItemActivity');if(itemBox&&['queued','running'].includes(status)){setAutomationItemActivity(automationScanPhaseLabel(job),`${String(job.detail||'')}${automationScanMeta(job)?` • ${automationScanMeta(job)}`:''}`);const track=$('autoItemScanProgressTrack'),fill=$('autoItemScanProgressFill'),meta=$('autoItemScanProgressMeta');track?.classList.remove('hidden');if(fill)fill.style.width=`${determinate?pct:15}%`;if(meta)meta.textContent=determinate?`${Math.round(pct)}%${job.eta_seconds?` • ETA ${formatEta(Number(job.eta_seconds))}`:''}`:'Discovering files…'}}
async function finishAutomationScan(job){if(libraryScanState.pollTimer){clearTimeout(libraryScanState.pollTimer);libraryScanState.pollTimer=null}libraryScanState.jobId='';if(libraryScanState.buttonDone){libraryScanState.buttonDone();libraryScanState.buttonDone=null}syncAutomationScanButtons(job);if(job.status==='failed'){toast(job.error||'Library scan failed.','error');return}const d=job.result||{},offline=(d.offline_roots||[]).length,changes=(d.changes||[]).length;await loadAutomation({quiet:true});if(libraryScanState.itemId)await openAutomationItem(libraryScanState.itemId);toast(`Scan complete: ${Number(d.matched||job.matched||0)} media file${Number(d.matched||job.matched||0)===1?'':'s'} matched${changes?` • ${changes} change${changes===1?'':'s'}`:''}${offline?` • ${offline} root${offline===1?'':'s'} offline`:''}.`,offline?'warning':'success');libraryScanState.itemId='';if(libraryScanState.hideTimer)clearTimeout(libraryScanState.hideTimer);libraryScanState.hideTimer=setTimeout(()=>{$('automationScanProgress')?.classList.add('hidden')},8000)}
async function pollAutomationScanProgress(jobId){if(!jobId||libraryScanState.jobId!==jobId)return;try{const job=await api(`/api/automation/library/scan/progress?job_id=${encodeURIComponent(jobId)}`,null,{timeoutMs:5000,timeoutMessage:'Library scan is still running, but this progress refresh timed out.'});if(libraryScanState.jobId!==jobId)return;renderAutomationScanProgress(job);if(['completed','failed'].includes(String(job.status||''))){await finishAutomationScan(job);return}}catch(e){const detail=$('automationScanProgressDetail');if(detail)detail.textContent=`Progress refresh delayed • ${e.message}`}libraryScanState.pollTimer=setTimeout(()=>pollAutomationScanProgress(jobId),500)}
async function resumeAutomationScanProgress(){if(libraryScanState.resumeChecked||libraryScanState.jobId)return;libraryScanState.resumeChecked=true;try{const job=await api('/api/automation/library/scan/progress',null,{timeoutMs:4000});if(['queued','running'].includes(String(job?.status||''))&&job.job_id){libraryScanState.jobId=String(job.job_id);libraryScanState.itemId=String(job.ident||'');renderAutomationScanProgress(job);void pollAutomationScanProgress(libraryScanState.jobId)}}catch(_e){}}
async function scanAutomationLibrary(id=''){if(libraryScanState.jobId){toast('A library scan is already running. Showing its current progress.','info');return}const b=id?$('autoItemScan'):$('automationScanBtn');libraryScanState.buttonDone=beginButtonActivity(b,id?'Starting scan…':'Starting library scan…');libraryScanState.itemId=String(id||'');if(id)setAutomationItemActivity('Preparing scan…','NewzDeck is starting a real-time library scan.');try{const job=await api('/api/automation/library/scan/start',{id},{timeoutMs:10000,timeoutMessage:'NewzDeck could not start the library scan in time.'});if(!job?.job_id)throw new Error('NewzDeck did not return a library scan job ID.');libraryScanState.jobId=String(job.job_id);libraryScanState.itemId=String(job.ident||id||'');renderAutomationScanProgress(job);if(job.already_running)toast('A library scan was already running; attached to its live progress.','info');void pollAutomationScanProgress(libraryScanState.jobId)}catch(e){if(libraryScanState.buttonDone){libraryScanState.buttonDone();libraryScanState.buttonDone=null}libraryScanState.jobId='';libraryScanState.itemId='';setAutomationItemActivity('');toast(e.message,'error')}}
...
async function openReleaseSearch(itemId,season=null,episode=null){
  ...
  try{
    ...
    $('releaseSearchBody').innerHTML=(d.errors||[]).map(x=>`<div class="release-indexer-error">${escapeHtml(x.indexer)}: ${escapeHtml(x.error)}</div>`).join('')+failedBanner+summary+(rows.length?`<div class="release-table"><div class="release-row header"><span>Release / decision</span><span>Quality</span><span>Size</span><span>Age</span><span>Score</span><span>Actions</span></div>${rows.map((r,i)=>{const age=r.published?Math.max(0,(Date.now()/1000-r.published)/86400):null;const decision=r.recommended?'RECOMMENDED':r.blacklisted?(r.blacklist_failed?'FAILED':'BLACKLISTED'):r.automatic_eligible?'ELIGIBLE':r.accepted?'MANUAL ONLY':'REJECTED';const profileRank=Number(r.profile_quality_rank||r.quality_rank||0),selectionScore=Number(r.selection_score??r.effective_score??r.score??0),sizeBonus=Number(r.selection_size_bonus||0),indexerPenalty=Number(r.selection_indexer_penalty||0);const components=(r.score_components||[]).map(c=>`<li><span>${escapeHtml(c.label)}</span><b class="${Number(c.score)>=0?'positive':'negative'}">${Number(c.score)>=0?'+':''}${Number(c.score)}</b></li>`).join('');const rejects=(r.rejections||[]).map(x=>`<li class="reject">${escapeHtml(x)}</li>`).join('');const why=`<details class="release-why"><summary>Why this release?</summary><div><ul>${components}</ul>${rejects?`<div class="release-rejections"><b>Rejected because</b><ul>${rejects}</ul></div>`:''}</div></details>`;return `<div class="release-row ${r.accepted?'accepted':'rejected'} ${r.recommended?'recommended':''}"><span><div class="release-title-line"><strong>${escapeHtml(r.title)}</strong><em class="release-decision ${decision.toLowerCase().replace(/\s+/g,'-')}">${decision}</em>${r.season_pack?'<em class="release-pack-badge">SEASON PACK</em>':''}</div><small>#${Number(r.rank||i+1)} • ${escapeHtml(r.indexer||'')} • ${escapeHtml((r.reasons||[]).slice(0,3).join(' • '))}</small>${why}</span><span>${escapeHtml(r.parsed?.quality||'Unknown')}<small>${escapeHtml([r.parsed?.codec,r.parsed?.hdr,r.parsed?.audio,r.parsed?.release_group].filter(Boolean).join(' • '))}</small></span><span>${r.size?formatBytes(r.size):'—'}</span><span>${age==null?'—':age<1?`${Math.max(1,Math.round(age*24))}h`:`${Math.round(age)}d`}</span><span><b class="release-score ${r.score>=0?'positive':'negative'}">${r.score>=0?'+':''}${r.score}</b><small>${profileRank?`profile #${profileRank}`:'outside profile'}${selectionScore!==Number(r.score||0)?` • selection ${selectionScore}`:''}${sizeBonus?` • size +${sizeBonus}`:''}${indexerPenalty?` • indexer -${indexerPenalty}`:''}</small></span><span class="release-actions"><button class="primary-btn compact" data-grab-release="${i}" ${r.accepted&&!r.blacklisted?'':'disabled'}>${r.season_pack?'Grab pack':'Grab'}</button><button class="secondary-btn compact" data-blacklist-release="${i}" ${r.blacklisted?'disabled':''}>${r.blacklisted?(r.blacklist_failed?'Failed':'Blocked'):'Blacklist'}</button></span></div>`}).join('')}</div>`:automationEmpty('⌕','No releases found',seasonPack?'No safe season packs were returned. You can still search individual episodes.':'Check your indexer configuration or try again later.'));
...
els.providerForm.onsubmit=async(e)=>{e.preventDefault();const done=beginButtonActivity($('saveProviderBtn'),'Saving provider…');try{const d=await api('/api/providers/save',providerFormData());state.providerId=d.provider.id;await loadProviders();editProvider(d.provider.id);toast('Provider saved.','success');if(state.onboardingActive){closeProviderModal();showWelcomeStep('folder')}}catch(err){toast(err.message,'error')}finally{done()}};
...
$('deleteProviderBtn').onclick=async()=>{const id=$('providerId').value;if(!id)return;if(!confirm('Delete this provider profile?'))return;const done=beginButtonActivity($('deleteProviderBtn'),'Deleting…');try{await api('/api/providers/delete',{id});if(state.providerId===id)state.providerId='';await loadProviders();newProvider();toast('Provider deleted.')}catch(e){toast(e.message,'error')}finally{done()}};
...
document.querySelectorAll('[data-download-filter]').forEach(b=>b.onclick=()=>{state.downloadFilter=b.dataset.downloadFilter;if(['completed','failed'].includes(state.downloadFilter)){state.downloadHistoryLimit=50;state.downloadHistoryOffset=0;state.downloadHistoryLoaded=[];}document.querySelectorAll('[data-download-filter]').forEach(x=>x.classList.toggle('active',x===b));loadDownloads({render:true,quiet:true});});if($('removeAllFailedBtn'))$('removeAllFailedBtn').onclick=()=>removeAllFailedDownloads();if($('downloadHistorySearch'))$('downloadHistorySearch').oninput=()=>{state.downloadSearchTerm=$('downloadHistorySearch').value||'';if(['completed','failed'].includes(state.downloadFilter)&&state.downloadSearchTerm.trim()){state.downloadHistoryLimit=500;state.downloadHistoryOffset=0;state.downloadHistoryLoaded=[];loadDownloads({render:true,quiet:true})}else renderDownloads()};
...
function scheduleNextDownloadPoll(delay=null){
  ...
  const c=state.downloadSnapshot?.counts||{};
  const busy=Number(c.downloading||0)+Number(c.queued||0)+Number(c.retry_wait||0)+Number(c.cancelling||0)+Number(state.downloadSnapshot?.post_processing_active||0)>0;
  // v3.6.48: the authoritative snapshot is large enough that 4 Hz polling creates
  // avoidable localhost/SAB contention. Keep active progress responsive at 2 Hz,
  // relax an idle Downloads page, and poll hidden/non-Downloads views even less.
