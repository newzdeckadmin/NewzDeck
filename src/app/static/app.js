...
const manualImportState={itemId:'',preview:null};
...
function refreshManualImportEpisodeOptions(){
  ...
  select.innerHTML=episodes.map(ep=>`<option value="${Number(ep.episode_number)}" ${Number(ep.episode_number)===prior?'selected':''}>S${String(season).padStart(2,'0')}E${String(ep.episode_number).padStart(2,'0')} · ${escapeHtml(ep.name||'Episode')}</option>`).join('');
  if(!select.value&&episodes.length)select.value=String(episodes[0].episode_number);
}
function refreshManualImportMode(){const episodeMode=$('manualImportMode')?.value==='episode';$('manualImportEpisodeField')?.classList.toggle('hidden',!episodeMode);refreshManualImportEpisodeOptions();manualImportState.preview=null;$('manualImportCommit').disabled=true;$('manualImportPreview').innerHTML='<div class="manual-import-hint">Choose the external media folder, then preview exactly how NewzDeck will rename, move, and reconcile the files.</div>'}
function openManualMediaImport(item){
  if(!item||item.kind!=='tv')return;manualImportState.itemId=String(item.id||'');manualImportState.preview=null;
  ...
  $('manualImportModal').classList.remove('hidden');
}
function closeManualMediaImport(){$('manualImportModal').classList.add('hidden');manualImportState.preview=null}
function invalidateManualImportPreview(message='Selection changed. Preview the import again before committing.'){manualImportState.preview=null;const commit=$('manualImportCommit');if(commit)commit.disabled=true;const preview=$('manualImportPreview');if(preview)preview.innerHTML=`<div class="manual-import-hint">${escapeHtml(message)}</div>`}
async function browseManualImportSource(){const b=$('manualImportBrowse'),old=b.textContent;b.disabled=true;b.textContent='Opening…';try{const r=await api('/api/automation/manual-import/choose-source',{initial:$('manualImportSource').value||localStorage.getItem('newzdeckManualImportSource')||'',title:`Choose folder containing media for ${manualImportItem()?.title||'this TV show'}`});if(r.cancelled)return;$('manualImportSource').value=r.folder||'';if(r.folder)localStorage.setItem('newzdeckManualImportSource',r.folder);manualImportState.preview=null;$('manualImportCommit').disabled=true;$('manualImportPreview').innerHTML='<div class="manual-import-hint">Folder selected. Preview the import before committing any changes.</div>'}catch(e){toast(e.message,'error')}finally{b.disabled=false;b.textContent=old}}
...
function renderManualImportPreview(d){
  manualImportState.preview=d;const rows=d.inspection||[],c=d.counts||{},cutoff=d.cutoff||'profile cutoff';
...
async function commitManualMediaImport(){if(!manualImportState.preview?.ok)return;const payload=manualImportPayload(),b=$('manualImportCommit'),old=b.textContent;b.disabled=true;b.textContent='Importing…';try{const d=await api('/api/automation/manual-import/commit',payload);if(!d.ok)throw new Error(d.reason||'Manual Media Import could not be completed.');await loadAutomation({quiet:true});closeManualMediaImport();await openAutomationItem(payload.item_id);const moved=Number(d.imported_count||0),kept=Number(d.kept_existing||0),satisfied=Number(d.wanted_satisfied_count||0);toast(`Manual Import complete • ${moved} file${moved===1?'':'s'} organized${kept?` • ${kept} existing kept`:''}${satisfied?` • ${satisfied} Wanted target${satisfied===1?'':'s'} satisfied`:''}.`,'success')}catch(e){toast(e.message,'error');b.disabled=false;b.textContent=old}}
