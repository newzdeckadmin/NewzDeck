...
async function restoreConfigBackup(file){if(!file)return;const done=beginButtonActivity($('settingsRestoreBtn'),'Restoring…');try{const text=await file.text();const payload=JSON.parse(text);const saved=await api('/api/config/restore',payload);await loadProviders();populateSettingsForm(saved,await api('/api/health'));toast('Configuration restored. Existing download queue was left unchanged.','success')}catch(e){toast(`Restore failed: ${e.message}`,'error')}finally{$('settingsRestoreInput').value='';done()}}
