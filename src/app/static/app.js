...
function pumpThumbQueue(){
  while(state.thumbActive<state.thumbConcurrency&&state.thumbQueue.length){
    ...
    if(pos<0)return;
    const task=state.thumbQueue.splice(pos,1)[0],sampleStarted=performance.now(),queueWaitMs=Math.max(0,Date.now()-Number(task.queuedAt||Date.now()));perfRecord('thumbnail_queue',queueWaitMs,true,{reason:task.role||'item'});
    ...
    (async()=>{
      ...
      }catch(e){const sameContext=task.group===state.selectedGroup&&task.provider===state.providerId,live=task.role==='set-cover'?(sameContext&&task.browseSession===state.browseSessionToken):(task.generation===state.galleryGeneration&&sameContext);if(live)markThumbnailError(task.index,e,task.kind,task.role,task.setKey);else if(task.role==='set-cover')state.relatedCoverStats.staleSessionDrops++;}
...
async function thumbnailImageApiTimed(payload,options={},reason='primary'){const started=performance.now();try{const data=await api('/api/thumbnail/image',payload,options),total=performance.now()-started;perfRecord('thumbnail_http',total,true,{reason});recordPairedThumbnailTransport(total,data?.thumbnail_server_ms,true,reason);return data}catch(e){const total=performance.now()-started,errorReason=thumbnailErrorReason(e);perfRecord('thumbnail_http',total,false,{reason:errorReason});recordPairedThumbnailTransport(total,e?.data?.thumbnail_server_ms,false,errorReason);throw e}}
...
async function fetchImageThumbnail(a,task=null){
  ...
  const request=(async()=>{
    ...
    try{return await finishImageThumbnailResponseTimed(a,await thumbnailImageApiTimed(payload,browseRequestOptions(task?.requestTimeoutMs?{timeoutMs:Number(task.requestTimeoutMs),timeoutMessage:'Related Media cover timed out. You can retry this cover.'}:{}),'primary'))}
    ...
    try{
      ...
      if(full?.url){
        try{const result=await finishImageThumbnailResponseTimed(a,await thumbnailImageApiTimed(payload,browseRequestOptions(),'retry-after-full-preview'));recoveryOK=true;recoveryReason='thumbnail-retry';return result}
