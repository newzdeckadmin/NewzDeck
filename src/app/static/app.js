...
async function fetchVideoThumbnail(a,task=null){
  ...
  const request=(async()=>{
    ...
    }catch(e){const failureReason=videoThumbnailPostFailureReason(e);postReason=`${demand}-${sampleClass}-${failureReason}`;if(['browser-decode-failed','ffmpeg-required'].includes(failureReason))perfRecord('video_thumbnail_policy',0,true,{reason:`${demand}-${sampleClass}-${failureReason}-nonretryable`});throw e}
