  ...
  if(manual){
    ...
    try{
      while(group===state.selectedGroup&&providerId===state.providerId){
        ...
        try{
          const data=await api('/api/articles/resolve-names',{provider_id:providerId,group,items:candidates.map(nameResolutionPayload)},browseRequestOptions({timeoutMs:75000,timeoutMessage:'Name resolution took too long. NewzDeck will retry this batch instead of marking the remaining files unavailable.'}));if(group!==state.selectedGroup||providerId!==state.providerId)return;resolved+=Number(data.resolved||0);const results=data.results||[];changed+=applyNameResolutionResults(results);if(results.length)renderArticles({preserveScroll:true,reason:'name-resolution-result'});else updateNameResolutionActivityDomInPlace();const transient=Number(data.retryable||0);if(transient){transientRounds++;const seconds=Math.max(2,Number(data.backoff_seconds||5));state.nameResolutionBackoffUntil=Math.max(state.nameResolutionBackoffUntil,Date.now()+seconds*1000);updateNameResolutionActivityDomInPlace();if(transientRounds>=4){toast('The provider is temporarily refusing or timing out on filename probes. NewzDeck paused instead of marking later files unavailable; it will retry automatically.');break}await new Promise(r=>setTimeout(r,seconds*1000+75));continue}transientRounds=0;
        }catch(e){failedBatches++;for(const a of candidates){const key=nameResolutionKey(a);state.nameResolutionAttempted.delete(key);state.nameResolutionDeferred.set(key,{message:String(e?.message||'Filename probe failed'),until:Date.now()+5000,code:'request_error'})}state.nameResolutionBackoffUntil=Math.max(state.nameResolutionBackoffUntil,Date.now()+5000);transientRounds++;renderArticles({preserveScroll:true,reason:'name-resolution-result'});if(transientRounds>=4)break}
    ...
    }finally{state.nameResolutionInFlight=false;if(group===state.selectedGroup){updateNameResolutionActivityDomInPlace();scheduleObfuscatedNameResolution()}}
    return;
  }
  const now=Date.now();if(state.nameResolutionBackoffUntil>now){scheduleObfuscatedNameResolution();return}
  const next=automaticNameResolutionCandidates(8),candidates=next.items;if(!candidates.length)return;for(const a of candidates)state.nameResolutionAttempted.add(nameResolutionKey(a));if(next.budgeted)state.nameResolutionAutoRemaining=Math.max(0,state.nameResolutionAutoRemaining-candidates.length);state.nameResolutionInFlight=true;updateNameResolutionActivityDomInPlace();try{const data=await api('/api/articles/resolve-names',{provider_id:providerId,group,items:candidates.map(nameResolutionPayload)},browseRequestOptions({timeoutMs:75000,timeoutMessage:'Name resolution took too long. NewzDeck will retry this batch.'}));if(group!==state.selectedGroup||providerId!==state.providerId)return;const results=data.results||[];applyNameResolutionResults(results);if(results.length)renderArticles({preserveScroll:true,reason:'name-resolution-result'});else updateNameResolutionActivityDomInPlace();if(Number(data.retryable||0)){const seconds=Math.max(2,Number(data.backoff_seconds||5));state.nameResolutionBackoffUntil=Math.max(state.nameResolutionBackoffUntil,Date.now()+seconds*1000)}}catch(e){for(const a of candidates){const key=nameResolutionKey(a);state.nameResolutionAttempted.delete(key);state.nameResolutionDeferred.set(key,{message:String(e?.message||'Filename probe failed'),until:Date.now()+5000,code:'request_error'})}state.nameResolutionBackoffUntil=Math.max(state.nameResolutionBackoffUntil,Date.now()+5000);renderArticles({preserveScroll:true,reason:'name-resolution-result'})}finally{state.nameResolutionInFlight=false;if(group===state.selectedGroup){updateNameResolutionActivityDomInPlace();scheduleObfuscatedNameResolution()}}
}
...
async function thumbnailImageApiTimed(payload,options={},reason='primary'){const started=performance.now();try{const data=await api('/api/thumbnail/image',payload,options);perfRecord('thumbnail_http',performance.now()-started,true,{reason});return data}catch(e){perfRecord('thumbnail_http',performance.now()-started,false,{reason:thumbnailErrorReason(e)});throw e}}
