...
function pumpThumbQueue(){
  while(state.thumbActive<state.thumbConcurrency&&state.thumbQueue.length){
    ...
    if(pos<0)return;
    const task=state.thumbQueue.splice(pos,1)[0],sampleStarted=performance.now(),queueWaitMs=Math.max(0,Date.now()-Number(task.queuedAt||Date.now()));
...
async function fetchImageThumbnail(a,task=null){
  ...
  })().finally(()=>{perfRecord('thumbnail',performance.now()-started);promiseMap.delete(promiseKey)});
