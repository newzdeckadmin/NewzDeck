...
const els = {
  ...
  articleLimit:$('articleLimit'), articleSort:$('articleSort'), contentFilter:$('contentFilter'), articleStatusFilter:$('articleStatusFilter'), jumpFirstUnseenBtn:$('jumpFirstUnseenBtn'), markAllSeenBtn:$('markAllSeenBtn'), galleryViewBtn:$('galleryViewBtn'), listViewBtn:$('listViewBtn'), thumbnailSize:$('thumbnailSize'), continuousBrowseBtn:$('continuousBrowseBtn'), groupRelatedBtn:$('groupRelatedBtn'),
...
function updateGroupRelatedButton(){if(!els.groupRelatedBtn)return;els.groupRelatedBtn.classList.toggle('active',state.groupRelatedMedia);els.groupRelatedBtn.setAttribute('aria-pressed',state.groupRelatedMedia?'true':'false');els.groupRelatedBtn.textContent=state.groupRelatedMedia?'▦ Grouped':'▦ Group related';}
...
function saveUiSettings(){
  clearTimeout(state.uiSaveTimer);state.uiSaveTimer=setTimeout(()=>api('/api/settings/save',{article_limit:Number(els.articleLimit?.value||300),thumbnail_size:state.thumbnailSize,continuous_browse:state.continuousMode,view_mode:state.viewMode,content_filter:els.contentFilter?.value||'images',download_organization:state.downloadOrganization||'flat',group_related_media:state.groupRelatedMedia,favorites:[...state.favorites],bookmark_folders:state.bookmarkFolders.map(f=>({id:f.id,name:f.name,groups:[...(f.groups||[])],collapsed:!!f.collapsed})),recent_groups:state.recentGroups,group_states:state.groupStates,blocked_posters:[...state.blockedPosters],group_seen_high:state.groupSeenHigh,group_read_states:compactReadStates(),browser_tabs:state.browserTabs.map(t=>({id:t.id,provider_id:t.provider_id,group:t.group,title:t.title||t.group})),active_browser_tab:state.activeBrowserTabId}).catch(()=>{}),250);
}
...
async function selectGroup(group,opts={}){if(!group)return;const previousGroup=state.selectedGroup,previousProvider=state.providerId;const tab=ensureBrowserTab(group,{newTab:false,tabId:opts.tabId||''});if(tab)state.activeBrowserTabId=tab.id;if(group===state.selectedGroup&&state.articles.length&&!opts.fromTab){state.loadedPages.clear();state.articlePage=1;state.articlePaging=null;state.galleryGeneration++;state.thumbQueue=[];state.thumbQueued.clear();els.articlesList.scrollTop=0;await loadArticles({page:1});return}captureCurrentGroupState();if(previousGroup&&previousGroup!==group)endGroupVisit(previousGroup,previousProvider);if(state.groupSearchJob&&['queued','scanning','cancelling'].includes(state.groupSearchJob.status)&&state.groupSearchJob.group!==group){api('/api/group-search/cancel',{id:state.groupSearchJob.id}).catch(()=>{});}if(state.groupSearchPollTimer){clearInterval(state.groupSearchPollTimer);state.groupSearchPollTimer=null}closeMediaViewer();state.selectedGroup=group;beginGroupVisit(group);markRecentGroup(group);renderBrowserTabs();state.selectedItems.clear();state.selectionAnchorKey='';state.articleSearchTerm='';state.activeMediaSetKey='';state.searchMode=false;state.groupSearchJob=null;state.continuousLoading=false;els.articleSearch.value='';state.galleryGeneration++;state.thumbQueue=[];state.thumbQueued.clear();renderGroups();updateSelectionBar();updateArticleSearchUi();els.entireGroupSearchBtn.disabled=false;els.articleTitle.textContent=group;els.articleEyebrow.textContent='VISUAL NEWSGROUP BROWSER';resetPreview();const key=groupStateKey(group),session=state.groupSessions.get(key),persisted=state.groupStates[key]||{};if(session){applyStoredGroupControls(session);state.articles=session.articles||[];state.articlePage=Number(session.articlePage||1);state.articlePaging=session.articlePaging||null;state.loadedPages=new Set(session.loadedPages||[state.articlePage]);state.articleSearchTerm=session.articleSearchTerm||'';state.activeMediaSetKey=session.activeMediaSetKey||'';state.selectedArticleKey=session.selectedArticleKey||'';state.searchMode=!!session.searchMode;state.groupSearchJob=session.groupSearchJob||null;state.browsePageBeforeSearch=Number(session.browsePageBeforeSearch||1);els.articleSearch.value=state.articleSearchTerm;if(state.searchMode&&state.groupSearchJob)els.articleEyebrow.textContent='ENTIRE NEWSGROUP SEARCH';sortArticles(false);renderArticles({preserveScroll:true,scrollTop:Number(session.scrollTop||0)});updateArticlePaging();renderArticleSummary();requestAnimationFrame(()=>{els.articlesList.scrollTop=Number(session.scrollTop||0);markGroupSeen(group)});return}applyStoredGroupControls(persisted);state.selectedArticleKey='';state.articlePage=Math.max(1,Number(persisted.page||1));state.articlePaging=null;state.loadedPages.clear();state.browseScrollTop=0;els.articlesList.scrollTop=0;await loadArticles({page:state.articlePage});requestAnimationFrame(()=>{els.articlesList.scrollTop=Number(persisted.scroll_top||0);markGroupSeen(group)});}
...
function renderArticleSummary(elapsedMs=0){
  ...
  const pages=[...state.loadedPages].sort((a,b)=>a-b);const pageText=pages.length>1?`<span><b>Pages ${pages[0].toLocaleString()}–${pages[pages.length-1].toLocaleString()}</b> loaded</span>`:(pages.length?`<span><b>Page ${pages[0].toLocaleString()}</b></span>`:'');
  const depth=daysBackFromLoaded();const depthText=state.articles.length?`<span class="browse-depth"><b>${state.articles.length.toLocaleString()}</b> posts loaded • ${depth?`${depth.toLocaleString()} day${depth===1?'':'s'} back`:'latest'}</span>`:'';
...
async function loadArticles({page=null,append=false,refresh=false}={}){
  ...
  try{
    const data=await api('/api/articles',{provider_id:state.providerId,group:state.selectedGroup,limit:Number(els.articleLimit.value),page:targetPage,media_only:false,refresh});
    ...
    if(append)state.loadedPages.add(Number(data.paging?.page||targetPage));else state.loadedPages=new Set([Number(data.paging?.page||targetPage)]);
...
async function loadOlderArticles(){
  ...
  await loadArticles({page:state.articlePage+1,append:true});
}
function updateArticlePaging(){
  ...
  if(searchPaging){
    ...
    const loaded=[...state.loadedPages].sort((a,b)=>a-b);const loadedText=loaded.length>1?`Pages ${loaded[0].toLocaleString()}–${loaded[loaded.length-1].toLocaleString()} loaded • `:'';
...
function setArticleLoading(on,append=false){
  ...
  if(on){els.articlesList.className='articles-list media-gallery';applyThumbnailSize();els.articlesList.innerHTML='<div class="gallery-loading">'+('<div class="gallery-skeleton"></div>'.repeat(8))+'</div>'}
  else{els.articleLimit.disabled=false;els.articleSort.disabled=false;els.contentFilter.disabled=false;els.galleryViewBtn.disabled=false;els.listViewBtn.disabled=false;els.articleSearch.disabled=false;els.clearArticleSearchBtn.disabled=false;els.entireGroupSearchBtn.disabled=!state.selectedGroup;if(els.thumbnailSize)els.thumbnailSize.disabled=false;if(els.continuousBrowseBtn)els.continuousBrowseBtn.disabled=false;updateArticlePaging();updateArticleSearchUi();applyThumbnailSize();updateContinuousButton()}
}
...
function captureBrowseAnchors(limit=12){
  if(!els.articlesList)return[];const listRect=els.articlesList.getBoundingClientRect();const nodes=els.articlesList.querySelectorAll('.media-card[data-index],.article-row[data-index],.media-set-card[data-set-key]');const out=[];
...
function renderArticles({preserveScroll=true,scrollTop=null,anchor=null}={}){
  ...
  const items=filteredArticles();updateArticleSearchUi(items.length);els.articlesList.classList.remove('empty-state','media-gallery','article-list-mode');
  els.articlesList.classList.add(state.viewMode==='gallery'?'media-gallery':'article-list-mode');applyThumbnailSize();
  els.galleryViewBtn.classList.toggle('active',state.viewMode==='gallery');els.listViewBtn.classList.toggle('active',state.viewMode==='list');
  if(!items.length){
    els.articlesList.classList.add('empty-state');
  ...
  if(state.viewMode==='gallery')renderGallery(items);else renderList(items);
...
function updateNewContentBoundaryDom(){if(!els.articlesList||state.searchMode||(els.articleSort?.value||'newest')!=='newest'){els.articlesList?.querySelectorAll('.new-content-boundary').forEach(x=>x.remove());return}const nodes=[...els.articlesList.querySelectorAll('.media-card[data-index],.article-row[data-index]')];els.articlesList.querySelectorAll('.new-content-boundary').forEach(x=>x.remove());if(!nodes.length)return;let sawNew=false,boundaryBefore=null;for(const node of nodes){const a=state.articles[Number(node.dataset.index)];if(isArticleNew(a)){sawNew=true;continue}if(sawNew){boundaryBefore=node;break}}if(!sawNew)return;const d=document.createElement('div');d.className='new-content-boundary';d.innerHTML='<span>NEW SINCE LAST VISIT</span><small>Previously available posts continue below</small>';if(boundaryBefore)boundaryBefore.parentElement.insertBefore(d,boundaryBefore);else{const sentinel=$('continuousSentinel');if(sentinel?.parentElement)sentinel.parentElement.insertBefore(d,sentinel);else nodes[nodes.length-1].parentElement.appendChild(d)}}
...
function renderList(items){
  els.articlesList.innerHTML=items.map(({a,index})=>{
    const media=a.media,key=articleKey(a),selected=state.selectedItems.has(key),dkey=itemDownloadKey(a),downloaded=!!media&&state.downloadedIndex.has(dkey),queued=!!media&&state.queuedIndex.has(dkey);const chip=media?`<span class="media-chip ${media.kind}">${media.kind.toUpperCase()}</span>`:'<span class="media-chip file">POST</span>';
    const comp=a.segment_total>1?`<span class="completion ${a.complete?'':'incomplete'}">${a.segment_count}/${a.segment_total} parts</span>`:'';const check=media&&a.complete&&selected?'<span class="row-selection-check" aria-hidden="true">✓</span>':'';const dl=downloaded?'<span class="completion">✓ Downloaded</span>':queued?'<span class="completion">⇣ Queued</span>':'';
    return `<div class="article-row ${articleStatusClass(a)} ${media&&a.complete?'selectable':''} ${selected?'selected':''} ${state.selectedArticleKey===key?'active keyboard-active':''}" data-index="${index}" data-download-key="${escapeHtml(dkey)}" aria-selected="${selected?'true':'false'}" title="${media&&a.complete?'Click to select • Ctrl-click toggles • Shift-click selects a range':'Click to preview'}"><div class="article-top">${chip}<div class="article-subject">${escapeHtml(a.subject)}</div>${articleStatusBadge(a)}${check}</div><div class="article-meta">${''}<span>${shortDate(a.date)}</span><span>${formatBytes(a.bytes)}</span><span class="poster">${escapeHtml(a.from)}</span>${comp}${dl}</div></div>`;
  }).join('')+continuousSentinelMarkup();
  els.articlesList.querySelectorAll('.article-row').forEach(r=>{r.onclick=e=>handleMediaSelectionClick(Number(r.dataset.index),e);r.ondblclick=e=>{const a=state.articles[Number(r.dataset.index)];if(['image','video'].includes(a?.media?.kind)&&a.complete){e.preventDefault();openMediaViewer(a)}};r.oncontextmenu=e=>{e.preventDefault();const a=state.articles[Number(r.dataset.index)];if(!a)return;showContextMenu(e.clientX,e.clientY,[{label:'Preview',action:()=>{handleMediaSelectionClick(Number(r.dataset.index),e);renderPreviewDetails(a,true)}},{label:'Add to download queue',disabled:!isSelectableMedia(a),action:()=>downloadItems([a])},{separator:true},{label:isArticleSeen(a)?'Mark post unseen':'Mark post seen',action:()=>setArticlesSeen([a],!isArticleSeen(a),{toastResult:true})},{separator:true},{label:'Copy filename',action:()=>copyText(a.media?.filename||a.subject)},{label:'Copy subject',action:()=>copyText(a.subject||'')},{label:'Copy Message-ID',action:()=>copyText(a.message_id||'')},{separator:true},{label:isPosterBlocked(a)?'Unmute this poster':'Mute this poster',action:()=>mutePoster(a.from)},{label:'Filter to this poster',action:()=>{state.articleSearchTerm=`from:${a.from}`;els.articleSearch.value=state.articleSearchTerm;renderArticles()}},{label:'Show details',action:()=>{state.selectedArticleKey=articleKey(a);renderPreviewDetails(a,false)}}])}});updateNewContentBoundaryDom();
}
...
function scanThumbnailDemand(){
  if(!els.articlesList||!state.selectedGroup||state.viewMode!=='gallery')return;
...
function setView(mode){state.viewMode=mode;localStorage.setItem('usenetViewMode',mode);saveUiSettings();renderArticles();}
...
function resetPreview(){
  ...
  els.previewContent.innerHTML='<div class="preview-orb">▦</div><h3>Visual newsgroup browser</h3><p>Image and video thumbnails load as you scroll. Click one for a larger preview, then download only what you want.</p><div class="feature-mini"><span>✓</span> Faster image + video thumbnails</div><div class="feature-mini"><span>✓</span> yEnc + multipart assembly</div><div class="feature-mini"><span>✓</span> Selective downloads</div>';
}
...
async function downloadItems(items,button=null){
  const valid=items.filter(a=>a?.media&&a.complete);if(!valid.length){toast('No complete media items selected.','error');return}
  ...
  try{
    ...
    for(const [group,groupItems] of byGroup){const payload=groupItems.map(a=>({segments:segmentPayload(a),media:a.media,subject:a.subject||'',from:a.from||'',message_id:a.message_id||''}));const data=await api('/api/downloads/add',{provider_id:state.providerId,group,items:payload});added+=data.added?.length||0;dupes+=data.duplicates?.length||0;if(data.disk_warning)warnings.push(data.disk_warning)}
    if(added)toast(`${added} file${added===1?'':'s'} added to the download queue.`,'success');if(dupes)toast(`${dupes} item${dupes===1?' was':'s were'} already queued.`);if(warnings[0])toast(warnings[0],'error');
...
function updateAutomationLiveIndicators(){
  ...
  if(state.automationTab==='setup'){
    ...
    if(status){
      ...
      status.textContent=enabled?(q.active?`◷ Quiet hours • resumes ${q.resume_ts?new Date(Number(q.resume_ts)*1000).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}):c.automatic_quiet_end}`:(a.running?'● Continuous Automation is checking now':'✓ Continuous Automation enabled')):'Continuous Automation is off';
    }
...
function wantedRow(x,type){const current=type==='upgrade'?`<span class="wanted-current">Current: ${escapeHtml(x.current_quality||'Unknown')} → ${escapeHtml(x.cutoff||'cutoff')}</span>`:'';const auto=(state.automation?.automatic?.target_states||{})[x.target_key||'']||{};const labels={queued:'QUEUED',grabbed:'QUEUED',searching:'SEARCHING',waiting:'WAITING',needs_root:'ROOT OFFLINE',error:'RETRYING',retrying:'RETRYING',downloading:'DOWNLOADING',processing:'PROCESSING',cancelling:'CANCELLING',release_detected:'RELEASE FOUND',quiet_hours:'QUIET',imported:'IMPORTED'};const pct=Number(auto.progress||0)>0?` ${Math.round(Number(auto.progress))}%`:'';const ast=labels[auto.status]?`<span class="wanted-pill auto ${escapeHtml(auto.status)}" title="${escapeHtml(auto.message||'')}">${labels[auto.status]}${pct}</span>`:'';const availability=x.kind==='movie'&&x.availability?` • ${x.availability==='home'?'Digital/physical release':x.availability==='assumed_home'?'Home-release fallback':'Theatrical release'}`:'';const why=`<span class="wanted-why"><b>${escapeHtml(x.reason_label|| (type==='upgrade'?'Quality below cutoff':'Missing file'))}</b><small>${escapeHtml(x.reason_detail||'')}${auto.last_selection_reason?` • Last choice: ${escapeHtml(auto.last_selection_reason)}`:''}</small></span>`;return `<div class="wanted-row"><div><strong>${escapeHtml(x.label)}</strong><small>${escapeHtml(x.episode_name||'')}${x.date?` • ${autoFmtDate(x.date)}`:''}${availability}${auto.message?` • ${escapeHtml(auto.message)}`:''}</small></div>${why}${current}<span class="wanted-pill ${type}">${type==='upgrade'?'UPGRADE':'MISSING'}</span>${ast}<button class="primary-btn compact" data-release-search="${escapeHtml(x.item_id)}" data-season="${x.season??''}" data-episode="${x.episode??''}">Search releases</button></div>`}
  ...
  $('automationContent').innerHTML=`<div class="automation-setup-grid">
    ...
    <article class="automation-setup-card automation-auto-card"><div class="eyebrow">CONTINUOUS AUTOMATION</div><h2>Always-on Monitoring</h2><p>NewzDeck can watch recent Newznab RSS releases every few minutes, discover newly aired episodes through metadata refreshes, fall back to scheduled Wanted searches, automatically retry misses, and queue acceptable releases without opening the app.</p>
      ...
      <div class="continuous-toggle-list">
        ...
        <label class="automation-check continuous-toggle"><input type="checkbox" id="automaticBacklogEnabled" ${c.automatic_backlog_enabled?'checked':''}><span><b>Search existing missing backlog</b><small>Include media that was already missing when Continuous Automation was enabled.</small></span></label>
        ...
        <label class="automation-check continuous-toggle"><input type="checkbox" id="automaticSeasonPacksEnabled" ${c.automatic_season_packs_enabled!==false?'checked':''}><span><b>Prefer complete season packs</b><small>Use a season pack when multiple released episodes are missing and the monitored season has fully aired.</small></span></label>
...
async function saveAutomationItem(item){const mode=$('autoItemMonitorMode')?.value||automationMonitoringMode(item);try{const payload={id:item.id,monitored:mode!=='none',monitor_mode:mode,quality_profile_id:$('autoItemProfile').value,root_folder:$('autoItemRoot').value};if(item.kind==='tv'){const entered=$('autoItemLibraryTitle')?.value.trim()||'',prior=String(item.library_title||'').trim();if(entered!==prior)payload.library_title=entered}await api('/api/automation/media/update',payload);await loadAutomation();toast(`Monitoring & library naming saved • ${automationMonitoringChoices(item.kind).find(x=>x.value===mode)?.label||'Updated'}.`,'success')}catch(e){toast(e.message,'error')}}
