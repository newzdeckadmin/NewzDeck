const state = {
  ...
  downloadSnapshot: {paused:false,jobs:[],collections:[],counts:{},folder:''}, downloadedIndex: new Set(), queuedIndex: new Set(), downloadIndexSignature:'', downloadFilter:'active', downloadHistoryLimit:50, downloadSearchTerm:'', selectedDownloads:new Set(), downloadSelectionAnchor:'', expandedDownloads:new Set(), expandedCollections:new Set(), postProgressMemory:new Map(), activeView:'browse', downloadOrganization:'flat',
...
async function loadDownloads({render=true,livePatch=false,quiet=false}={}){
  ...
  try{
    ...
    const limit=terminalView?Math.max(50,Number(state.downloadHistoryLimit||50)):200;
    const downloads=await api(`/api/downloads?scope=${encodeURIComponent(scope)}&limit=${limit}`,null,{timeoutMs:3500,timeoutMessage:'Downloads status took too long to respond. NewzDeck will retry automatically.'});
...
function renderDownloads({livePatch=false}={}){
  ...
  if(['completed','failed'].includes(state.downloadFilter)&&view.has_more){
    ...
    if(more)more.onclick=async()=>{more.disabled=true;more.textContent='Loading…';state.downloadHistoryLimit=Math.min(500,Number(state.downloadHistoryLimit||50)+50);await loadDownloads({render:true,quiet:false})};
  }
...
document.querySelectorAll('[data-download-filter]').forEach(b=>b.onclick=()=>{state.downloadFilter=b.dataset.downloadFilter;if(['completed','failed'].includes(state.downloadFilter))state.downloadHistoryLimit=50;document.querySelectorAll('[data-download-filter]').forEach(x=>x.classList.toggle('active',x===b));loadDownloads({render:true,quiet:true});});if($('removeAllFailedBtn'))$('removeAllFailedBtn').onclick=()=>removeAllFailedDownloads();if($('downloadHistorySearch'))$('downloadHistorySearch').oninput=()=>{state.downloadSearchTerm=$('downloadHistorySearch').value||'';if(['completed','failed'].includes(state.downloadFilter)&&state.downloadSearchTerm.trim()){state.downloadHistoryLimit=500;loadDownloads({render:true,quiet:true})}else renderDownloads()};
