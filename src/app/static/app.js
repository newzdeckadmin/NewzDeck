...
function mediaSetRepresentative(group){const complete=group.members.filter(x=>x.a?.complete),candidates=complete.filter(x=>!state.unpreviewableMediaKeys.has(previewKey(x.a))),pool=candidates.length?candidates:(complete.length?complete:group.members),ranked=[...pool].map(liveMediaSetItem).sort((x,y)=>{const ax=x.a,ay=y.a;const cx=(state.imageThumbCache.has(previewKey(ax))||state.videoThumbCache.has(previewKey(ax)))?0:1,cy=(state.imageThumbCache.has(previewKey(ay))||state.videoThumbCache.has(previewKey(ay)))?0:1;if(cx!==cy)return cx-cy;const sx=Number(ax.segment_count||1),sy=Number(ay.segment_count||1);if(sx!==sy)return sx-sy;const bx=Number(ax.bytes||0)||Number.MAX_SAFE_INTEGER,by=Number(ay.bytes||0)||Number.MAX_SAFE_INTEGER;if(bx!==by)return bx-by;return Number(ay.article||0)-Number(ax.article||0)});return ranked[0]||liveMediaSetItem(group.members[0])}
...
async function fetchVideoThumbnail(a,task=null){
  ...
  const request=(async()=>{
    ...
    }catch(e){postReason=`${demand}-${sampleClass}-${videoThumbnailPostFailureReason(e)}`;throw e}
